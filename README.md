...
|:--|:--|:--|
| `irs-reduction-threshold-lower` | At `delta = P/Q`, `irsCertifiedGammaError(delta) <= epsilon*` and `q(delta) <= 2^-b` | maximize `B` |
| `irs-reduction-threshold-upper` | For every admissible `delta >= delta* = i/2^18`, `winningSetSoundness(delta) > epsilon*`, and `2^-b <= q(delta*)` | minimize `B` |
...
ArkLib proves `winningSetSoundness <= irsCertifiedGammaError`, this is a
...
unsafe suffix because no monotonicity theorem for `winningSetSoundness` is
