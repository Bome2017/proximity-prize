...
theorem exists_bchks_weighted_factor_selection
  ...
  let Good := S \ Bad
  have hGoodLarge :
      2 * 40826205 * 32414 * 312 ^ 2 + (bchksErrors + 1) * 312 < Good.card := by
    simpa [Good] using card_sdiff_large_for_bchks S Bad hS hBad
  have hGoodNonempty : Good.Nonempty := by
    apply Finset.card_pos.mp
    exact (by norm_num : 0 <
      2 * 40826205 * 32414 * 312 ^ 2 + (bchksErrors + 1) * 312).trans hGoodLarge
  obtain ⟨z₀, hz₀⟩ := hGoodNonempty
  have hQdeg : Q.natDegree ≤ 312 := by
  ...
  have hCandidates : Candidates.card ≤ 312 := by
    exact (Finset.card_filter_le _ _).trans
      (positiveNormalizedFactorPairs_card_le' Q hQ x₀ 312 hQdeg hx₀)
  have hRXdeg : ∀ R ∈ UniqueFactorizationMonoid.normalizedFactors Q,
      (triSpecializeX R x₀).natDegree ≤ 312 := by
    intro R hR
  ...
  have hweighted :
      (∑ RH ∈ Candidates, RH.1.natDegree * RH.2.natDegree) ≤ 312 ^ 2 := by
  ...
  have hCapacityLarge :
    ...
    exact (Nat.add_le_add_left
      ...
      (2 * 40826205 * 32414 * 312 ^ 2)).trans_lt hGoodLarge
  obtain ⟨RH, hRHcand, hRHlarge⟩ :=
    exists_pair_fiber_gt_BCHKS_capacity Good.attach Candidates Rel
      ...
      40826205 32414 312 bchksErrors hcover hweighted hCapacityLarge
