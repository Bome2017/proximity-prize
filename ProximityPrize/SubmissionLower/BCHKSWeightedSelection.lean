...
theorem exists_bchks_weighted_factor_selection
  ...
  let Good := S \ Bad
  have hGoodLarge :
      2 * 40866320 * 32414 * 312 ^ 2 + (bchksErrors + 1) * 312 < Good.card := by
    simpa [Good] using card_sdiff_large_for_bchks S Bad hS hBad
  have hGoodNonempty : Good.Nonempty := by
    apply Finset.card_pos.mp
    exact (by norm_num : 0 <
      2 * 40866320 * 32414 * 312 ^ 2 + (bchksErrors + 1) * 312).trans hGoodLarge
  ...
  have hCapacityLarge :
    ...
    exact (Nat.add_le_add_left
      ...
      (2 * 40866320 * 32414 * 312 ^ 2)).trans_lt hGoodLarge
  obtain ⟨RH, hRHcand, hRHlarge⟩ :=
    exists_pair_fiber_gt_BCHKS_capacity Good.attach Candidates Rel
      ...
      40866320 32414 312 bchksErrors hcover hweighted hCapacityLarge
