...
the interpolation `YZ` cap makes its size at most `32413` (and hence also the
...
theorem exists_bchks_Qbad_removal
  ...
  have hdeg : ((Q.coeff j).coeff a).natDegree < 32414 := by
    have := hQYZ j a ha0
    omega
  let Sgood := S.filter (fun z => triSpecializeZ Q z ≠ 0)
  have hbad : (badZSpecializations Q S).card ≤ 32413 :=
    badZSpecializations_card_le_32413 Q S j a ha0 hdeg
  ...
  · simpa [badZSpecializations] using
      hbad.trans (by norm_num : 32413 ≤ 2 * 32414 * 312)
