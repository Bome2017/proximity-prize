...
the interpolation `YZ` cap makes its size at most `261560` (and hence also the
...
theorem exists_bchks_Qbad_removal
  ...
  have hdeg : ((Q.coeff j).coeff a).natDegree < 261561 := by
    have := hQYZ j a ha0
    omega
  let Sgood := S.filter (fun z => triSpecializeZ Q z ≠ 0)
  have hbad : (badZSpecializations Q S).card ≤ 261560 :=
    badZSpecializations_card_le_261560 Q S j a ha0 hdeg
  ...
  · simpa [badZSpecializations] using
      hbad.trans (by norm_num : 261560 ≤ 2 * 261561 * 729)
