...
the interpolation `YZ` cap makes its size at most `453560` (and hence also the
...
theorem exists_bchks_Qbad_removal
    ...
    ∃ Sgood : Finset F,
      ...
      (2 * 104951682 * 453561) * 801 +
          (634000 * 453561) * 801 ^ 2 +
          (bchksErrors + 1) * 801 + 2 * 453561 * 801 < Sgood.card := by
  classical
  ...
  have hdeg : ((Q.coeff j).coeff a).natDegree < 453561 := by
    have := hQYZ j a ha0
    omega
  let Sgood := S.filter (fun z => triSpecializeZ Q z ≠ 0)
  have hbad : (badZSpecializations Q S).card ≤ 453560 :=
    badZSpecializations_card_le_453560 Q S j a ha0 hdeg
  ...
  · simpa [badZSpecializations] using
      hbad.trans (by norm_num : 453560 ≤ 2 * 453561 * 801)
