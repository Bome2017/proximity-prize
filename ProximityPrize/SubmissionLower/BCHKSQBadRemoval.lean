...
theorem exists_bchks_Qbad_removal
    ...
    ∃ Sgood : Finset F,
      ...
      2 * 33398999 * 63302 * 255 ^ 2 +
          (bchksErrors + 1) * 255 + 2 * 63302 * 255 < Sgood.card := by
  classical
  ...
  have hdeg : ((Q.coeff j).coeff a).natDegree < 63302 := by
    have := hQYZ j a ha0
    omega
  let Sgood := S.filter (fun z => triSpecializeZ Q z ≠ 0)
  have hbad : (badZSpecializations Q S).card ≤ 63301 :=
    badZSpecializations_card_le_63301 Q S j a ha0 hdeg
  ...
  · simpa [badZSpecializations] using
      hbad.trans (by norm_num : 63301 ≤ 2 * 63302 * 255)
