...
the interpolation `YZ` cap makes its size at most `549940` (and hence also the
...
theorem exists_bchks_Qbad_removal
    ...
    ∃ Sgood : Finset F,
      ...
      (S.filter (fun z => triSpecializeZ Q z = 0)).card ≤
        2 * 549941 * 885 ∧
      632199 * 549940 * 885 ^ 2 +
          (bchksErrors + 1) * 885 + 2 * 549941 * 885 < Sgood.card := by
  classical
  ...
  have hdeg : ((Q.coeff j).coeff a).natDegree < 549941 := by
    have := hQYZ j a ha0
    omega
  let Sgood := S.filter (fun z => triSpecializeZ Q z ≠ 0)
  have hbad : (badZSpecializations Q S).card ≤ 549940 :=
  ...
  · simpa [badZSpecializations] using
      hbad.trans (by norm_num : 549940 ≤ 2 * 549941 * 885)
