...
def radius6401 : ℝ≥0 := claimedRadius 313467 1048576
def errors6401 : ℕ := 78366
...
theorem radius6401_cell_cross :
    313467 * Fintype.card IRSProfile.Index < (errors6401 + 1) * 1048576 := by
...
theorem radius6401_power_rational_bound :
    (1 - radius6401) ^ IRSProfile.repetitions ≤
      ((1 : ℝ≥0) / 2 ^ (65 : ℕ)) * ((1000000000 : ℝ≥0) / 1006955551) ^ (58 : ℕ) := by
  rw [← NNReal.coe_le_coe]
  norm_num [radius6401, claimedRadius, IRSProfile.repetitions, div_pow,
    div_le_iff₀]

theorem radius6401_score :
    (1 - radius6401) ^ IRSProfile.repetitions ≤ claimedError 6558 := by
  ...
  have hscale :
      ((1000000000 : ℝ≥0) / 1006955551) ^ (58 : ℕ) ≤
        (2 : ℝ≥0) ^ (-((58 : ℝ) / 100)) := by
    calc
      ((1000000000 : ℝ≥0) / 1006955551) ^ (58 : ℕ) ≤
          ((2 : ℝ≥0) ^ (-((1 : ℝ) / 100))) ^ (58 : ℕ) :=
        pow_le_pow_left₀ (by positivity) hbase 58
      _ = (2 : ℝ≥0) ^ (-((58 : ℝ) / 100)) := by
        rw [← NNReal.rpow_natCast (((2 : ℝ≥0) ^ (-((1 : ℝ) / 100)))) 58,
          ← NNReal.rpow_mul]
        norm_num
  calc
    (1 - radius6401) ^ IRSProfile.repetitions ≤
        ((1 : ℝ≥0) / 2 ^ (65 : ℕ)) * ((1000000000 : ℝ≥0) / 1006955551) ^ (58 : ℕ) :=
      radius6401_power_rational_bound
    _ ≤ ((1 : ℝ≥0) / 2 ^ (65 : ℕ)) *
        (2 : ℝ≥0) ^ (-((58 : ℝ) / 100)) :=
      mul_le_mul_of_nonneg_left hscale (by positivity)
    _ = claimedError 6558 := by
      unfold claimedError
      rw [show -((((6558 : ℕ) : ℝ) / 100)) =
          -((65 : ℕ) : ℝ) + -((58 : ℝ) / 100) by norm_num,
...
theorem lambda6401_le_of_alignment
  ...
  AlignmentInterleavedLambda.irs_squared_claimedRadius_lambda_le
    313467 1048576 errors6401 budget6401 (by norm_num) radius6401_cell_cross
...
theorem protocolClaim6401_of_alignment
    ...
    ProtocolClaim 6558 313467 1048576 where
  admissible := by
