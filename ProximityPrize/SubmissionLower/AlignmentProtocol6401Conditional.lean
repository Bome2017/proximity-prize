...
/-!
# Conditional protocol certificate at 64.92 bits

Model label: gpt-5.

The sole mathematical premise of the final theorem is the explicit strong
alignment bound at 77701 errors with budget 10^17. The actual squared-eight
...
def radius6401 : ℝ≥0 := claimedRadius 310807 1048576
def errors6401 : ℕ := 77701
...
theorem radius6401_cell_cross :
    310807 * Fintype.card IRSProfile.Index < (errors6401 + 1) * 1048576 := by
...
theorem radius6401_power_rational_bound :
    (1 - radius6401) ^ IRSProfile.repetitions ≤
      ((1 : ℝ≥0) / 2 ^ (64 : ℕ)) * ((1000000000 : ℝ≥0) / 1006955551) ^ (92 : ℕ) := by
  rw [← NNReal.coe_le_coe]
  norm_num [radius6401, claimedRadius, IRSProfile.repetitions, div_pow,
    div_le_iff₀]

theorem radius6401_score :
    (1 - radius6401) ^ IRSProfile.repetitions ≤ claimedError 6492 := by
  ...
  have hscale :
      ((1000000000 : ℝ≥0) / 1006955551) ^ (92 : ℕ) ≤
        (2 : ℝ≥0) ^ (-((92 : ℝ) / 100)) := by
    calc
      ((1000000000 : ℝ≥0) / 1006955551) ^ (92 : ℕ) ≤
          ((2 : ℝ≥0) ^ (-((1 : ℝ) / 100))) ^ (92 : ℕ) :=
        pow_le_pow_left₀ (by positivity) hbase 92
      _ = (2 : ℝ≥0) ^ (-((92 : ℝ) / 100)) := by
        rw [← NNReal.rpow_natCast (((2 : ℝ≥0) ^ (-((1 : ℝ) / 100)))) 92,
          ← NNReal.rpow_mul]
        norm_num
  calc
    (1 - radius6401) ^ IRSProfile.repetitions ≤
        ((1 : ℝ≥0) / 2 ^ (64 : ℕ)) * ((1000000000 : ℝ≥0) / 1006955551) ^ (92 : ℕ) :=
      radius6401_power_rational_bound
    _ ≤ ((1 : ℝ≥0) / 2 ^ (64 : ℕ)) *
        (2 : ℝ≥0) ^ (-((92 : ℝ) / 100)) :=
      mul_le_mul_of_nonneg_left hscale (by positivity)
    _ = claimedError 6492 := by
      unfold claimedError
      rw [show -((((6492 : ℕ) : ℝ) / 100)) =
          -((64 : ℕ) : ℝ) + -((92 : ℝ) / 100) by norm_num,
...
theorem lambda6401_le_of_alignment
  ...
  AlignmentInterleavedLambda.irs_squared_claimedRadius_lambda_le
    310807 1048576 errors6401 budget6401 (by norm_num) radius6401_cell_cross
...
theorem protocolClaim6401_of_alignment
    (halign : AffineLineAlignmentBound IRSProfile.baseCode
      77701 100000000000000000) :
    ProtocolClaim 6492 310807 1048576 where
  admissible := by
