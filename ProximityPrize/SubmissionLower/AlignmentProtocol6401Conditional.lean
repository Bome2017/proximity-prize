...
/-!
# Conditional protocol certificate at 65.69 bits

Model label: gpt-5.

The sole mathematical premise of the final theorem is the explicit strong
alignment bound at 78473 errors at the protected seed budget. The actual squared-eight
...
def radius6401 : ℝ≥0 := claimedRadius 313895 1048576
def errors6401 : ℕ := 78473
...
theorem radius6401_cell_cross :
    313895 * Fintype.card IRSProfile.Index < (errors6401 + 1) * 1048576 := by
...
theorem radius6401_score :
  ...
  have hscale :
      ((1000000000 : ℝ≥0) / 1006955551) ^ (69 : ℕ) ≤
        (2 : ℝ≥0) ^ (-((69 : ℝ) / 100)) := by
    calc
      ((1000000000 : ℝ≥0) / 1006955551) ^ (69 : ℕ) ≤
          ((2 : ℝ≥0) ^ (-((1 : ℝ) / 100))) ^ (69 : ℕ) :=
        pow_le_pow_left₀ (by positivity) hbase 69
      _ = (2 : ℝ≥0) ^ (-((69 : ℝ) / 100)) := by
        rw [← NNReal.rpow_natCast (((2 : ℝ≥0) ^ (-((1 : ℝ) / 100)))) 69,
          ← NNReal.rpow_mul]
        norm_num
  calc
    (1 - radius6401) ^ IRSProfile.repetitions ≤
    ...
    _ = claimedError 6569 := by
      unfold claimedError
      rw [show -((((6569 : ℕ) : ℝ) / 100)) =
          -((65 : ℕ) : ℝ) + -((69 : ℝ) / 100) by norm_num,
...
theorem lambda6401_le_of_alignment
  ...
  AlignmentInterleavedLambda.irs_squared_claimedRadius_lambda_le
    313895 1048576 errors6401 budget6401 (by norm_num) radius6401_cell_cross
...
theorem protocolClaim6401_of_alignment
    ...
    ProtocolClaim 6569 313895 1048576 where
  admissible := by
