...
alignment bound at 77045 errors with budget 10^17. The actual squared-eight
...
def radius6401 : ℝ≥0 := claimedRadius 308183 1048576
def errors6401 : ℕ := 77045
...
theorem radius6401_cell_cross :
    308183 * Fintype.card IRSProfile.Index < (errors6401 + 1) * 1048576 := by
...
/-- Upper rational approximation to `2^(26/100)`. -/
theorem two_rpow_one_hundred_le :
    (2 : ℝ≥0) ^ ((26 : ℝ) / 100) ≤ (6 : ℝ≥0) / 5 := by
  have hroot :
      ((2 : ℝ≥0) ^ (26 : ℕ)) ^ ((100 : ℝ)⁻¹) ≤ (6 : ℝ≥0) / 5 := by
...
theorem radius6401_score :
    (1 - radius6401) ^ IRSProfile.repetitions ≤ claimedError 6426 := by
  have hscale : (5 : ℝ≥0) / 6 ≤ (2 : ℝ≥0) ^ (-((26 : ℝ) / 100)) := by
    calc
      (5 : ℝ≥0) / 6 = 1 / ((6 : ℝ≥0) / 5) := by norm_num
      _ ≤ 1 / ((2 : ℝ≥0) ^ ((26 : ℝ) / 100)) :=
  ...
  calc
    ...
    _ = claimedError 6426 := by
      unfold claimedError
      rw [show -((((6426 : ℕ) : ℝ) / 100)) =
          -((64 : ℕ) : ℝ) + -((26 : ℝ) / 100) by norm_num,
...
theorem lambda6401_le_of_alignment
  ...
  AlignmentInterleavedLambda.irs_squared_claimedRadius_lambda_le
    308183 1048576 errors6401 budget6401 (by norm_num) radius6401_cell_cross
