...
/-! .
 -/
namespace ProximityPrize.SubmissionLower.ContactMovingSeedless6731Research
...
theorem coefficient_count_exact :
    coefficientCount weightedCap w yTotalCap slopeCap = 2131882082 := by decide
theorem local_rank_exact :
    localRankBound multiplicity yTotalCap slopeCap = 8132 := by decide
theorem nullity_exact :
    coefficientCount weightedCap w yTotalCap slopeCap -
      n*localRankBound multiplicity yTotalCap slopeCap = 127074 := by
...
theorem list_numeric_values :
    agreements=182071 ∧ gap=51000 ∧ weightedCap=7464911 ∧
    singularListCap=1176 ∧ listNumerator=41369186410467 ∧
    listNumerator/gap+1=811160518 ∧
    listNumerator<811160518*gap ∧ 811160518<listBudget := by decide
...
def radius : ℝ≥0 := claimedRadius 320295 1048576
def score : ℕ := 6731
theorem radius_numerator_exact : 320295 = 4*errors+3 := by decide
theorem radius_floor :
    ⌊(radius : ℝ)*(Fintype.card IRSProfile.Index : ℝ)⌋₊ = errors := by
  norm_num [radius, claimedRadius, errors, IRSProfile.Index]
theorem radius_cell_cross :
    320295*Fintype.card IRSProfile.Index < (errors+1)*1048576 := by
...
/-- . -/
theorem root_power_integer : (2 : ℕ)^31 * 146^100 ≤ 181^100 := by decide
theorem radius_power_integer :
    (728281 : ℕ)^128 * (2^67*181) ≤ 146*1048576^128 := by decide

theorem two_rpow_fraction_le :
    (2 : ℝ≥0)^((31 : ℝ)/100) ≤ (181 : ℝ≥0)/146 := by
  have hroot : ((2 : ℝ≥0)^(31 : ℕ))^((100 : ℝ)⁻¹) ≤ (181 : ℝ≥0)/146 := by
    rw [NNReal.rpow_inv_le_iff (by norm_num : (0 : ℝ)<100)]
    rw [NNReal.rpow_ofNat,div_pow,le_div_iff₀ (by positivity)]
    exact_mod_cast root_power_integer
  calc
    (2 : ℝ≥0)^((31 : ℝ)/100)=((2 : ℝ≥0)^(31 : ℕ))^((100 : ℝ)⁻¹) := by
      rw [← NNReal.rpow_natCast_mul]
      norm_num [div_eq_mul_inv]
    _ ≤ (181 : ℝ≥0)/146 := hroot

theorem radius_power_rational_bound :
    (1-radius)^IRSProfile.repetitions ≤ ((1 : ℝ≥0)/2^(67 : ℕ))*(146/181) := by
  have hsub : (1-radius : ℝ≥0)=728281/1048576 := by
    have hr : radius≤1 := by
    ...
    norm_num [radius,claimedRadius]
  change (1-radius)^128 ≤ ((1 : ℝ≥0)/2^(67 : ℕ))*(146/181)
...
theorem radius_score : (1-radius)^IRSProfile.repetitions ≤ claimedError score := by
  have hscale : (146 : ℝ≥0)/181 ≤ (2 : ℝ≥0)^(-((31 : ℝ)/100)) := by
    calc
      (146 : ℝ≥0)/181=1/((181 : ℝ≥0)/146) := by norm_num
      _ ≤ 1/((2 : ℝ≥0)^((31 : ℝ)/100)) :=
        one_div_le_one_div_of_le (by positivity) two_rpow_fraction_le
      _ = (2 : ℝ≥0)^(-((31 : ℝ)/100)) := by rw [one_div,NNReal.rpow_neg]
  calc
    (1-radius)^IRSProfile.repetitions ≤ ((1 : ℝ≥0)/2^(67 : ℕ))*(146/181) :=
      radius_power_rational_bound
    _ ≤ ((1 : ℝ≥0)/2^(67 : ℕ))*(2 : ℝ≥0)^(-((31 : ℝ)/100)) :=
      mul_le_mul_of_nonneg_left hscale (by positivity)
    _ = claimedError score := by
      unfold claimedError score
      rw [show -((((6731 : ℕ) : ℝ)/100)) =
          -((67 : ℕ) : ℝ)+-((31 : ℝ)/100) by norm_num,
...
theorem field_capacity_split :
    2^(128 : ℕ) * (ContactMovingParameters6731Research.mcaBudget + listBudget) ≤
...
/-- .
 -/
theorem protocolClaim6731_of_reduction
    ...
    ProtocolClaim 6731 320295 1048576 := by
  exact ⟨radius_admissible, hred, radius_score⟩

end
end ProximityPrize.SubmissionLower.ContactMovingSeedless6731Research
