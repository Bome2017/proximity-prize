...
theorem coefficient_count_exact:
    coefficientCount weightedCap w yTotalCap slopeCap=2364557378:=by decide
theorem local_rank_exact:
    localRankBound multiplicity yTotalCap slopeCap=9020:=by decide
theorem nullity_exact:
    coefficientCount weightedCap w yTotalCap slopeCap-
      n*localRankBound multiplicity yTotalCap slopeCap=18498:=by
...
theorem list_numeric_values:
    agreements=182052∧gap=50981∧weightedCap=7828236∧
    singularListCap=1239∧listNumerator=43585392724533∧
    listNumerator/gap+1=854934049∧
    listNumerator<854934049*gap∧listBudget=854934049:=by decide
...
theorem root_power_integer:(2:ℕ)^33*599^100 ≤ 753^100:=by decide
theorem radius_power_integer:
    (23302529:ℕ)^128*(2^67*753) ≤ 599*33554432^128:=by decide
theorem two_rpow_fraction_le:
    (2:ℝ≥0)^((33:ℝ)/100) ≤ (753:ℝ≥0)/599:=by
  have hroot:((2:ℝ≥0)^(33:ℕ))^((100:ℝ)⁻¹) ≤ (753:ℝ≥0)/599:=by
...
theorem radius_power_rational_bound:
    (1-radius)^IRSProfile.repetitions ≤ ((1:ℝ≥0)/2^(67:ℕ))*(599/753):=by
  have hsub:(1-radius:ℝ≥0)=23302529/33554432:=by
    ...
    norm_num [radius,claimedRadius]
  change (1-radius)^128 ≤ ((1:ℝ≥0)/2^(67:ℕ))*(599/753)
...
theorem radius_score:(1-radius)^IRSProfile.repetitions ≤ claimedError score:=by
  have hscale:(599:ℝ≥0)/753 ≤ (2:ℝ≥0)^(-((33:ℝ)/100)):=by
    calc
      (599:ℝ≥0)/753=1/((753:ℝ≥0)/599):=by norm_num
      _ ≤ 1/((2:ℝ≥0)^((33:ℝ)/100)):=
        one_div_le_one_div_of_le (by positivity) two_rpow_fraction_le
      _=(2:ℝ≥0)^(-((33:ℝ)/100)):=by rw [one_div,NNReal.rpow_neg]
  calc
    (1-radius)^IRSProfile.repetitions ≤ ((1:ℝ≥0)/2^(67:ℕ))*(599/753):=
