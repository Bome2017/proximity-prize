...
theorem weightedCost_reducedResidualAgreementFlag
    ...
    B.weightedCost (reducedResidualAgreementFlag support d) C =
      d * B.weightedCost (reducedAgreementDirection support) C +
...
theorem flagMixed_reducedResidualAgreement_direction
    (p : FlagDegree) (support : ResidualSupportParameters) (d : ℕ) :
    flagMixed p (reducedResidualAgreementFlag support d)
        (reducedAgreementDirection support) =
      d * flagMixed p (reducedAgreementDirection support)
          (reducedAgreementDirection support) +
        flagMixed p (reducedAgreementDirection support) unitYZFlag := by
  simp [flagMixed, reducedResidualAgreementFlag, reducedAgreementDirection,
    unitYZFlag]
  ring

theorem flagMixed_reducedResidualAgreement_unit
    (p : FlagDegree) (support : ResidualSupportParameters) (d : ℕ) :
    flagMixed p (reducedResidualAgreementFlag support d) unitYZFlag =
      d * flagMixed p (reducedAgreementDirection support) unitYZFlag +
        flagMixed p unitYZFlag unitYZFlag := by
  simp [flagMixed, reducedResidualAgreementFlag, reducedAgreementDirection,
    unitYZFlag]
  ring

theorem flagMixed_reducedResidualAgreement_z
    (p : FlagDegree) (support : ResidualSupportParameters) (d : ℕ) :
    flagMixed p (reducedResidualAgreementFlag support d) unitZFlag =
      d * flagMixed p (reducedAgreementDirection support) unitZFlag +
        flagMixed p unitYZFlag unitZFlag := by
  simp [flagMixed, reducedResidualAgreementFlag, reducedAgreementDirection,
    unitYZFlag, unitZFlag]
  ring

theorem proper_cut_seed_bound_of_recursive_prime_flag_budget_reduced_z_yz
    (hphi : Function.Injective phi)
    (F : MvPolynomial (Fin 4) K) (G T : MvPolynomial (Fin 3) Omega)
    (selected : K → Polynomial K) (Gamma : Finset K)
    (nodes : Finset Iota) (x u0 u1 : Iota → K)
    (p e d a U V : ℕ) [CharP Omega p]
    (surfaceFlag : FlagDegree) (support : ResidualSupportParameters)
    (hdiv : G ∣ surfaceMap phi F)
    (hGflag : PolynomialInFlag surfaceFlag G)
    (hTflag : PolynomialInFlag (reducedResidualAgreementFlag support d) T)
    ...
    Gamma.card * (a - d) ≤
      U * flagMixed surfaceFlag (reducedResidualAgreementFlag support d)
          (reducedAgreementDirection support) +
        V * flagMixed surfaceFlag (reducedResidualAgreementFlag support d)
          unitYZFlag +
        (e + 1) * (a - d) *
          (flagMixed surfaceFlag (reducedResidualAgreementFlag support d)
              unitZFlag +
            flagMixed surfaceFlag (reducedResidualAgreementFlag support d)
              unitYZFlag) := by
  classical
  ...
  let degreeCost : RegularComponent Omega G T H → ℕ :=
    fun C ↦ B.weightedCost (reducedAgreementDirection support) C
  ...
  have hcomponent : ∀ C : RegularComponent Omega G T H,
    ...
    let S := regularComponentCurveStageOfSupport support F G T selected Gamma
      ...
      (reducedResidualAgreementFlag support d) hdiv hGflag hTflag
    ...
    apply recursive_curve_stratified_incidence_of_prime_flag_budget_for_cuts
      hphi S (reducedResidualAgreementFlag support)
      (fun r ↦ B.weightedCost r C) (B.primeBudget C)
      (degreeCost C) (unitCost C) U V (largeCost C)
    · intro t
      simpa only [degreeCost, unitCost] using
        weightedCost_reducedResidualAgreementFlag B support C t
    · intro D _ i _
      exact agreement_in_flag_mod_ideal phi support
        ⟨D.stage.surface_s_weight, D.stage.surface_ys_weight,
          D.stage.surface_total_weight⟩
        D.stage.primeIdeal D.stage.surface_mem_primeIdeal D.degree
    ...
    · intro D hmany
      ...
      have hcharged : (e + 1) * (a - d) ≤
          (e + 1) * (a - d) * largeCost C := by
        have hm := Nat.mul_le_mul_left ((e + 1) * (a - d))
          (hzyzPositive C)
        simpa only [largeCost, Nat.mul_one] using hm
  ...
  have hlargeSum : (∑ C : RegularComponent Omega G T H, largeCost C) ≤
      flagMixed surfaceFlag (reducedResidualAgreementFlag support d) unitZFlag +
        flagMixed surfaceFlag (reducedResidualAgreementFlag support d)
          unitYZFlag := by
    calc
  ...
  exact aggregate_component_stratified_incidence G T H Gamma
    ...
    (flagMixed surfaceFlag (reducedResidualAgreementFlag support d)
      (reducedAgreementDirection support))
    (flagMixed surfaceFlag (reducedResidualAgreementFlag support d) unitYZFlag)
    (flagMixed surfaceFlag (reducedResidualAgreementFlag support d) unitZFlag +
      flagMixed surfaceFlag (reducedResidualAgreementFlag support d) unitYZFlag)
    degreeCost unitCost largeCost hcomponent
    (by simpa only [degreeCost] using
      B.sum_weightedCost_le (reducedAgreementDirection support))
...
theorem terminal_outer_fiber_bound_of_prime_flag_budget_profile_reduced_z_yz
    (hphi : Function.Injective phi) (p : Profile)
    (support : ResidualSupportParameters)
    {flag : FlagDegree}
    ...
    (B : PrimeFlagBudgetFamily
      (G := D.stage.G)
      (T := reducedAgreementPolynomial phi support D.stage.F D.degree
        (x i) (D.stage.u0 i) (D.stage.u1 i))
    ...
    (Gamma.filter (fun gamma ↦ D.stage.Agrees gamma i)).card * p.gap ≤
      D.degree * factorDegreeCostYZ p (reducedAgreementDirection support) flag +
        factorUnitCostYZ p (reducedAgreementDirection support) flag := by
  classical
  let GammaI := Gamma.filter (fun gamma ↦ D.stage.Agrees gamma i)
  let Torig := agreementPolynomial phi D.stage.F D.degree
    (x i) (D.stage.u0 i) (D.stage.u1 i)
  let Tred := reducedAgreementPolynomial phi support D.stage.F D.degree
    (x i) (D.stage.u0 i) (D.stage.u1 i)
  ...
  have hd : D.stage.G ∣ Torig - Tred :=
    D.stage.G_dvd_surface.trans
      (agreementPolynomial_sub_reduced_dvd phi support D.stage.F D.degree
        (x i) (D.stage.u0 i) (D.stage.u1 i))
  have hTpoint : ∀ gamma ∈ GammaI,
      MvPolynomial.eval (selectedPoint phi D.stage.selected gamma) Tred = 0 := by
    intro gamma hgamma
    obtain ⟨hGamma, hagree⟩ := Finset.mem_filter.mp hgamma
    have horig : MvPolynomial.eval
        (selectedPoint phi D.stage.selected gamma) Torig = 0 :=
      (selected_agreement_zero_iff phi D.stage.F D.stage.selected
    ...
    have hzero : MvPolynomial.eval
        (selectedPoint phi D.stage.selected gamma) (Torig - Tred) = 0 := by
      rw [hQ, map_mul, D.stage.on_component gamma hGamma, zero_mul]
    simpa only [map_sub, horig, zero_sub, neg_eq_zero] using hzero
  ...
  have hdegree : ∀ k ≤ D.degree,
    ...
    have htotal : total ≤ p.w := by dsimp only [total]; omega
    ...
    have hn : p.n - total = D.stage.nodes.card - k := by
      ...
      dsimp only [total]
      omega
    have hw : p.w - total = D.degree - k := by dsimp only [total]; omega
    have ha : p.agreements - total = aD - k := by dsimp only [total, aD]; omega
    rw [hgap]
    rw [hn, hw, ha] at h
    exact h
  have hunit : ∀ k ≤ D.degree,
    ...
    exact h
  have hbound :=
    proper_cut_seed_bound_of_recursive_prime_flag_budget_reduced_z_yz
      hphi D.stage.F D.stage.G Tred D.stage.selected GammaI D.stage.nodes
      x D.stage.u0 D.stage.u1 pchar p.errors D.degree aD
      p.degreeIncidence p.unitIncidence flag support
      ...
      (by simpa only [Tred] using hzyzPositive) hdegree hunit
  have hgapEq : aD - D.degree = p.gap := by
    have := D.degree_le
    dsimp only [aD, Profile.gap]
    omega
  rw [hgapEq] at hbound
  rw [flagMixed_reducedResidualAgreement_direction,
    flagMixed_reducedResidualAgreement_unit,
    flagMixed_reducedResidualAgreement_z] at hbound
  simp only [factorDegreeCostYZ, factorUnitCostYZ]
  calc
    _ ≤ _ := hbound
    _ = _ := by ring

theorem recursive_scaled_factorReducedYZ_of_adaptive_projection_families
    (hphi : Function.Injective phi) (p : Profile)
    (support : ResidualSupportParameters)
    {flag : FlagDegree}
    (S : ResidualStage phi Gamma x pchar p.errors flag p.w support)
    ...
    (hunitGlobal : ∀ k ≤ p.w,
      (p.n - k) * p.gap ≤ p.unitIncidence * (p.agreements - k))
    (hprojection : TerminalAdaptiveProjectionFamiliesReducedYZ support S) :
    Gamma.card * p.gap ^ 2 ≤
      factorRegularLedgerYZForDirection p
        (reducedAgreementDirection support) flag := by
  apply recursive_scaled_factorYZ hphi p support
    (reducedAgreementDirection support) S hwa hagreement
  · intro D i hi hproper
    obtain ⟨base, ⟨P⟩⟩ := hprojection D i hi hproper
    exact terminal_outer_fiber_bound_of_prime_flag_budget_profile_reduced_z_yz
