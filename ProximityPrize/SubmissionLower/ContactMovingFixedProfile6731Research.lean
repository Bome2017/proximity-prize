...
/-! Fixed 67.31 profile and the linear proper/identity factor ledger. -/
namespace ProximityPrize.SubmissionLower.ContactMovingFixedProfile6731Research
...
def fixedProfile : Profile := ⟨262144,131071,182071,7282840,1205,11⟩

def fixedSupport : ResidualSupportParameters :=
  ContactMovingAgreementCertificate6719Research.support 1150 43 9

def fixedTightProfile : TightParameters :=
  ⟨262144,131071,182071,7282840,1205,11⟩
...
def factorLedger (p : Profile) (a b s : ℕ) (flag : FlagDegree) : ℕ :=
  (p.w+1)*factorDegreeCost p a b s flag + factorUnitCost p a b s flag +
...
theorem sum_factorLedger_le {J : Type*} [Fintype J]
  ...
  simp only [factorLedger, factorDegreeCost, factorUnitCost, identityDegree,
...
def fixedFactorLedger : ℕ := factorLedger fixedProfile 1150 43 9 fixedFlag

theorem fixed_values :
    fixedProfile.errors=80073 ∧ fixedProfile.gap=51000 ∧
    fixedProfile.yCap=55 ∧ fixedProfile.degreeIncidence=9624450283 ∧
    fixedProfile.unitIncidence=131073 ∧ fixedFlag=⟨1150,44,11⟩ ∧
    identityDegree fixedProfile 1150 43 9 fixedFlag=
      ContactMovingParameters6731Research.identityZDegree+
        ContactMovingParameters6731Research.identityYZDegree := by decide

theorem fixed_regular_ceiling :
    fixedFactorLedger/fixedProfile.gap+2 =
      ContactMovingParameters6731Research.fixedRegularCost := by decide

theorem fixed_singular_ceiling : fixedTightProfile.countCap+1 =
    ContactMovingParameters6731Research.fixedSingularCost := by decide

theorem fixed_inclusive_cost_le :
    (fixedFactorLedger*fixedProfile.gap)/fixedProfile.gap^2+
      fixedTightProfile.countCap ≤ ContactMovingParameters6731Research.fixedRegularCost+
        ContactMovingParameters6731Research.fixedSingularCost := by decide
...
theorem fixed_flag_gates :
    ...
    2*fixedSupport.total*(1150+43+9+4) < prime := by decide
...
end ProximityPrize.SubmissionLower.ContactMovingFixedProfile6731Research
