...
theorem concrete_incidence_inequality (dH d D tcard : ℕ)
    (ht : 2 * 104951682 * dH * d * D + 76717 + 1 ≤ tcard) :
    (262144 - 76717 - 131071) * tcard >
...
theorem concrete_many_large_fibers
  ...
  apply many_large_fibers T A 262144 76717 131071
...
/-- Alignment-only incidence arithmetic.  Once the unused full-`DX`
coefficient-vanishing conjunct is dropped, `634000 * D` challenges per
degree product already force the same `k+1` rich coordinate fibers. -/
theorem concrete_incidence_inequality_alignment_only (dH d D tcard : ℕ)
    (ht : 634000 * D * dH * d + 76717 + 1 ≤ tcard) :
    (262144 - 76717 - 131071) * tcard >
      (262144 - 131071) * ((2 * 131071 + 2) * dH * d * D) := by
  norm_num at ht ⊢
  nlinarith

theorem concrete_many_large_fibers_alignment_only
    {ι ζ : Type} [Fintype ι] [DecidableEq ι] [DecidableEq ζ]
    (T : Finset ζ) (A : ζ → Finset ι) (dH d D : ℕ)
    (hn : Fintype.card ι = 262144)
    (hrow : ∀ z ∈ T, 262144 - 76717 ≤ (A z).card)
  ...
  apply many_large_fibers T A 262144 76717 131071
    ((2 * 131071 + 2) * dH * d * D) hn hrow
  exact concrete_incidence_inequality_alignment_only dH d D T.card hT
