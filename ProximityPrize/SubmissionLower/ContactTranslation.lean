...
open ContactRankKernel ContactInterpolation ContactAlignmentParameters
...
theorem exists_frozen_translated_contact_interpolant
    ...
    ∃ Q : MvPolynomial (Fin 4) IRSProfile.Field,
      Q ≠ 0 ∧ Q ∈ globalCoefficientBox IRSProfile.Field 3688860 131071 164 6 ∧
      Q.degreeOf (1 : Fin 4) ≤ yCap ∧
      ∀ (i : IRSProfile.Index) (r : ℕ),
        slopeDifference IRSProfile.Field ^ (20 - r) ∣
          (homogenizedTranslation IRSProfile.Field
            (IRSProfile.domain i) (u₀ i) (u₁ i) Q).coeff r := by
  obtain ⟨Q, θ, hQ, hcaps, hY, hreconstruct, hequations⟩ :=
    exists_frozen_nonzero_polynomial_and_equations u₀ u₁
  refine ⟨Q, hQ, hcaps, hY, ?_⟩
  intro i r
  rw [hreconstruct, translation_reconstruct_coeff]
  exact all_blocks_divisible_of_equations IRSProfile.Field 3688860 131071 164 6 20
