...
/- Four original contact kernels share one ambient coefficient box. Their
...
abbrev Ambient := CoefficientIndex 12738110 131071 100000 20 → K
...
theorem auxBox_le_aBox : globalCoefficientBox K 11464299 131071 100000 18 ≤
    globalCoefficientBox K 12738110 131071 100000 20 := by
  intro Q hQ d hd
  obtain ⟨ht, hs, hc⟩ := hQ hd
  exact ⟨ht, hs.trans (by decide), hc.trans_le (by decide)⟩

def embedAux (u0 u1 : I → K) : AuxKernel u0 u1 →ₗ[K] Ambient :=
  polynomialCoefficientsLinear 12738110 131071 100000 20
    (kernelReconstructLinear (K := K) 11464299 131071 100000 18 63
      IRSProfile.domain u0 u1)

@[simp] theorem reconstruct_embedAux (u0 u1 : I → K) (v : AuxKernel u0 u1) :
    reconstruct K 12738110 131071 100000 20 (embedAux u0 u1 v) =
      reconstruct K 11464299 131071 100000 18 v.1 := by
  have hbox : kernelReconstructLinear (K := K) 11464299 131071 100000 18 63
      IRSProfile.domain u0 u1 v ∈ globalCoefficientBox K 12738110 131071 100000 20 := by
    rw [kernelReconstructLinear_apply]
    exact auxBox_le_aBox (reconstruct_mem_globalCoefficientBox
      K 11464299 131071 100000 18 v.1)
  have h := reconstruct_polynomialCoefficientsLinear 12738110 131071 100000 20
    (kernelReconstructLinear (K := K) 11464299 131071 100000 18 63
      IRSProfile.domain u0 u1) v hbox
  simpa only [embedAux, kernelReconstructLinear_apply] using h

theorem cBox_le_aBox : globalCoefficientBox K 8188785 131071 100000 13 ≤
    globalCoefficientBox K 12738110 131071 100000 20 := by
  intro Q hQ d hd
  obtain ⟨ht, hs, hc⟩ := hQ hd
  exact ⟨ht, hs.trans (by decide), hc.trans_le (by decide)⟩
...
def embedC (u0 u1 : I → K) : CKernel u0 u1 →ₗ[K] Ambient :=
  polynomialCoefficientsLinear 12738110 131071 100000 20
    (kernelReconstructLinear (K := K) 8188785 131071 100000 13 45
      IRSProfile.domain u0 u1)

@[simp] theorem reconstruct_embedC (u0 u1 : I → K) (v : CKernel u0 u1) :
    reconstruct K 12738110 131071 100000 20 (embedC u0 u1 v) =
      reconstruct K 8188785 131071 100000 13 v.1 := by
  have hbox : kernelReconstructLinear (K := K) 8188785 131071 100000 13 45
      IRSProfile.domain u0 u1 v ∈ globalCoefficientBox K 12738110 131071 100000 20 := by
    rw [kernelReconstructLinear_apply]
    exact cBox_le_aBox (reconstruct_mem_globalCoefficientBox
      K 8188785 131071 100000 13 v.1)
  have h := reconstruct_polynomialCoefficientsLinear 12738110 131071 100000 20
    (kernelReconstructLinear (K := K) 8188785 131071 100000 13 45
      IRSProfile.domain u0 u1) v hbox
  simpa only [embedC, kernelReconstructLinear_apply] using h

theorem thinBox_le_aBox : globalCoefficientBox K 8188785 131071 100000 12 ≤
    globalCoefficientBox K 12738110 131071 100000 20 := by
  intro Q hQ d hd
  obtain ⟨ht, hs, hc⟩ := hQ hd
  exact ⟨ht, hs.trans (by decide), hc.trans_le (by decide)⟩
...
def embedThin (u0 u1 : I → K) : ThinKernel u0 u1 →ₗ[K] Ambient :=
  polynomialCoefficientsLinear 12738110 131071 100000 20
    (kernelReconstructLinear (K := K) 8188785 131071 100000 12 45
      IRSProfile.domain u0 u1)

@[simp] theorem reconstruct_embedThin (u0 u1 : I → K) (v : ThinKernel u0 u1) :
    reconstruct K 12738110 131071 100000 20 (embedThin u0 u1 v) =
      reconstruct K 8188785 131071 100000 12 v.1 := by
  have hbox : kernelReconstructLinear (K := K) 8188785 131071 100000 12 45
      IRSProfile.domain u0 u1 v ∈ globalCoefficientBox K 12738110 131071 100000 20 := by
    rw [kernelReconstructLinear_apply]
    exact thinBox_le_aBox (reconstruct_mem_globalCoefficientBox
      K 8188785 131071 100000 12 v.1)
  have h := reconstruct_polynomialCoefficientsLinear 12738110 131071 100000 20
    (kernelReconstructLinear (K := K) 8188785 131071 100000 12 45
      IRSProfile.domain u0 u1) v hbox
...
theorem reconstruct_joinedMap (u0 u1 : I → K)
    (v : AKernel u0 u1 × (AuxKernel u0 u1 × (CKernel u0 u1 × ThinKernel u0 u1))) :
    reconstruct K 12738110 131071 100000 20 (joinedMap u0 u1 v) =
      reconstruct K 12738110 131071 100000 20 v.1.1 +
        (reconstruct K 11464299 131071 100000 18 v.2.1.1 +
          reconstruct K 8188785 131071 100000 13 v.2.2.1.1 +
          reconstruct K 8188785 131071 100000 12 v.2.2.2.1) := by
...
structure SelectedPair (u0 u1 : I → K) where
  ...
  QA_ne : QA ≠ 0
  QB_ne : QB ≠ 0
  QA_flag : QA ∈ globalCoefficientBox K 12738110 131071 100000 20
  QB_flag : QB ∈ globalCoefficientBox K 15285732 131071 1420 26
  common_divides_A : ∀ v : AKernel u0 u1,
    gcd QA QB ∣ reconstruct K 12738110 131071 100000 20 v.1
  common_divides_Aux : ∀ v : AuxKernel u0 u1,
    gcd QA QB ∣ reconstruct K 11464299 131071 100000 18 v.1
  common_divides_C : ∀ v : CKernel u0 u1,
    gcd QA QB ∣ reconstruct K 8188785 131071 100000 13 v.1
  common_divides_Thin : ∀ v : ThinKernel u0 u1,
    gcd QA QB ∣ reconstruct K 8188785 131071 100000 12 v.1
  common_divides_B : ∀ v : BKernel u0 u1,
    gcd QA QB ∣ reconstruct K 15285732 131071 1420 26 v.1
  universal_vanishing :
    ∀ (gamma : K) (P : Polynomial K) (points : Finset I),
      P.natDegree ≤ 131071 → 181973 ≤ points.card →
...
theorem exists_selected_pair (u0 u1 : I → K) : Nonempty (SelectedPair u0 u1) := by
  ...
  obtain ⟨thetaA, htA, hkA⟩ := exists_nonzero_kernel_array (I := I)
    K 12738110 131071 100000 20 70 IRSProfile.domain u0 u1 gateA
  obtain ⟨thetaB, htB, hkB⟩ := exists_nonzero_kernel_array (I := I)
    K 15285732 131071 1420 26 84 IRSProfile.domain u0 u1 gateB
  ...
  have hHBbox : HB ∈ globalCoefficientBox K 15285732 131071 1420 26 :=
    commonGCD_mem_flagBox (BKernel u0 u1) bB
  have hcardHB : (normalizedFactorSet HB).card < ENat.card K :=
    normalizedFactorSet_card_lt_field_of_mem_flagBox HB 15285732 1420 26
  ...
  have hQAbox : QA ∈ globalCoefficientBox K 12738110 131071 100000 20 := by
    dsimp only [QA]
    rw [submoduleReconstructLinear_apply]
    exact reconstruct_mem_globalCoefficientBox K 12738110 131071 100000 20 vA.1
  have hcardQA : (normalizedFactorSet QA).card < ENat.card K :=
    normalizedFactorSet_card_lt_field_of_mem_flagBox QA 12738110 100000 20
  ...
  have hQBbox : QB ∈ globalCoefficientBox K 15285732 131071 1420 26 := by
    dsimp only [QB]
    rw [submoduleReconstructLinear_apply]
    exact reconstruct_mem_globalCoefficientBox K 15285732 131071 1420 26 vB.1
  ...
  · intro v
    ...
    change gcd QA QB ∣ reconstruct K 12738110 131071 100000 20 (embedAux u0 u1 v) at h
    simpa only [reconstruct_embedAux] using h
  · intro v
    have h := hHHA.trans (commonGCD_dvd (JoinedKernel u0 u1) bA (includeC u0 u1 v))
    change gcd QA QB ∣ reconstruct K 12738110 131071 100000 20 (embedC u0 u1 v) at h
    simpa only [reconstruct_embedC] using h
  · intro v
    have h := hHHA.trans (commonGCD_dvd (JoinedKernel u0 u1) bA (includeThin u0 u1 v))
    change gcd QA QB ∣ reconstruct K 12738110 131071 100000 20 (embedThin u0 u1 v) at h
  ...
  · intro gamma P points hP hcard hvalues
    ...
    · dsimp only [QB]
      ...
      exact specialization_eq_zero_of_agreements K
        15285732 131071 1420 26 84 181973 IRSProfile.domain u0 u1
