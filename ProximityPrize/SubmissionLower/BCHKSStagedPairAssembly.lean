...
/-- Concrete normalized-factor selection with a coefficient depending on the
selected first-stage factor. -/
theorem exists_concrete_mixed_staged_factor_selection
    ...
    (badCap pairA : Polynomial (Polynomial (Polynomial F)) → Nat)
    (e : Nat)
    (hQ : Q ≠ 0)
    ...
    (hglobal :
      (∑ R ∈ (UniqueFactorizationMonoid.normalizedFactors Q).toFinset.filter
          (fun R => 0 < R.natDegree),
        (pairA R * R.natDegree + e * R.natDegree + badCap R)) < S.card) :
    ∃ R H T,
      ...
      pairA R * H.natDegree + e < T.card := by
  classical
  ...
  obtain ⟨R, hRs, H, hHs, T, hTS, hTbad, hrel, hmargin⟩ :=
    exists_staged_weighted_selection_by_R S Rs Hs Polynomial.natDegree
      Polynomial.natDegree badCap pairA e RelR RelH Bad hRcover
...
structure StagedGoodFamily
  ...
  bad_cap : ∀ R ∈ UniqueFactorizationMonoid.normalizedFactors Q,
    0 < R.natDegree → badCap R ≤ 2 * R.natDegree * 497543
...
theorem exists_staged_pair_with_setup
    ...
    (hcard : (632164 * 497543) * 842 ^ 2 +
      (bchksErrors + 1) * 842 + 2 * 497543 * 842 < S.card) :
    ∃ R H T,
      R ∈ UniqueFactorizationMonoid.normalizedFactors Q ∧ 0 < R.natDegree ∧
      ...
      (if R.natDegree = 1 then 2 * 110327280 * 497543
        else 632164 * 497543 * R.natDegree) * H.natDegree +
      ...
      Polynomial.Bivariate.totalDegree (triSpecializeX R (G.x₀ R)) ≤ 497543 ∧
      (∀ j a, ((R.coeff j).coeff a) ≠ 0 →
        a + 131071 * j < 110327280) ∧
      RationalFunctions.HenselNumerators.Hypotheses (G.x₀ R) R H := by
  let pairA : Polynomial (Polynomial (Polynomial F)) → Nat := fun R =>
    if R.natDegree = 1 then 2 * 110327280 * 497543
    else 632164 * 497543 * R.natDegree
  have hsum :
      (∑ R ∈ (UniqueFactorizationMonoid.normalizedFactors Q).toFinset.filter
          (fun R => 0 < R.natDegree),
        (pairA R * R.natDegree +
        ...
        (632164 * 497543) * 842 ^ 2 +
          (bchksErrors + 1) * 842 + 2 * 497543 * 842 := by
    simpa [pairA] using positive_normalizedFactors_mixed_staged_cap_le
      Q hQ G.badCap 110327280 632164 (bchksErrors + 1) 497543 842 hQY
      (by norm_num)
      (by
        intro R hR
        have hm := Finset.mem_filter.mp hR
  ...
  obtain ⟨R, H, T, hRQ, hRpos, hHR, hHpos, hTS, hTbad, hvan, hmargin⟩ :=
    exists_concrete_mixed_staged_factor_selection S P Q G.x₀ G.Bad G.badCap
      pairA (bchksErrors + 1) hQ hQeval hQz
