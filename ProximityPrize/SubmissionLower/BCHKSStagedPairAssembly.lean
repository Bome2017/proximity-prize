...
structure StagedGoodFamily
  ...
  bad_cap : ∀ R ∈ UniqueFactorizationMonoid.normalizedFactors Q,
    0 < R.natDegree → badCap R ≤ 2 * R.natDegree * 261561
...
theorem exists_staged_pair_with_setup
    ...
    ∃ R H T,
      ...
      Polynomial.Bivariate.totalDegree (triSpecializeX R (G.x₀ R)) ≤ 261561 ∧
  ...
  have hsum :
      (∑ R ∈ (UniqueFactorizationMonoid.normalizedFactors Q).toFinset.filter
          (fun R => 0 < R.natDegree),
        (631967 * 261561 * R.natDegree ^ 2 +
          (2 * 95502114 * 261561 + (bchksErrors + 1) + 261561) * R.natDegree + G.badCap R)) ≤
        631967 * 261561 * 729 ^ 2 +
          (2 * 95502114 * 261561 + (bchksErrors + 1) + 261561) * 729 +
          2 * 261561 * 729 := by
    apply positive_normalizedFactors_staged_cap_le Q hQ G.badCap
      (631967 * 261561) (2 * 95502114 * 261561 + (bchksErrors + 1) + 261561) 261561 729 hQY
    intro R hR
  ...
  have hglobal :
      (∑ R ∈ (UniqueFactorizationMonoid.normalizedFactors Q).toFinset.filter
          (fun R => 0 < R.natDegree),
        (631967 * 261561 * R.natDegree ^ 2 +
          (2 * 95502114 * 261561 + (bchksErrors + 1) + 261561) * R.natDegree + G.badCap R)) < S.card :=
    hsum.trans_lt hcard
  obtain ⟨R, H, T, hRQ, hRpos, hHR, hHpos, hTS, hTbad, hvan, hmargin⟩ :=
    exists_concrete_staged_factor_selection S P Q G.x₀ G.Bad G.badCap
      (631967 * 261561) (2 * 95502114 * 261561 + (bchksErrors + 1) + 261561) hQ hQeval hQz
