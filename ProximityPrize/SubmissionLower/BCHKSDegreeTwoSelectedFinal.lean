...
theorem exists_large_domain_fibers
    ...
    (hT : (262144 - 76599 - 131071) * T.card >
      (262144 - 131071) * ((2 * 131071 + 2) * dH * d * D))
    (hagree : ∀ z : T, ∀ i ∈ A z,
  ...
  have hG : 131072 ≤ G.card := by
    simpa [G] using many_large_fibers T A 262144 76599 131071
      ((2 * 131071 + 2) * dH * d * D)
...
theorem degree_ge_two_selected_adapter
    ...
    (hrow : ∀ z ∈ T, 262144 - 76599 ≤ (Arow z).card)
    (hT : 2 * 36923454 * H.natDegree * d * D + 76599 + 1 ≤ T.card)
    (hagree : ∀ z : T, ∀ i ∈ Arow z,
  ...
  classical
  have hT' : (262144 - 76599 - 131071) * T.card >
      (262144 - 131071) * ((2 * 131071 + 2) * H.natDegree * d * D) := by
    norm_num at hT ⊢
    nlinarith
...
theorem degree_two_selected_final
    ...
    (hTbad : ∀ z ∈ T, z ∉ Bad)
    (hmargin : bchksNonlinearCapacity * R.natDegree * H.natDegree +
      (bchksErrors + 1) < T.card)
    (hRi : Irreducible R) (hHi : Irreducible H) (hHpos : 0 < H.natDegree)
    (hHyp : Hypotheses x₀ R H)
    (hHtot : Bivariate.totalDegree H ≤ 55744)
    (hYZ : YZCap R 55744)
    (hsimple : ∀ z ∉ Bad, ∀ y,
  ...
  have hT : (262144 - 76599 - 131071) * T.card >
      (262144 - 131071) *
        ((2 * 131071 + 2) * H.natDegree * R.natDegree * 55744) := by
    norm_num [bchksErrors, bchksNonlinearCapacity] at hmargin ⊢
    nlinarith
  obtain ⟨A, hA, Fib, hFib, hinc⟩ :=
    exists_large_domain_fibers U T Arow (fun z : T => P z) H.natDegree R.natDegree
      55744 hrow hT (fun z i hi => hagree z z.property i hi)
  ...
  have hbadW : (T.filter fun z => H.leadingCoeff.eval z = 0).card ≤ 55744 - H.natDegree := by
    calc
      ...
      _ ≤ 55744 - H.natDegree := leadingCoeff_natDegree_le_of_totalDegree_le hHtot
  ...
  have hprod : 55744 ≤ bchksNonlinearCapacity * R.natDegree * H.natDegree := by
    norm_num [bchksNonlinearCapacity]
    nlinarith
  have hTgood_card : bchksErrors + 1 < Tgood.card := by
    have hlarge : 55744 + (bchksErrors + 1) < T.card :=
  ...
  have hFibgood : ∀ x : A,
      (((2 * 131071 + 1) * Bivariate.natDegreeY R * 55744) + 1) * H.natDegree <
        (Fibgood x).card := by
    intro x
    have hc := coarse_fiber_filter_leadingCoeff_card_exact H 55744 131072
  ...
  have hRdegYle : Bivariate.natDegreeY R ≤ R.natDegree := by
    exact le_rfl
  let S := selectedHenselData_of_pair x₀ R hHyp hzeta 55744 R.natDegree 131071
    36923454 Tgood PT hfactor hExact hsimp (hNP z₀).hsL
  ...
  obtain ⟨p₀, p₁, hp₀, hp₁, hp⟩ := hensel_baseZ_alignment_final_exact_yz
    x₀ R hHyp hzeta 55744 131071 36923454
