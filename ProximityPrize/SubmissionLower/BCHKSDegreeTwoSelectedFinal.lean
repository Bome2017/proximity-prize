...
theorem exists_large_domain_fibers
    ...
    (dH d D : ℕ)
    (hrow : ∀ z ∈ T, 262144 - 76570 ≤ (A z).card)
    (hT : 2 * 40826205 * dH * d * D + 76570 + 1 ≤ T.card)
    (hagree : ∀ z : T, ∀ i ∈ A z,
...
theorem degree_ge_two_selected_adapter
    ...
    (hrow : ∀ z ∈ T, 262144 - 76570 ≤ (Arow z).card)
    (hT : 2 * 40826205 * H.natDegree * d * D + 76570 + 1 ≤ T.card)
    (hagree : ∀ z : T, ∀ i ∈ Arow z,
...
theorem degree_two_selected_final
    ...
    (hTbad : ∀ z ∈ T, z ∉ Bad)
    (hmargin : 2 * 40826205 * 32414 * R.natDegree * H.natDegree +
  ...
  have hT : 2 * 40826205 * H.natDegree * R.natDegree * 32414 + 76570 + 1 ≤ T.card := by
  ...
  have hprod : 32414 ≤ 2 * 40826205 * 32414 * R.natDegree * H.natDegree := by
    nlinarith
  ...
  let S := selectedHenselData_of_pair x₀ R hHyp hzeta 32414 R.natDegree 131071
    40826205 Tgood PT hfactor hExact hsimp (hNP z₀).hsL
  ...
  have hcard : 2 * 40826205 * H.natDegree * R.natDegree * 32414 +
  ...
  obtain ⟨_, p₀, p₁, hp₀, hp₁, hp⟩ := hensel_baseZ_alignment_final_exact_yz
    x₀ R hHyp hzeta 32414 R.natDegree 131071 40826205 (bchksErrors - 32414)
