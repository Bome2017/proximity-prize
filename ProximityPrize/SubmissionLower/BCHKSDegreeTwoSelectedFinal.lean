...
theorem exists_large_domain_fibers
    ...
    (dH d D : ℕ)
    (hrow : ∀ z ∈ T, 262144 - 76730 ≤ (A z).card)
    (hT : 2 * 33398999 * dH * d * D + 76730 + 1 ≤ T.card)
    (hagree : ∀ z : T, ∀ i ∈ A z,
...
theorem degree_ge_two_selected_adapter
    ...
    (hrow : ∀ z ∈ T, 262144 - 76730 ≤ (Arow z).card)
    (hT : 2 * 33398999 * H.natDegree * d * D + 76730 + 1 ≤ T.card)
    (hagree : ∀ z : T, ∀ i ∈ Arow z,
...
theorem degree_two_selected_final
  ...
  have hT : 2 * 33398999 * H.natDegree * R.natDegree * 63302 + 76730 + 1 ≤ T.card := by
