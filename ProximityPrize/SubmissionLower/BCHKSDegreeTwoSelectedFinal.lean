...
theorem exists_large_domain_fibers
    ...
    (dH d D : ℕ)
    (hrow : ∀ z ∈ T, 262144 - 76703 ≤ (A z).card)
    (hT : 631967 * dH * d * D + 76703 + 1 ≤ T.card)
    (hagree : ∀ z : T, ∀ i ∈ A z,
...
theorem degree_ge_two_selected_adapter
    ...
    (hrow : ∀ z ∈ T, 262144 - 76703 ≤ (Arow z).card)
    (hT : 631967 * H.natDegree * d * D + 76703 + 1 ≤ T.card)
    (hagree : ∀ z : T, ∀ i ∈ Arow z,
...
theorem degree_two_selected_final
    ...
    (hTbad : ∀ z ∈ T, z ∉ Bad)
    (hmargin : 631967 * 261561 * R.natDegree * H.natDegree +
      (bchksErrors + 1) + 261561 < T.card)
    (hRi : Irreducible R) (hHi : Irreducible H) (hHpos : 0 < H.natDegree)
    (hHyp : Hypotheses x₀ R H)
    (hHtot : Bivariate.totalDegree H ≤ 261561)
    (hYZ : YZCap R 261561)
    (hsimple : ∀ z ∉ Bad, ∀ y,
  ...
  have hT : 631967 * H.natDegree * R.natDegree * 261561 + 76703 + 1 ≤ T.card := by
    dsimp [bchksErrors] at hmargin
    ring_nf at hmargin ⊢
    omega
  obtain ⟨A, hA, Fib, hFib, hinc⟩ :=
    exists_large_domain_fibers U T Arow (fun z : T => P z) H.natDegree R.natDegree
      261561 hrow hT (fun z i hi => hagree z z.property i hi)
  ...
  have hbadW : (T.filter fun z => H.leadingCoeff.eval z = 0).card ≤ 261561 - H.natDegree := by
    calc
      ...
      _ ≤ 261561 - H.natDegree := leadingCoeff_natDegree_le_of_totalDegree_le hHtot
  ...
  have hTgood_card : bchksErrors + 1 < Tgood.card := by
    have hlarge : 261561 + (bchksErrors + 1) < T.card := by
      have h0 : 0 ≤ 631967 * 261561 * R.natDegree * H.natDegree := Nat.zero_le _
      omega
    omega
  ...
  have hFibgood : ∀ x : A,
      (((2 * 131071 + 1) * Bivariate.natDegreeY R * 261561) + 1) * H.natDegree <
        (Fibgood x).card := by
    intro x
    have hc := coarse_fiber_filter_leadingCoeff_card_exact H 261561 131072
  ...
  have hRdegYle : Bivariate.natDegreeY R ≤ R.natDegree := by
    exact le_rfl
  let S := selectedHenselData_of_pair x₀ R hHyp hzeta 261561 R.natDegree 131071
  ...
  have hcard : 2 * 131072 * H.natDegree * R.natDegree * 261561 +
      (bchksErrors - 261561) + 1 < Tgood.card := by
  ...
  obtain ⟨_, p₀, p₁, hp₀, hp₁, hp⟩ := hensel_baseZ_alignment_final_exact_yz
    x₀ R hHyp hzeta 261561 R.natDegree 131071 131072 (bchksErrors - 261561)
