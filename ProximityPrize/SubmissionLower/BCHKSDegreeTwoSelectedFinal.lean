...
/- Legacy coarse-capacity assembly.  It is retained as source history but is
not elaborated: the submitted theorem graph uses the exact-capacity path below. -/
/-
...
/-- Exact-capacity incidence extraction for the `76720` error target.  This
keeps the coarse affine-root fibre threshold while replacing the old
`2 * DX` row multiplier by the sharp coefficient `632164`. -/
theorem exists_large_domain_fibers_exact_capacity
    (U : Fin 2 → IRSProfile.Index → IRSProfile.Field)
    (T : Finset IRSProfile.Field)
    (A : IRSProfile.Field → Finset IRSProfile.Index)
    (P : T → Polynomial IRSProfile.Field)
  ...
  let G : Finset IRSProfile.Index := Finset.univ.filter fun i =>
    (2 * 131071 + 2) * dH * d * D <
      (T.filter fun z => i ∈ A z).card
  have hG : 131072 ≤ G.card := by
    simpa [G] using exact_capacity_many_large_fibers T A dH d D
      (by norm_num [IRSProfile.Index]) hrow hT
  let A' : Finset IRSProfile.Field := G.image IRSProfile.domain
  have hA' : A'.card = G.card := by
    exact Finset.card_image_iff.mpr fun a _ b _ hab => IRSProfile.domain.injective hab
...
/- The following coarse-capacity and intermediate adapter declarations are no
longer in the live proof graph.  Avoid elaborating their duplicated Hensel
assembly in every clean verifier run. -/
/-
...
/-- The degree-at-least-two selected-data adapter with exact incidence
capacity.  Its conclusion is alignment only: no middle-coefficient statement
or global `2 * DX` cardinality hypothesis is needed. -/
theorem degree_ge_two_selected_adapter_exact_incidence
    (U : Fin 2 → IRSProfile.Index → IRSProfile.Field)
    {H : IRSProfile.Field[X][Y]} [Fact (Irreducible H)] [Fact (0 < H.natDegree)]
    (x₀ : IRSProfile.Field) (R : IRSProfile.Field[X][X][Y])
    (hHyp : RationalFunctions.HenselNumerators.Hypotheses x₀ R H)
    ...
    (hkF : k < Fintype.card IRSProfile.Field)
    (Arow : IRSProfile.Field → Finset IRSProfile.Index)
    (hrow : ∀ z ∈ T, 262144 - 76720 ≤ (Arow z).card)
    (hT : 632164 * H.natDegree * d * D + 76720 + 1 ≤ T.card)
    (hagree : ∀ z : T, ∀ i ∈ Arow z,
      Polynomial.eval (IRSProfile.domain i) (P z) =
        U 0 i + (z : IRSProfile.Field) * U 1 i)
    (hk : k = 131071) :
  ...
  classical
  obtain ⟨A, hA, Fib, hFib, hinc⟩ :=
    exists_large_domain_fibers_exact_capacity U T Arow P H.natDegree d D hrow hT hagree
  have hne : ∀ x : A, (Fib x).Nonempty := by
    intro x
    exact Finset.card_pos.mp (Nat.zero_lt_of_lt (hFib x))
  let zpick : ∀ x : A, T := fun x => Classical.choose (hne x)
  ...
  exact hensel_baseZ_alignment_final_exact_yz_alignmentOnly
    x₀ R hHyp hzeta D k hHD hRD hRdeg T S.root P hPdeg S.hx S.hy S.hsL
    S.hsimple S.hExact S.hslope S.hW S.hxi (fun t _ z => (hNP z).hden t) hkF
    A (by simpa [hk] using hA) U₀ U₁ Fib (by
      intro x
      calc
        (((2*k+1) * Bivariate.natDegreeY R * D) + 1) * H.natDegree ≤
            (2*k+2) * d * D * H.natDegree := by
              have hDpos : 1 ≤ D := by
                have hHpos : 0 < H.natDegree := Fact.out
                have hHne : H ≠ 0 := Polynomial.ne_zero_of_natDegree_gt
                  hHpos
                have hHin : H.natDegree ∈ H.support :=
                  Polynomial.mem_support_iff.mpr
                    (Polynomial.leadingCoeff_ne_zero.mpr hHne)
                have htotal := Bivariate.coeff_totalDegree_le H hHin
                have hnat : H.natDegree ≤ D := by
                  exact Nat.le_trans (by omega) (htotal.trans hHD)
                omega
              have hdpos : 1 ≤ d := by omega
              have hdegree : (2*k+1) * Bivariate.natDegreeY R * D ≤
                  (2*k+1) * d * D := by gcongr
              have hdDpos : 0 < d * D := Nat.mul_pos (by omega) (by omega)
              have hone : 1 ≤ d * D := by omega
              calc
                _ ≤ (((2*k+1) * d * D) + d * D) * H.natDegree := by
                  exact Nat.mul_le_mul_right H.natDegree
                    (Nat.add_le_add hdegree hone)
                _ = (2*k+2) * d * D * H.natDegree := by ring
        _ < (Fib x).card := by
          simpa [hk, Nat.mul_assoc, Nat.mul_left_comm, Nat.mul_comm] using hFib x) halign
...
/-- Degree-at-least-two selected-factor alignment at the exact `76720`
incidence capacity.  The staging margin uses coefficient `632164`, and the
final Hensel step is alignment-only. -/
theorem degree_two_selected_final_exact_incidence
    (U : Fin 2 → IRSProfile.Index → IRSProfile.Field)
    (P : IRSProfile.Field → IRSProfile.Field[X])
    (Arow : IRSProfile.Field → Finset IRSProfile.Index)
    (R : IRSProfile.Field[X][X][Y]) (H : IRSProfile.Field[X][Y])
    ...
    (hvan : ∀ z ∈ T, triEval R z (P z) = 0 ∧ biEval H ((P z).eval x₀) z = 0)
    (hTbad : ∀ z ∈ T, z ∉ Bad)
    (hmargin : 632164 * 497543 * R.natDegree * H.natDegree +
      (bchksErrors + 1) < T.card)
    (hRi : Irreducible R) (hHi : Irreducible H) (hHpos : 0 < H.natDegree)
    (hHyp : Hypotheses x₀ R H)
    (hHtot : Bivariate.totalDegree H ≤ 497543)
    (hYZ : YZCap R 497543)
    (hsimple : ∀ z ∉ Bad, ∀ y,
      Polynomial.eval y (biSpecializeZ (triSpecializeX R x₀) z) = 0 →
      Polynomial.eval y (biSpecializeZ (triSpecializeX R.derivative x₀) z) ≠ 0)
  ...
  letI : Fact (Irreducible H) := ⟨hHi⟩
  letI : Fact (0 < H.natDegree) := ⟨hHpos⟩
  have hT : 632164 * H.natDegree * R.natDegree * 497543 + 76720 + 1 ≤ T.card := by
    dsimp [bchksErrors] at hmargin
    ring_nf at hmargin ⊢
    omega
  obtain ⟨A, hA, Fib, hFib, hinc⟩ :=
    exists_large_domain_fibers_exact_capacity U T Arow (fun z : T => P z)
      H.natDegree R.natDegree 497543 hrow hT
      (fun z i hi => hagree z z.property i hi)
  let Tgood := T.filter fun z => H.leadingCoeff.eval z ≠ 0
  have hWne : H.leadingCoeff ≠ 0 := Polynomial.leadingCoeff_ne_zero.mpr hHi.ne_zero
  have hbadW : (T.filter fun z => H.leadingCoeff.eval z = 0).card ≤
      497543 - H.natDegree := by
    calc
      _ ≤ H.leadingCoeff.roots.toFinset.card := by
        apply Finset.card_le_card
        intro z hz
      ...
      _ ≤ H.leadingCoeff.roots.card := Multiset.toFinset_card_le _
      _ ≤ H.leadingCoeff.natDegree := Polynomial.card_roots' _
      _ ≤ 497543 - H.natDegree := leadingCoeff_natDegree_le_of_totalDegree_le hHtot
  have hpart : Tgood.card + (T.filter fun z => H.leadingCoeff.eval z = 0).card =
      T.card := by
    simpa [Tgood] using Finset.filter_card_add_filter_neg_card_eq_card
      (s := T) (p := fun z => H.leadingCoeff.eval z ≠ 0)
  have hprod : 497543 ≤
      632164 * 497543 * R.natDegree * H.natDegree := by
    nlinarith
  have hTgood_card : bchksErrors + 1 < Tgood.card := by
    have hlarge : 497543 + (bchksErrors + 1) < T.card :=
      lt_of_le_of_lt (Nat.add_le_add_right hprod _) hmargin
    omega
  let Fibgood : A → Finset Tgood := fun x =>
    let E : {z // z ∈ (Fib x).filter fun z : T =>
  ...
  have hFibgood : ∀ x : A,
      (((2 * 131071 + 1) * Bivariate.natDegreeY R * 497543) + 1) * H.natDegree <
        (Fibgood x).card := by
    intro x
    have hc := coarse_fiber_filter_leadingCoeff_card_exact H 497543 131072
      R.natDegree H.natDegree rfl (by norm_num) (by omega) (by omega) hHtot
      ((Fib x).image fun z : T => (z : IRSProfile.Field)) (by
        rw [Finset.card_image_iff.mpr (fun a _ b _ hab => Subtype.ext hab)]
        simpa [Nat.mul_assoc, Nat.mul_left_comm, Nat.mul_comm] using hFib x)
    ...
    dsimp [Fibgood]
    rw [Finset.card_map]
    change _ < (((Fib x).filter fun z : T =>
      H.leadingCoeff.eval (z : IRSProfile.Field) ≠ 0).attach).card
    rw [Finset.card_attach]
    simpa [Bivariate.natDegreeY, Nat.mul_assoc, Nat.mul_left_comm, Nat.mul_comm]
      using hc'
  have hFibgood_mem (x : A) (z : Tgood) (hz : z ∈ Fibgood x) :
      (⟨(z : IRSProfile.Field), (Finset.mem_filter.mp z.property).1⟩ : T) ∈ Fib x := by
    dsimp [Fibgood] at hz
  ...
  let root₀ := rationalRootOfPair H hHpos (z₀ : IRSProfile.Field) ((P z₀).eval x₀) (by
    have hv := (hvan z₀ hz₀T).2
    exact (eval_map_eval_eq_eval_eval_C H ((P z₀).eval x₀)
      (z₀ : IRSProfile.Field)).symm.trans hv)
  have hx₀ := GoodAt.fieldTo𝕃 (H:=H) (z₀ : IRSProfile.Field) root₀ x₀
  ...
  have hs₀ : FiniteHensel.ySlope (triSpecializeZ R (z₀ : IRSProfile.Field)) x₀
      ((P z₀).eval x₀) ≠ 0 := by
    rw [hslope_eval]
    exact hsimple z₀ (hTbad z₀ (Finset.mem_filter.mp z₀.property).1)
      ((P z₀).eval x₀) (by
        exact (eval_triEval_eq_biEval_triSpecializeX R (P z₀) x₀
          (z₀ : IRSProfile.Field)).symm.trans
            (by simpa using (congrArg (fun q => q.eval x₀) (hvan z₀ hz₀T).1)))
  have hzeta := zeta_ne_zero_of_selected_slope x₀ R (z₀ : IRSProfile.Field)
    root₀ x₀ ((P z₀).eval x₀) hx₀ hy₀ hs₀
  let PT : Tgood → IRSProfile.Field[X] := fun z => P z
  have hfactor : ∀ z : Tgood, Polynomial.evalEval (z : IRSProfile.Field)
      ((PT z).eval x₀) H = 0 := by
    intro z
  ...
  have hNP : ∀ z : Tgood, SelectedNonpoleData x₀ R H hHyp (z : IRSProfile.Field)
    ...
    exact selectedNonpoleData x₀ R hHyp hzeta (z : IRSProfile.Field) (root z) x₀
      ((PT z).eval x₀) (hx z) (hy z) (Finset.mem_filter.mp z.property).2 (hsimp z)
  have hRdegY : 2 ≤ Bivariate.natDegreeY R := hRdeg
  have hRdegYle : Bivariate.natDegreeY R ≤ R.natDegree := le_rfl
  let S := selectedHenselData_of_pair x₀ R hHyp hzeta 497543 R.natDegree 131071
    131071 Tgood PT hfactor hExact hsimp (hNP z₀).hsL
    (fun z => (hNP z).hslope) (fun z => (Finset.mem_filter.mp z.property).2)
    (fun z => by simpa [root] using (hNP z).hxi)
    (fun _ _ _ z => by simpa [root] using (hNP z).hden _)
    hHtot hYZ hRdegY hRdegYle
  ...
  have halign : ∀ x : A, ∀ z ∈ Fibgood x,
    ...
    subst i
    simpa [PT, U₀, U₁, x.property] using he
  obtain ⟨_, p₀, p₁, hp₀, hp₁, hp⟩ :=
    hensel_baseZ_alignment_final_exact_yz
      x₀ R hHyp hzeta 497543 0 131071 131072 0 (by norm_num)
      hHtot hYZ hRdegY Tgood S.root PT hPdegT
      S.hx S.hy S.hsL S.hsimple S.hExact S.hslope S.hW S.hxi
      (fun t _ z => (hNP z).hden t)
      (fun t hkt htk => by omega)
      (by
        have : 1 < Tgood.card := by omega
        simpa using this)
      (by norm_num [IRSProfile.Field])
      A (by simpa using hA) U₀ U₁ Fibgood hFibgood halign
  exact ⟨Tgood, Finset.filter_subset _ _, hTgood_card, p₀, p₁, hp₀, hp₁,
    fun z hz => hp ⟨z, hz⟩⟩
