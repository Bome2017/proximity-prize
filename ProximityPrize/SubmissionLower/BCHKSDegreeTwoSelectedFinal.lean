...
theorem exists_large_domain_fibers
    ...
    (dH d D : ℕ)
    (hrow : ∀ z ∈ T, 262144 - 76388 ≤ (A z).card)
    (hT : 2 * 40866320 * dH * d * D + 76388 + 1 ≤ T.card)
    (hagree : ∀ z : T, ∀ i ∈ A z,
...
theorem degree_ge_two_selected_adapter
    ...
    (hrow : ∀ z ∈ T, 262144 - 76388 ≤ (Arow z).card)
    (hT : 2 * 40866320 * H.natDegree * d * D + 76388 + 1 ≤ T.card)
    (hagree : ∀ z : T, ∀ i ∈ Arow z,
...
theorem degree_two_selected_final
    ...
    (hTbad : ∀ z ∈ T, z ∉ Bad)
    (hmargin : 2 * 40866320 * 32414 * R.natDegree * H.natDegree +
      (bchksErrors + 1) < T.card)
    (hRi : Irreducible R) (hHi : Irreducible H) (hHpos : 0 < H.natDegree)
    (hHyp : Hypotheses x₀ R H)
    (hHtot : Bivariate.totalDegree H ≤ 32414)
    (hYZ : YZCap R 32414)
    (hsimple : ∀ z ∉ Bad, ∀ y,
  ...
  have hT : 2 * 40866320 * H.natDegree * R.natDegree * 32414 + 76388 + 1 ≤ T.card := by
    dsimp [bchksErrors] at hmargin
    ring_nf at hmargin ⊢
    omega
  obtain ⟨A, hA, Fib, hFib, hinc⟩ :=
    exists_large_domain_fibers U T Arow (fun z : T => P z) H.natDegree R.natDegree
      32414 hrow hT (fun z i hi => hagree z z.property i hi)
  ...
  have hbadW : (T.filter fun z => H.leadingCoeff.eval z = 0).card ≤ 32414 - H.natDegree := by
    calc
      ...
      _ ≤ 32414 - H.natDegree := leadingCoeff_natDegree_le_of_totalDegree_le hHtot
  ...
  have hprod : 32414 ≤ 2 * 40866320 * 32414 * R.natDegree * H.natDegree := by
    nlinarith
  have hTgood_card : bchksErrors + 1 < Tgood.card := by
    have hlarge : 32414 + (bchksErrors + 1) < T.card :=
  ...
  have hFibgood : ∀ x : A,
      (((2 * 131071 + 1) * Bivariate.natDegreeY R * 32414) + 1) * H.natDegree <
        (Fibgood x).card := by
    intro x
    have hc := coarse_fiber_filter_leadingCoeff_card_exact H 32414 131072
  ...
  have hRdegYle : Bivariate.natDegreeY R ≤ R.natDegree := by
    exact le_rfl
  let S := selectedHenselData_of_pair x₀ R hHyp hzeta 32414 R.natDegree 131071
    40866320 Tgood PT hfactor hExact hsimp (hNP z₀).hsL
  ...
  have hcard : 2 * 40866320 * H.natDegree * R.natDegree * 32414 +
      (bchksErrors - 32414) + 1 < Tgood.card := by
  ...
  obtain ⟨_, p₀, p₁, hp₀, hp₁, hp⟩ := hensel_baseZ_alignment_final_exact_yz
    x₀ R hHyp hzeta 32414 R.natDegree 131071 40866320 (bchksErrors - 32414)
