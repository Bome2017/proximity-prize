...
/-- The factor-dependent form of `exists_staged_weighted_selection`.  This is
useful when the linear and nonlinear first-stage factors have different
continuation costs. -/
theorem exists_staged_weighted_selection_var
    {σ ρ η : Type*} [DecidableEq σ] [DecidableEq ρ] [DecidableEq η]
    (S : Finset σ) (Rs : Finset ρ) (Hs : ρ → Finset η)
    (degR : ρ → Nat) (degH : η → Nat) (bad A : ρ → Nat)
    (e : Nat)
    (RelR : σ → ρ → Prop) [DecidableRel RelR]
    (RelH : σ → ρ → η → Prop) [∀ r, DecidableRel (fun z h => RelH z r h)]
    (Bad : ρ → Finset σ)
...
theorem exists_concrete_staged_factor_selection
  ...
  obtain ⟨R, hRs, H, hHs, T, hTS, hTbad, hrel, hmargin⟩ :=
    exists_staged_weighted_selection_var S Rs Hs Polynomial.natDegree Polynomial.natDegree
