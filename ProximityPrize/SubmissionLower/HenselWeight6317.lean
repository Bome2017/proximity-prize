...
-/
import ProximityPrize.SubmissionLower.HenselLift6317
...
`Z`-degree of `p`; powers of the recentered middle variable do not cost regular weight. -/
lemma regularWeightLe_coeff_liftCoeffToPowerSeries {D : ℕ} (hD : Bivariate.totalDegree H ≤ D)
  ...
  rw [Nat.add_zero]
  by_cases h0 : p.coeff s = 0
  · rw [h0]; simp
  · exact Finset.le_sup (f := fun i => (p.coeff i).natDegree)
      (Polynomial.mem_support_iff.mpr h0)
...
lemma henselClearedTerm_weight (x₀ : F) (R : F[X][X][Y]) (H : F[X][Y])
  ...
  · -- Case B: all parts ≤ t.
    ...
    -- coefficient weight ≤ degreeZ (R.coeff j) ≤ D - j
    have hcoeffW : RegularWeightLe hH
        (PowerSeries.coeff p.1 (liftCoeffToPowerSeries x₀ H (R.coeff j))) D
        (Bivariate.degreeX (R.coeff j)) := regularWeightLe_coeff_liftCoeffToPowerSeries hD_H hH
            x₀ (R.coeff j) p.1
    have htd_le : Bivariate.degreeX (R.coeff j) ≤ D - j := by
