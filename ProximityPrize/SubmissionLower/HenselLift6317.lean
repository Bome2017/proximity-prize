...
-/
import ProximityPrize.SubmissionLower.HenselSetup6317
...
/-- Specializing `X = x₀` cannot raise the `(Y,Z)` degree.  For
`R.coeff i : F[Z][X]`, `degreeX` is the degree in `Z`, uniformly over the powers of the
middle variable `X`. -/
theorem evalX_totalDegree_le_of_coeff_bound (x₀ : F) (R : F[X][X][Y]) {D : ℕ}
  ...
  have heval_deg : ((Bivariate.evalX (Polynomial.C x₀) R).coeff i).natDegree ≤
      Bivariate.degreeX (R.coeff i) := by
    rw [hcoeff_eq]
    ...
    simpa [Bivariate.degreeX_as_weighted_deg] using hle
  have hD := hD_R i hiR
  omega
...
theorem beta_zero_eq_X_of_shape (x₀ : F) (R : F[X][X][Y]) (H : F[X][Y])
    ...
    (_hD_R : ∀ i ∈ R.support, Bivariate.degreeX (R.coeff i) + i ≤ D)
