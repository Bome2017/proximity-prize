...
theorem bchksPolynomialAlignment_of_linear
    (hlinear : ∀
      ...
      (∀ z ∈ T, 185441 ≤ (Arow z).card) →
      (∀ z ∈ T, ∀ i ∈ Arow z,
        Polynomial.eval (IRSProfile.domain i) (P z) = U (0 : Fin 2) i + z * U (1 : Fin 2) i) →
      Irreducible R → YZCap R 261561 →
      (∀ j a, ((R.coeff j).coeff a) ≠ 0 →
        a + 131071 * j < 95502114) →
      R.natDegree = 1 →
      (bchksErrors + 1) + 2 * 95502114 * 261561 < T.card →
  ...
  have hrow : ∀ z ∈ T, 262144 - 76703 ≤ (A z).card := by
    intro z hz
    have hzS : z ∈ S := hSgoodS (hTS hz)
    norm_num
    exact hAcard ⟨z, hzS⟩
  ...
  have hYZ : RationalFunctions.HenselNumerators.ConcreteFiniteNumerators.YZCap R 261561 := by
    intro j a ha
    have hh := YZFactorCap.normalizedFactor_YZ_cap Q R 261561 hQ hRQ
  ...
  · obtain ⟨Tgood, hgoodT, hgoodcard, p₀, p₁, hp₀, hp₁, halign⟩ :=
      hlinear U PE A R T hPT (fun z hz => (hvan z hz).1)
        ...
        hagreeT hRi hYZ hRweighted hdeg (by
          refine Nat.lt_of_le_of_lt ?_ hmargin
          rw [Nat.add_comm (bchksErrors + 1)]
          exact Nat.le_trans (Nat.le_add_right _ _) (Nat.le_add_left _ _))
  ...
  · have hdeg2 : 2 ≤ R.natDegree := by omega
    obtain ⟨Tgood, hgoodT, hgoodcard, p₀, p₁, hp₀, hp₁, halign⟩ :=
      degree_two_selected_final U PE A R H T x₀ Bad hPT hvan hTbad (by
        refine Nat.lt_of_le_of_lt ?_ hmargin
        rw [Nat.add_assoc (631967 * 261561 * R.natDegree * H.natDegree)]
        exact Nat.add_le_add_left
          (Nat.add_le_add_right (Nat.le_add_left _ _) _) _)
