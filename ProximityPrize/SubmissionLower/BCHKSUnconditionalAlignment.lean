...
theorem bchksPolynomialAlignment_of_linear
    (hlinear : ∀
      ...
      (∀ j a, ((R.coeff j).coeff a) ≠ 0 →
        a + 131071 * j < 40870720) →
      R.natDegree = 1 →
      (bchksErrors + 1) + 2 * 40870720 * 32414 < T.card →
  ...
  have hrow : ∀ z ∈ T, 262144 - 76368 ≤ (A z).card := by
    intro z hz
    have hzS : z ∈ S := hSgoodS (hTS hz)
    norm_num
    exact hAcard ⟨z, hzS⟩
  ...
  have hYZ : RationalFunctions.HenselNumerators.ConcreteFiniteNumerators.YZCap R 32414 := by
    intro j a ha
    have hh := YZFactorCap.normalizedFactor_YZ_cap Q R 32414 hQ hRQ
  ...
  · obtain ⟨Tgood, hgoodT, hgoodcard, p₀, p₁, hp₀, hp₁, halign⟩ :=
      hlinear U PE A R T hPT (fun z hz => (hvan z hz).1)
        ...
        hagreeT hRi hYZ hRweighted hdeg (by
          ...
          have hm : 2 * 40870720 * 32414 ≤
              2 * 40870720 * 32414 * R.natDegree * H.natDegree := by
            rw [hdeg]
            nlinarith
          have hmargin' : (bchksErrors + 1) +
              2 * 40870720 * 32414 * R.natDegree * H.natDegree < T.card := by
