...
/- The whole gcd keeps its ordinary C45/B84 caps. The A63 and A70 collars
   are applied separately to its irreducible R=10/11/12 factors. -/
...
theorem gcd_fixed_count_le
    ...
    (LocatorCover.fixed
      ...
      273021899526170515 + LocatorArithmetic.fixedSingular.countCap := by
  classical
  ...
  have hHflagC : H ∈ RCN100.globalCoefficientBox K 8188785 131071 100000 13 :=
    S.common_C_flag
  have hHcaps := (mem_flagGlobalCoefficientBox_iff H
    8188785 131071 100000 13 (by decide)).mp hHflagC
  have hT : wt residualTotalWeights H ≤ 1417 := S.common_total_le
  have hY : wt residualYSWeights H ≤ 59 := S.common_ys_le
  have hS : wt residualSWeights H ≤ 12 := S.common_slope_le
  have hflag : H ∈ RCN100.globalCoefficientBox K 8188785 131071 1417 12 :=
    (mem_flagGlobalCoefficientBox_iff H 8188785 131071 1417 12 (by decide)).mpr
      ⟨hT, hS, hHcaps.2.2⟩
  have hbox := flag_box_to_ordinary K 8188785 131071 1417 12 H hflag
  ...
  have hagreementD : ∀ gamma ∈ Delta, 181973 ≤
      ((Finset.univ : Finset I).filter (fun i =>
        (selected gamma).eval (IRSProfile.domain i) = u0 i + gamma * u1 i)).card :=
    fun gamma hg => hagreement gamma (hsub hg)
  have hnoD : NoLargeSelectedPencil selected Delta 131071 80171 :=
    noLargeSelectedPencil_mono selected Gamma Delta 131071 80171 hsub hno
  ...
  exact LocatorFixed.fixed_count_le H hH hbox hsupport
    selected Delta u0 u1 hsolution hdegreeD hagreementD hnoD h10 h11 h12
