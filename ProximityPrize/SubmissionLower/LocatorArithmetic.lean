import ProximityPrize.SubmissionLower.L1
import ProximityPrize.SubmissionLower.N5
import ProximityPrize.SubmissionLower.E5
import ProximityPrize.SubmissionLower.Z8
import ProximityPrize.SubmissionLower.D1

namespace ProximityPrize.SubmissionLower.LocatorArithmetic
open ProximityPrize.Benchmark
open scoped BigOperators NNReal
open RCN100 RCN119 RCN302
open RCN318 RCN260
noncomputable section
set_option autoImplicit false
set_option maxRecDepth 100000
set_option maxHeartbeats 5000000

def n:ℕ:=262144
def w:ℕ:=131071
def errors:ℕ:=80181
def agreements:ℕ:=181963
def gap:ℕ:=50892
def prime:ℕ:=2130706433
def budget:ℕ:=274980727078395087
def LA:ℕ:=100000
def LB:ℕ:=1451
def weightedA:ℕ:=14739003
def weightedAux:ℕ:=13465262
def weightedC:ℕ:=8188335
def weightedB:ℕ:=15830781
def fixedRegularCap:ℕ:=272527456851746043

theorem row_values:agreements+errors=n ∧ agreements - w=gap ∧
    weightedA=81*agreements ∧ weightedAux=74*agreements ∧
    weightedC=45*agreements ∧ weightedB=87*agreements ∧ w<agreements ∧
    w+1≤weightedC ∧ weightedA<prime:=by decide

theorem kernelA_rank:localRankBound 81 100000 24=6360297725:=by decide
theorem kernelAux_rank:localRankBound 74 95923 21=4532520894:=by decide
theorem kernelC_rank:localRankBound 45 100000 13=1121192233:=by decide
theorem kernelThin_rank:localRankBound 81 18286 24=1161244475:=by decide
theorem kernelB_rank:localRankBound 87 1451 27=114793336:=by decide

theorem kernelA_nullity:
    coefficientCount 14739003 131071 100000 24 -
      262144*localRankBound 81 100000 24=10160890152125:=by
  rw [kernelA_rank,coefficientCount_eq_sum_range_of_weighted_cutoff
    14739003 131071 100000 24 113 (by decide) (by decide)]
  decide
theorem kernelAux_nullity:
    coefficientCount 13465262 131071 95923 21 -
      262144*localRankBound 74 95923 21=6185093728418:=by
  rw [kernelAux_rank,coefficientCount_eq_sum_range_of_weighted_cutoff
    13465262 131071 95923 21 103 (by decide) (by decide)]
  decide
theorem kernelC_nullity:
    coefficientCount 8188335 131071 100000 13 -
      262144*localRankBound 45 100000 13=111677764051:=by
  rw [kernelC_rank,coefficientCount_eq_sum_range_of_weighted_cutoff
    8188335 131071 100000 13 63 (by decide) (by decide)]
  decide
theorem kernelThin_nullity:
    coefficientCount 14739003 131071 18286 24 -
      262144*localRankBound 81 18286 24=1733670175175:=by
  rw [kernelThin_rank,coefficientCount_eq_sum_range_of_weighted_cutoff
    14739003 131071 18286 24 113 (by decide) (by decide)]
  decide
theorem kernelB_nullity:
    coefficientCount 15830781 131071 1451 27 -
      262144*localRankBound 87 1451 27=1028848471:=by
  rw [kernelB_rank,coefficientCount_eq_sum_range_of_weighted_cutoff
    15830781 131071 1451 27 121 (by decide) (by decide)]
  decide

theorem C_ys60_quotient_upper:
    coefficientCount 324088 131071 99940 13=89542780407:=by
  rw [coefficientCount_eq_sum_range_of_weighted_cutoff
    324088 131071 99940 13 3 (by decide) (by decide)]
  decide

theorem kernelC_ys60_quotient_lt (r:ℕ) (hr:r≤13):
    coefficientCount (8188335 - (60*131071 - r)) 131071
      (100000 - 60) (13 - r)<
    coefficientCount 8188335 131071 100000 13 -
      262144*localRankBound 45 100000 13:=by
  rw [kernelC_nullity]
  have hmono:=RCN180.Numeric6733.coefficientCount_mono_D_s
    (D:=8188335 - (60*131071 - r)) (D':=324088)
    (w:=131071) (L:=99940) (s:=13 - r) (s':=13)
    (by omega) (by omega)
  rw [C_ys60_quotient_upper] at hmono
  exact hmono.trans_lt (by decide)

theorem kernelB_total_quotient_lt:
    coefficientCount 15830781 131071 5 27<
    coefficientCount 15830781 131071 1451 27 -
      262144*localRankBound 87 1451 27:=by
  rw [kernelB_nullity]
  decide

def fixedSingular:TightParameters:=⟨n,w,agreements,weightedC,1445,13⟩
def residualStage:UnequalParameters:=⟨n,w,agreements,120,27,LB,112,24,LA⟩
def residualSingular:TightParameters:=⟨n,w,agreements,weightedB,LB,27⟩

theorem fixedSingular_exact:fixedSingular.countCap=114220154601797:=by decide
theorem residualStage_exact:residualStage.regularCountCap=837784402002013:=by decide
theorem residualSingular_exact:residualSingular.countCap=997052597918475:=by decide

structure SingularGates (P:TightParameters):Prop where
  s_pos:1≤P.s
  s_small:P.s<prime
  w_pos:1≤P.w
  w_small:P.w<prime
  kD:P.w<P.kappa*P.D
  algebraic_pos:1≤P.algebraicCap
  implicit_small:P.implicitYCap<prime
  algebraic_small:P.algebraicCap<prime
  mixed_small:2*P.implicitYCap*P.algebraicCap<prime
  wa:P.w<P.a
  an:P.a≤P.n

theorem fixed_singular_gates:SingularGates fixedSingular:=by constructor<;>decide
theorem residual_singular_gates:SingularGates residualSingular:=by constructor<;>decide

structure ResidualGates:Prop where
  qY:(residualSingular.D - 1) / w≤residualStage.leftY
  qR:residualSingular.s≤residualStage.leftR
  qZ:residualSingular.L≤residualStage.leftZ
  rightY:(weightedA - 1) / w≤residualStage.rightY
  leftR_pos:1≤residualStage.leftR
  leftY_small:residualStage.leftY<prime
  leftR_small:residualStage.leftR<prime
  leftZ_small:residualStage.leftZ<prime
  mixedY_small:residualStage.mixedCost.y<prime
  mixedR_small:residualStage.mixedCost.r<prime
  mixedZ_small:residualStage.mixedCost.z<prime

theorem residual_gates:ResidualGates:=by constructor<;>decide

def ledger:ℕ:=fixedRegularCap+fixedSingular.countCap+
  residualStage.regularCountCap+residualSingular.countCap+1
theorem ledger_exact:ledger=274476514006268329:=by
  rw [ledger,fixedRegularCap,fixedSingular_exact,
    residualStage_exact,residualSingular_exact]
theorem ledger_lt:ledger<budget:=by rw [ledger_exact]; decide

def radiusNumerator:ℕ:=10263295
def radiusDenominator:ℕ:=33554432
def radius:ℝ≥0:=claimedRadius radiusNumerator radiusDenominator

theorem radius_floor:
    ⌊(radius:ℝ)*(Fintype.card IRSProfile.Index:ℝ)⌋₊=errors:=by
  norm_num [radius,claimedRadius,radiusNumerator,radiusDenominator,
    errors,IRSProfile.Index]
theorem radius_admissible:
    radius∈Set.Ioo (0:ℝ≥0) IRSProfile.minRelativeDistance:=by
  constructor<;>norm_num [radius,claimedRadius,radiusNumerator,radiusDenominator,
    IRSProfile.minRelativeDistance]
theorem score_root_integer:(2:ℕ)^42*1000000^100≤1337928^100:=by decide
theorem score_radius_integer:
    (23291137:ℕ)^128*(2^67*1337928)≤1000000*33554432^128:=by decide

theorem two_rpow_fraction_le:
    (2:ℝ≥0)^((42:ℝ)/100)≤(1337928:ℝ≥0)/1000000:=by
  have hroot:((2:ℝ≥0)^(42:ℕ))^((100:ℝ)⁻¹)≤(1337928:ℝ≥0)/1000000:=by
    rw [NNReal.rpow_inv_le_iff (by norm_num:(0:ℝ)<100)]
    rw [NNReal.rpow_ofNat,div_pow,le_div_iff₀ (by positivity)]
    exact_mod_cast score_root_integer
  calc
    (2:ℝ≥0)^((42:ℝ)/100)=((2:ℝ≥0)^(42:ℕ))^((100:ℝ)⁻¹):=by
      rw [← NNReal.rpow_natCast_mul]
      norm_num [div_eq_mul_inv]
    _≤_:=hroot

theorem radius_power_bound:
    (1 - radius)^IRSProfile.repetitions≤
      ((1:ℝ≥0)/2^(67:ℕ))*(1000000/1337928):=by
  have hsub:(1 - radius:ℝ≥0)=23291137/33554432:=by
    have hr:radius≤1:=by
      rw [← NNReal.coe_le_coe]
      norm_num [radius,claimedRadius,radiusNumerator,radiusDenominator]
    apply NNReal.coe_injective
    rw [NNReal.coe_sub hr]
    norm_num [radius,claimedRadius,radiusNumerator,radiusDenominator]
  change (1 - radius)^128≤((1:ℝ≥0)/2^(67:ℕ))*(1000000/1337928)
  rw [hsub,div_pow,div_mul_div_comm,one_mul,
    div_le_div_iff₀ (by positivity) (by positivity)]
  exact_mod_cast score_radius_integer

theorem score_target_le:
    (1 - radius)^IRSProfile.repetitions≤claimedError 6742:=by
  have hscale:(1000000:ℝ≥0)/1337928≤(2:ℝ≥0)^(-((42:ℝ)/100)):=by
    calc
      (1000000:ℝ≥0)/1337928=1/((1337928:ℝ≥0)/1000000):=by norm_num
      _≤1/((2:ℝ≥0)^((42:ℝ)/100)):=
        one_div_le_one_div_of_le (by positivity) two_rpow_fraction_le
      _=_:=by rw [one_div,NNReal.rpow_neg]
  calc
    (1 - radius)^IRSProfile.repetitions≤
        ((1:ℝ≥0)/2^(67:ℕ))*(1000000/1337928):=radius_power_bound
    _≤((1:ℝ≥0)/2^(67:ℕ))*(2:ℝ≥0)^(-((42:ℝ)/100)):=
      mul_le_mul_of_nonneg_left hscale (by positivity)
    _=claimedError 6742:=by
      unfold claimedError
      rw [show -((((6742:ℕ):ℝ)/100))=
          -((67:ℕ):ℝ)+-((42:ℝ)/100) by norm_num,
        NNReal.rpow_add (by norm_num:(2:ℝ≥0)≠0)]
      simp only [NNReal.rpow_neg,NNReal.rpow_natCast,one_div]

end
end ProximityPrize.SubmissionLower.LocatorArithmetic
