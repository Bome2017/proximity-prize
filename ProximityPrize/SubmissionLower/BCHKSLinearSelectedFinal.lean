...
theorem bchksLinearSelectedFinal
    ...
    (hsize : 2 * 40870720 * 1 * 1 * 32415 + 76368 + 1 ≤ T.card)
    (hQdeg : ∀ x, (Q x).natDegree ≤ 32415)
  ...
  let A : Finset F := Finset.univ.filter fun x =>
    32415 < (T.filter fun z => x ∈ Arow z).card
  have hAcard : 131072 ≤ A.card := by
    simpa [A] using concrete_many_large_fibers T Arow 1 1 32415
      hcardF hrow hsize
  let Fib : {x // x ∈ A} → Finset F := fun x =>
    T.filter fun z => (x : F) ∈ Arow z
  have hline := bchksLinearBranchFinal 131071 32415 T A P U₀ U₁ Q Hlin
