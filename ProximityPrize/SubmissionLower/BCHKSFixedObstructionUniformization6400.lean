...
theorem fixed_obstruction_uniformizes_gamma_6400
  ...
  have hGdegree : ∀ i, (G i).natDegree ≤ 1670075 := by
    intro i
    exact fixedAffineOwnerObstruction_degree_le_of_yzCap
      R (omega i) (U₀ i) (U₁ i) (J i) 1670075
      (hB0 i) (hJmem i) hYZ
  ...
  have hcount :
      (262144 - 76780 - 131071) * T.card >
        (262144 - 131071) * 1670075 :=
    score6400_fixed_obstruction_large_fibers_numeric T.card hTcard
  have hzeroCount :
      131071 + 1 ≤ (Finset.univ.filter fun i : I => G i = 0).card := by
    apply many_zero_obstructions_of_many_large_fibers
      T A G 262144 76780 131071 1670075 hn
    · intro z hz
