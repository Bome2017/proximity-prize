...
theorem bchks_staged_capacity_budget :
    631967 * 261561 * 729 ^ 2 +
      ...
      2 * 261561 * 729 + 261561 < bchksNumerator := by
...
theorem bchks_staged_card_after_Qbad
    ...
    (hS : bchksNumerator < S.card) (hQBad : (S ∩ QBad).card ≤ 261561) :
    631967 * 261561 * 729 ^ 2 +
      (2 * 95502114 * 261561 + (bchksErrors + 1) + 261561) * 729 + 2 * 261561 * 729 < (S \ QBad).card := by
  rw [Finset.card_sdiff]
  apply Nat.lt_sub_of_add_lt
  have hb := bchks_staged_capacity_budget
  have hi : (QBad ∩ S).card ≤ 261561 := by simpa [Finset.inter_comm] using hQBad
...
theorem bchks_staged_after_badZSpecializations
    ...
    (hdeg : ((Q.coeff j).coeff a).natDegree < 261561)
    (hS : bchksNumerator < S.card) :
    631967 * 261561 * 729 ^ 2 +
      (2 * 95502114 * 261561 + (bchksErrors + 1) + 261561) * 729 + 2 * 261561 * 729 <
        (S \ badZSpecializations Q S).card := by
  apply bchks_staged_card_after_Qbad S (badZSpecializations Q S) hS
  have hb := badZSpecializations_card_le_261560 Q S j a hc hdeg
