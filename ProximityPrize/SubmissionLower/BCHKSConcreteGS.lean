...
open scoped BigOperators in
theorem card_var : Fintype.card VarIndex = 9104482636515090 := by
  rw [Fintype.card_sigma]
  simp only [Fintype.card_prod, Fintype.card_fin, Finset.sum_fin_eq_sum_range]
  norm_num (config := { maxSteps := 80000000 }) [DX, DY, DZ, k, Finset.sum_range_succ]

open scoped BigOperators in
theorem card_con : Fintype.card ConIndex = 9104482634301440 := by
  rw [Fintype.card_prod, Fintype.card_sigma]
  simp only [Fintype.card_prod, Fintype.card_fin, Finset.sum_fin_eq_sum_range]
  norm_num (config := { maxSteps := 80000000 }) [n, m, DZ, Finset.sum_range_succ]
