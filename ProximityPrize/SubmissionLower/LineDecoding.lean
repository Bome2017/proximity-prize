...
private theorem curated_outside_exists_collision_mismatch
  ...
  let zset := Finset.univ.filter fun i : ι =>
    i ∉ J ∧ u 0 i = 0 ∧ u 1 i = 0
  have hZS : zset.card < S.card := lt_of_lt_of_le hZcard hScard
...
private theorem exists_synthetic_affine_zero_set
    ...
    (hinj : Function.Injective pick) (zset : Finset ι)
    (hpickNotZ : ∀ γ, pick γ ∉ zset)
    (k : ℕ) (hkpos : 0 < k) (hkZ : k ≤ zset.card)
    (x : A) (hx : x ≠ 0) :
    ∃ v₀ v₁ : ι → A, ∀ β : F,
...
private theorem active_outside_common_zero_card_lt
    ...
    let zset := Finset.univ.filter fun i : ι =>
      i ∉ J ∧ u 0 i = 0 ∧ u 1 i = 0
    zset.card < Fintype.card ι - Nat.floor (δ * Fintype.card ι) := by
  ...
  by_contra hZlt
  have hkZ : k ≤ zset.card := Nat.le_of_not_gt hZlt
  ...
  have hZpos : 0 < zset.card := hkpos.trans_le hkZ
...
private theorem subsingleton_original_line_exact
  ...
  · intro hclose
    rw [Code.relCloseToWord_iff_exists_agreementCols] at hclose
    obtain ⟨S, hScard, hSagree⟩ := hclose
    let zset := Finset.univ.filter fun i : ι => u 0 i = 0 ∧ u 1 i = 0
    have hZS : zset.card < S.card := hZcard.trans_le hScard
    obtain ⟨i, hiS, hiZ⟩ := Finset.exists_mem_notMem_of_card_lt_card
      (s := zset) (t := S) hZS
...
private theorem subsingleton_synthetic_line_exact
  ...
  have hkpos : 0 < k := by
    simpa only [k, n] using card_sub_floor_pos_of_lt_one hnpos δ hδlt
  have hkZ : k ≤ zset.card := by
    simpa only [k, n, zset] using hZcard
  obtain ⟨pick, hinj, _hroot, hmismatch⟩ :=
    subsingleton_bad_set_pick C δ u B w
  have hpickNotZ : ∀ γ, pick γ ∉ zset := by
    intro γ hiZ
  ...
  obtain ⟨v₀, v₁, hzero⟩ :=
    exists_synthetic_affine_zero_set B pick hinj zset hpickNotZ k hkpos hkZ x hx
  refine ⟨v₀, v₁, ?_⟩
  intro γ
