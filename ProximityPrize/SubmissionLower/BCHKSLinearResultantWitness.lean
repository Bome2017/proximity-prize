...
theorem exists_linear_resultant_witness
    ...
    ∃ Res : Polynomial ProximityPrize.Benchmark.IRSProfile.Field,
      Res ≠ 0 ∧ Res.natDegree ≤ (2 * 116073546 - 131071) * 549940 ∧
      ∀ z ∈ T, Res.eval z ≠ 0 → ∀ x,
        (linearHSpecialization R z x).natDegree = 1 := by
  classical
  have hOuter : ∀ j, (R.coeff j).natDegree ≤ 116073545 := by
    intro j
    ...
    · have hne : (R.coeff j).coeff (R.coeff j).natDegree ≠ 0 := by
      ...
      omega
  have hInner : ∀ j, Polynomial.Bivariate.degreeX (R.coeff j) ≤ 549940 := by
    intro j
  ...
  have hOuterOne : (R.coeff 1).natDegree + 131071 ≤ 116073545 := by
  ...
  obtain ⟨w, hwdeg⟩ := effectivePrimitiveObstructionZ_of_irreducible
    R hRi (by omega) 549940 116073545 131071 (by norm_num [F,
