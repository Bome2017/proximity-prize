...
/-- A monic irreducible positive-`Y` factor with `Y`-degree below 312 has
...
theorem concrete_monic_factor_discr_ne_zero
    ...
    (hpos : 0 < R.natDegree) (hdeg : R.natDegree < 312) : R.discr ≠ 0 := by
...
theorem rawDiscrRat_ne_zero_and_natDegree
    (R : Polynomial (Polynomial (Polynomial
      ProximityPrize.Benchmark.IRSProfile.Field)))
    (hmonic : R.Monic) (hirr : Irreducible R)
    (hpos : 0 < R.natDegree) (hdeg : R.natDegree < 312) :
...
budget.  It allows 312 factors, one degree-32414 survival obstruction and one
`(2·312-1)·32414` resultant/discriminant obstruction per factor. -/
theorem crude_concrete_bad_degree_budget :
    312 * (32414 + (2 * 312 - 1) * 32414) ≤ bchksBadBudget := by
...
theorem rat_discriminants_sum_degree_le_badBudget
    ...
    (hcard : T.card ≤ 312)
    (hdeg : ∀ r ∈ T,
      (ratDiscr (R r)).natDegree ≤ 32414 + (2 * 312 - 1) * 32414) :
    (∑ r ∈ T, (ratDiscr (R r)).natDegree) ≤ bchksBadBudget := by
  calc
    (∑ r ∈ T, (ratDiscr (R r)).natDegree) ≤
        ∑ _r ∈ T, (32414 + (2 * 312 - 1) * 32414) :=
      Finset.sum_le_sum hdeg
    _ = T.card * (32414 + (2 * 312 - 1) * 32414) := by simp
    _ ≤ 312 * (32414 + (2 * 312 - 1) * 32414) := by
