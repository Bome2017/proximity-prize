...
/-- A monic irreducible positive-`Y` factor with `Y`-degree below 842 has
...
theorem concrete_monic_factor_discr_ne_zero
    ...
    (hpos : 0 < R.natDegree) (hdeg : R.natDegree < 842) : R.discr ≠ 0 := by
...
theorem rawDiscrRat_ne_zero_and_natDegree
    (R : Polynomial (Polynomial (Polynomial
      ProximityPrize.Benchmark.IRSProfile.Field)))
    (hmonic : R.Monic) (hirr : Irreducible R)
    (hpos : 0 < R.natDegree) (hdeg : R.natDegree < 842) :
...
budget.  It allows 842 factors, one degree-497543 survival obstruction and one
`(2·842-1)·497543` resultant/discriminant obstruction per factor. -/
theorem crude_concrete_bad_degree_budget :
    842 * (497543 + (2 * 842 - 1) * 497543) ≤ bchksBadBudget := by
...
theorem rat_discriminants_sum_degree_le_badBudget
    ...
    (hcard : T.card ≤ 842)
    (hdeg : ∀ r ∈ T,
      (ratDiscr (R r)).natDegree ≤ 497543 + (2 * 842 - 1) * 497543) :
    (∑ r ∈ T, (ratDiscr (R r)).natDegree) ≤ bchksBadBudget := by
  calc
    (∑ r ∈ T, (ratDiscr (R r)).natDegree) ≤
        ∑ _r ∈ T, (497543 + (2 * 842 - 1) * 497543) :=
      Finset.sum_le_sum hdeg
    _ = T.card * (497543 + (2 * 842 - 1) * 497543) := by simp
    _ ≤ 842 * (497543 + (2 * 842 - 1) * 497543) := by
