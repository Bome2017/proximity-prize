...
/-- A monic irreducible positive-`Y` factor with `Y`-degree below 729 has
...
theorem concrete_monic_factor_discr_ne_zero
    ...
    (hpos : 0 < R.natDegree) (hdeg : R.natDegree < 729) : R.discr ≠ 0 := by
...
theorem rawDiscrRat_ne_zero_and_natDegree
    (R : Polynomial (Polynomial (Polynomial
      ProximityPrize.Benchmark.IRSProfile.Field)))
    (hmonic : R.Monic) (hirr : Irreducible R)
    (hpos : 0 < R.natDegree) (hdeg : R.natDegree < 729) :
...
budget.  It allows 729 factors, one degree-261561 survival obstruction and one
`(2·729-1)·261561` resultant/discriminant obstruction per factor. -/
theorem crude_concrete_bad_degree_budget :
    729 * (261561 + (2 * 729 - 1) * 261561) ≤ bchksBadBudget := by
...
theorem rat_discriminants_sum_degree_le_badBudget
    ...
    (hcard : T.card ≤ 729)
    (hdeg : ∀ r ∈ T,
      (ratDiscr (R r)).natDegree ≤ 261561 + (2 * 729 - 1) * 261561) :
    (∑ r ∈ T, (ratDiscr (R r)).natDegree) ≤ bchksBadBudget := by
  calc
    (∑ r ∈ T, (ratDiscr (R r)).natDegree) ≤
        ∑ _r ∈ T, (261561 + (2 * 729 - 1) * 261561) :=
      Finset.sum_le_sum hdeg
    _ = T.card * (261561 + (2 * 729 - 1) * 261561) := by simp
    _ ≤ 729 * (261561 + (2 * 729 - 1) * 261561) := by
