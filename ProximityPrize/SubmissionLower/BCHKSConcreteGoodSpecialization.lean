...
/-- A monic irreducible positive-`Y` factor with `Y`-degree below 282 has
...
theorem concrete_monic_factor_discr_ne_zero
    ...
    (hpos : 0 < R.natDegree) (hdeg : R.natDegree < 282) : R.discr ≠ 0 := by
...
theorem rawDiscrRat_ne_zero_and_natDegree
    (R : Polynomial (Polynomial (Polynomial
      ProximityPrize.Benchmark.IRSProfile.Field)))
    (hmonic : R.Monic) (hirr : Irreducible R)
    (hpos : 0 < R.natDegree) (hdeg : R.natDegree < 282) :
...
budget.  It allows 282 factors, one degree-55744 survival obstruction and one
`(2·282-1)·55744` resultant/discriminant obstruction per factor. -/
theorem crude_concrete_bad_degree_budget :
    282 * (55744 + (2 * 282 - 1) * 55744) ≤ bchksBadBudget := by
...
theorem rat_discriminants_sum_degree_le_badBudget
    ...
    (hcard : T.card ≤ 282)
    (hdeg : ∀ r ∈ T,
      (ratDiscr (R r)).natDegree ≤ 55744 + (2 * 282 - 1) * 55744) :
    (∑ r ∈ T, (ratDiscr (R r)).natDegree) ≤ bchksBadBudget := by
  calc
    (∑ r ∈ T, (ratDiscr (R r)).natDegree) ≤
        ∑ _r ∈ T, (55744 + (2 * 282 - 1) * 55744) :=
      Finset.sum_le_sum hdeg
    _ = T.card * (55744 + (2 * 282 - 1) * 55744) := by simp
    _ ≤ 282 * (55744 + (2 * 282 - 1) * 55744) := by
