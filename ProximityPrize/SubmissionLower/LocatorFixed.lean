...
/-- Whole-polynomial support.  The Y=56/52/48 refinements below concern individual
irreducible factors only; they do not change this whole Y=59 budget. -/
def wholeSupport : ResidualSupportParameters :=
  ⟨12, 59, 1417, by decide, by decide, by decide, by decide⟩
...
theorem own_parameter_caps (p : FlagDegree)
    ...
    padSlope p + 2 ≤ 12 ∧ padB p + padSlope p + 3 ≤ 59 ∧
      padA p + padB p + padSlope p + 3 ≤ 1417 := by
  have hp := pad_sums p
  have hps : padS p ≤ 12 := max_le hs (by decide)
  have hpy : padY p ≤ 59 := max_le hy (by omega)
  have hpt : padT p ≤ 1417 := max_le ht (by omega)
  rw [hp.1, hp.2.1, hp.2.2]
  exact ⟨hps, hpy, hpt⟩

theorem regular_factor_count
    (Q : P4) (hQ : Q ≠ 0)
    (hbox : Q ∈ RCN174.globalCoefficientBox K 8188785 131071 1417 12)
    (HQ : ResidualSupportData wholeSupport Q)
    (selected : K → Polynomial K) (Gamma : Finset K) (u0 u1 : I → K)
    (hdegree : ∀ gamma ∈ Gamma, (selected gamma).natDegree ≤ 131071)
    (hagreement : ∀ gamma ∈ Gamma, 181973 ≤
      ((Finset.univ : Finset I).filter (fun i =>
        (selected gamma).eval (IRSProfile.domain i) = u0 i + gamma * u1 i)).card)
    (hno : NoLargeSelectedPencil selected Gamma 131071 80171)
    (R : RegularIndex Q) :
  ...
  have hstage (g : GeometricFactor K R.1) :
    ...
    have hnodes : S.nodes.card = 181973 + 80171 := by
      change (Finset.univ : Finset I).card = _
      norm_num [I, IRSProfile.Index]
    have hag : ∀ gamma ∈ geometricSeeds K R.1 selected
        (regularSeeds Q selected Gamma R) g, 181973 ≤ (S.agreementFiber gamma).card := by
      intro gamma hgamma
...
theorem regular_sum_count
    ...
    (∑ R : RegularIndex Q, (regularSeeds Q selected Gamma R).card) ≤
      273021899526170515 := by
  have hb := regularCumulativeFlag_budgets Q hQ HQ
  have hsSum : (∑ R : RegularIndex Q, (regularCumulativeFlag Q R).all) ≤ 12 :=
    hb.1
  have hySum : (∑ R : RegularIndex Q, middle (regularCumulativeFlag Q R)) ≤ 59 :=
    hb.2.1
  have htSum : (∑ R : RegularIndex Q, total (regularCumulativeFlag Q R)) ≤ 1417 :=
    hb.2.2
  have hs (R : RegularIndex Q) : (regularCumulativeFlag Q R).all ≤ 12 :=
    (Finset.single_le_sum (fun _ _ => Nat.zero_le _) (Finset.mem_univ R)).trans hsSum
  have hy (R : RegularIndex Q) : middle (regularCumulativeFlag Q R) ≤ 59 :=
    (Finset.single_le_sum (fun _ _ => Nat.zero_le _) (Finset.mem_univ R)).trans hySum
  have hcost := LocatorFactorAggregate.aggregate_6741 (regularCumulativeFlag Q)
    (regularCumulativeFlag_positive Q) hs hy htSum h10 h11 h12
...
def profile : RCN276.Profile :=
  ⟨262144, 131071, 181973, 8188785, 1417, 12⟩

/-- Fixed-polynomial count for the 80171-error row.  The whole polynomial keeps
its T/Y/R = 1417/59/12 support; only its individual R=10, R=11 and R=12 factors
receive the Y=56, Y=52 and Y=48 refinements. -/
theorem fixed_count_le
    ...
    (h12 : ∀ R : RegularIndex Q, (regularCumulativeFlag Q R).all = 12 →
      middle (regularCumulativeFlag Q R) ≤ 48) :
    Gamma.card ≤ 273021899526170515 + LocatorArithmetic.fixedSingular.countCap := by
  ...
  have hreg := regular_sum_count Q hQ hbox HQ selected Gamma u0 u1
    hdegree hagreement hno h10 h11 h12
