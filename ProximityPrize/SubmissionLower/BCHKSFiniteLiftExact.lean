...
theorem concreteFiniteAlpha_middle_vanish
    ...
    (C : ℕ → F[X])
    (hembed : ∀ t, 131071 < t → t < 40866320 →
      embeddingOf𝒪Into𝕃 H (concreteBetaUpTo x₀ R hHyp hzeta 40866320 t) =
        finiteAlpha (R := R) (H := H) x₀ 40866320 t *
          liftToFunctionField (H := H) (C t))
    (hC : ∀ t, 131071 < t → t < 40866320 → C t ≠ 0)
    (hweight : ∀ t, 131071 < t → t < 40866320 →
      regularWeight (Fact.out : 0 < H.natDegree)
          (concreteBetaUpTo x₀ R hHyp hzeta 40866320 t) D ≤
        (WithBot.some ((2 * t + 1) * d * D) : WithBot ℕ))
    (T : Finset F)
    (root : ∀ z : T, rationalRoot (monicize H) (z : F))
    (hden : ∀ t, 131071 < t → t < 40866320 →
      ∀ z : T, (C t).eval (z : F) ≠ 0)
    (hspecialize : ∀ t, 131071 < t → t < 40866320 → ∀ z : T,
      piZOfDiv (z : F) (root z)
        (concreteBetaUpTo x₀ R hHyp hzeta 40866320 t) (C t) = 0)
    (hcard : 2 * 40866320 * H.natDegree * d * D + e + 1 < T.card) :
    ∀ t, 131071 < t → t < 40866320 →
      finiteAlpha (R := R) (H := H) x₀ 40866320 t = 0 := by
  exact functionField_middle_coefficients_eq_zero_of_many_pair_roots
    (Fact.out : 0 < H.natDegree) D d 131071 40866320 e hD
    (finiteAlpha (R := R) (H := H) x₀ 40866320)
    (concreteBetaUpTo x₀ R hHyp hzeta 40866320) C
...
theorem concreteFiniteLift_trunc_exact
    ...
    (hres : FiniteHensel.VanishesThrough
      (FiniteHensel.residual (liftedR (R := R) (H := H))
        (fieldTo𝕃 (H := H) x₀) α 40866320) 40866320)
    (hmatch : ∀ n, n < 40866320 →
      ((liftedR (R := R) (H := H)).eval
        (FiniteHensel.truncSeries α 131071)).coeff n =
      (FiniteHensel.residual (liftedR (R := R) (H := H))
        (fieldTo𝕃 (H := H) x₀) α 40866320).coeff n)
    (hcap : ((liftedR (R := R) (H := H)).eval
      (FiniteHensel.truncSeries α 131071)).natDegree < 40866320) :
    (liftedR (R := R) (H := H)).eval
      (FiniteHensel.truncSeries α 131071) = 0 := by
  exact eval_truncSeries_eq_zero_of_residual_match_and_cap _ _ 40866320
    hres hmatch hcap
...
theorem concreteFiniteAlpha_trunc_exact
    ...
    (hres : FiniteHensel.VanishesThrough
      (FiniteHensel.residual (liftedR (R := R) (H := H))
        (fieldTo𝕃 (H := H) x₀)
        (finiteAlpha (R := R) (H := H) x₀ 40866320) 40866320) 40866320)
    (hmatch : ∀ n, n < 40866320 →
      ((liftedR (R := R) (H := H)).eval
        (FiniteHensel.truncSeries
          (finiteAlpha (R := R) (H := H) x₀ 40866320) 131071)).coeff n =
      (FiniteHensel.residual (liftedR (R := R) (H := H))
        (fieldTo𝕃 (H := H) x₀)
        (finiteAlpha (R := R) (H := H) x₀ 40866320) 40866320).coeff n)
    (hcap : ((liftedR (R := R) (H := H)).eval
      (FiniteHensel.truncSeries
        (finiteAlpha (R := R) (H := H) x₀ 40866320) 131071)).natDegree < 40866320) :
    (liftedR (R := R) (H := H)).eval
      (FiniteHensel.truncSeries
        (finiteAlpha (R := R) (H := H) x₀ 40866320) 131071) = 0 := by
  exact concreteFiniteLift_trunc_exact x₀ R
    (finiteAlpha (R := R) (H := H) x₀ 40866320) hres hmatch hcap
