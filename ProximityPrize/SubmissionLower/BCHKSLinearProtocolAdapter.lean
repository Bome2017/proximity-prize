...
theorem bchksLinearProtocolAdapter
    ...
    (hWeighted : ∀ j a, ((R.coeff j).coeff a) ≠ 0 →
      a + 131071 * j < 40866320)
    (hRdeg : R.natDegree = 1)
    (hTmargin : (76388 + 1) + 2 * 40866320 * 32414 < T.card) :
  ...
  have hBad : Bad.card ≤ (2 * 40866320 - 131071) * 32414 := by
    calc
  ...
  have hTgood : 76388 + 1 < Tgood.card := by omega
  have hlarge : 2 * 40866320 + 76388 + 1 ≤ Tgood.card := by
    have hsaved : 2 * 40866320 < 131071 * 32414 := by norm_num
    omega
  ...
  have hFibBig : ∀ x : Afield, 32414 < (Fib x).card := by
    intro x
    have := hFib x
    norm_num at this ⊢
    omega
  ...
  have hQdeg : ∀ x, (linearQSpecialization R U₀f U₁f x).natDegree ≤ 32414 := by
    intro x
  ...
  have hFibfcard : ∀ x : Afield, 32414 < (Fibf x).card := by
    intro x
    rw [show (Fibf x).card = (Fib x).card by
      exact Finset.card_image_iff.mpr fun a _ b _ h => Subtype.ext h]
    exact hFibBig x
  have hcore := bchksLinearSelectedCore 131071 32414 Tgood Afield P U₀f U₁f
