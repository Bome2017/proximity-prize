...
theorem bchksLinearProtocolAdapter
    ...
    (hWeighted : ∀ j a, ((R.coeff j).coeff a) ≠ 0 →
      a + 131071 * j < 95502114)
    (hRdeg : R.natDegree = 1)
    (hTmargin : (76703 + 1) + 2 * 95502114 * 261561 < T.card) :
  ...
  have hBad : Bad.card ≤ (2 * 95502114 - 131071) * 261561 := by
    calc
  ...
  have hTgood : 76703 + 1 < Tgood.card := by omega
  have hlarge : 631967 * 1 * 1 * 1 + 76703 + 1 ≤ Tgood.card := by
    have hsaved : 2 * 95502114 < 131071 * 261561 := by norm_num
    omega
  ...
  have hFibBig : ∀ x : Afield, 261561 < (Fib x).card := by
    intro x
    have := hFib x
    norm_num at this ⊢
    omega
  ...
  have hQdeg : ∀ x, (linearQSpecialization R U₀f U₁f x).natDegree ≤ 261561 := by
    intro x
  ...
  have hFibfcard : ∀ x : Afield, 261561 < (Fibf x).card := by
    intro x
    rw [show (Fibf x).card = (Fib x).card by
      exact Finset.card_image_iff.mpr fun a _ b _ h => Subtype.ext h]
    exact hFibBig x
  have hcore := bchksLinearSelectedCore 131071 261561 Tgood Afield P U₀f U₁f
