...
theorem bchksLinearProtocolAdapter
    ...
    (hRdeg : R.natDegree = 1)
    (hTmargin : (76717 + 1) + 2 * 104951682 * 453561 < T.card) :
  ...
  have hBad : Bad.card ≤ (2 * 104951682 - 131071) * 453561 := by
    calc
  ...
  have hTupper : T.card ≤ Tgood.card + (2 * 104951682 - 131071) * 453561 := by
    calc
      T.card = Tgood.card + (T.filter fun z => z ∈ Bad).card := hpart.symm
      _ ≤ Tgood.card + Bad.card := Nat.add_le_add_left hbadT _
      _ ≤ Tgood.card + (2 * 104951682 - 131071) * 453561 :=
        Nat.add_le_add_left hBad _
  have hTgoodLower : 59448770550 ≤ Tgood.card := by
    have hsum :
        (2 * 104951682 - 131071) * 453561 + 59448770550 ≤
          (2 * 104951682 - 131071) * 453561 + Tgood.card := by
      calc
        (2 * 104951682 - 131071) * 453561 + 59448770550 =
            (76717 + 1) + 2 * 104951682 * 453561 + 1 := by norm_num
        _ ≤ T.card := Nat.succ_le_of_lt hTmargin
        _ ≤ Tgood.card + (2 * 104951682 - 131071) * 453561 := hTupper
        _ = (2 * 104951682 - 131071) * 453561 + Tgood.card := by omega
    exact Nat.le_of_add_le_add_left hsum
  have hTgood : 76717 + 1 < Tgood.card := by omega
  have hlarge : 634000 * 2 + 76717 + 1 ≤ Tgood.card := by
    omega
  let PT : Tgood → IRSProfile.Field[X] := fun z => P z
  obtain ⟨Afield, hAfield, Fib, hFib, hinc⟩ :=
    exists_large_domain_fibers_alignment_only U Tgood Arow PT 1 1 2
      (by intro z hz; exact hrow z (hsub hz)) hlarge
      (by intro z i hi; exact hagree z (hsub z.property) i hi)
  have hFibBig : ∀ x : Afield, 453561 < (Fib x).card := by
    intro x
    have hx : 524288 < (Fib x).card := by simpa using hFib x
    exact lt_trans (by norm_num : 453561 < 524288) hx
  ...
  have hQdeg : ∀ x, (linearQSpecialization R U₀f U₁f x).natDegree ≤ 453561 := by
    intro x
  ...
  have hFibfcard : ∀ x : Afield, 453561 < (Fibf x).card := by
    intro x
    rw [show (Fibf x).card = (Fib x).card by
      exact Finset.card_image_iff.mpr fun a _ b _ h => Subtype.ext h]
    exact hFibBig x
  have hcore := bchksLinearSelectedCore 131071 453561 Tgood Afield P U₀f U₁f
