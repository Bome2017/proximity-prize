...
theorem regularVector_budgets (Q : MvPolynomial (Fin 4) K) (hQ : Q ≠ 0)
    ...
    (∑ F : RegularIndex Q, (regularVector Q F).y) ≤ 33 ∧
      (∑ F : RegularIndex Q, (regularVector Q F).r) ≤ 7 ∧
      (∑ F : RegularIndex Q, (regularVector Q F).z) ≤ 246 := by
  classical
  have hb := directFactor_input_budgets Q hQ weightedCap w seedTotalCap slopeCap (by decide) hbox
  have hy : (∑ F ∈ positiveRFactors Q, F.degreeOf (1 : Fin 4)) ≤ 33 := by
    have hh := hb.1
