...
theorem bchks_staged_unconditional
    ...
    ∃ R H T x₀, ∃ Bad : Finset ProximityPrize.Benchmark.IRSProfile.Field,
      ...
      2 * 40870720 * 32414 * R.natDegree * H.natDegree +
  ...
  let cert : ∀ R : Polynomial (Polynomial (Polynomial
    ...
    intro R hRQ hp
    have hRYZ := YZFactorCap.normalizedFactor_YZ_cap Q R 32414 hQ hRQ hQYZ
    have hRW := WeightedFactorCaps.coeff_cap_of_dvd Q R 131071 40870720 hQ
  ...
  have hcertBound : ∀ R (hRQ : R ∈ UniqueFactorizationMonoid.normalizedFactors Q)
    ...
    exact Classical.choose_spec
      (effectivePrimitiveObstruction_of_irreducible R
        ...
        (by
          intro j
          have hcap := YZFactorCap.normalizedFactor_YZ_cap Q R 32414 hQ hRQ hQYZ
          rw [show (Polynomial.Bivariate.swap (R.coeff j)).natDegree =
            Polynomial.Bivariate.natDegreeY (Polynomial.Bivariate.swap (R.coeff j)) from rfl,
            Polynomial.Bivariate.natDegreeY_swap]
          unfold Polynomial.Bivariate.degreeX
        ...
        (by
          ...
          by_cases hz : R.coeff j = 0
          · simp [hz]
          · have hcap := WeightedFactorCaps.coeff_cap_of_dvd Q R 131071 40870720 hQ
              (UniqueFactorizationMonoid.dvd_of_mem_normalizedFactors hRQ) hQweightedX
    ...
    have hfx : (factorXObstruction R).natDegree ≤
      ...
      · unfold Polynomial.Bivariate.degreeX
        ...
        exact Polynomial.natDegree_map_le.trans (by
          have hc := WeightedFactorCaps.coeff_cap_of_dvd Q R 131071 40870720 hQ
            (UniqueFactorizationMonoid.dvd_of_mem_normalizedFactors hRQ) hQweightedX
          by_cases hz : R.coeff j = 0
          · simp [hz]
  ...
  · intro R hRQ hp x
    have hRYZ := YZFactorCap.normalizedFactor_YZ_cap Q R 32414 hQ hRQ hQYZ
    have hcoeffCap : ∀ j, Polynomial.Bivariate.degreeX (R.coeff j) ≤ 32414 := by
      intro j
      unfold Polynomial.Bivariate.degreeX
      apply Finset.sup_le
      intro a ha
    ...
    have hevalCap : ∀ p : Polynomial (Polynomial
        ProximityPrize.Benchmark.IRSProfile.Field),
        Polynomial.Bivariate.degreeX p ≤ 32414 →
        (Polynomial.eval (Polynomial.C x) p).natDegree ≤ 32414 := by
      intro p hpdeg
      ...
      exact Polynomial.natDegree_map_le.trans (by
        rw [show (Polynomial.Bivariate.swap p).natDegree =
          ...
          Polynomial.Bivariate.natDegreeY_swap]
        exact hpdeg)
    apply factorXObstruction_eval_natDegree_le R x R.natDegree 32414 hp (le_refl _)
    · unfold Polynomial.Bivariate.degreeX
      apply Finset.sup_le
      intro j hj
  ...
  · let E := ProximityPrize.Benchmark.IRSProfile.Field
    ...
    by_contra hn
    have hle : ringChar E ≤ 312 := Nat.le_of_not_gt hn
