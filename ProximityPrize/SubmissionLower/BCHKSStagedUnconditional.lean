...
theorem bchks_staged_unconditional
    ...
    (hQweightedX : ∀ j a, ((Q.coeff j).coeff a) ≠ 0 →
      a + 131071 * j < 95502114)
    (hS : 631967 * 261561 * 729 ^ 2 +
      (2 * 95502114 * 261561 + (bchksErrors + 1) + 261561) * 729 + 2 * 261561 * 729 < S.card) :
    ∃ R H T x₀, ∃ Bad : Finset ProximityPrize.Benchmark.IRSProfile.Field,
      ...
      631967 * 261561 * R.natDegree * H.natDegree +
        (2 * 95502114 * 261561 + (bchksErrors + 1) + 261561) < T.card ∧
  ...
  let cert : ∀ R : Polynomial (Polynomial (Polynomial
    ...
    intro R hRQ hp
    have hRYZ := YZFactorCap.normalizedFactor_YZ_cap Q R 261561 hQ hRQ hQYZ
    have hRW := WeightedFactorCaps.coeff_cap_of_dvd Q R 131071 95502114 hQ
  ...
  have hcertBound : ∀ R (hRQ : R ∈ UniqueFactorizationMonoid.normalizedFactors Q)
    ...
    exact Classical.choose_spec
      (effectivePrimitiveObstruction_of_irreducible R
        ...
        (by
          intro j
          have hcap := YZFactorCap.normalizedFactor_YZ_cap Q R 261561 hQ hRQ hQYZ
          rw [show (Polynomial.Bivariate.swap (R.coeff j)).natDegree =
            Polynomial.Bivariate.natDegreeY (Polynomial.Bivariate.swap (R.coeff j)) from rfl,
            Polynomial.Bivariate.natDegreeY_swap]
          unfold Polynomial.Bivariate.degreeX
        ...
        (by
          ...
          by_cases hz : R.coeff j = 0
          · simp [hz]
          · have hcap := WeightedFactorCaps.coeff_cap_of_dvd Q R 131071 95502114 hQ
              (UniqueFactorizationMonoid.dvd_of_mem_normalizedFactors hRQ) hQweightedX
    ...
    have hfx : (factorXObstruction R).natDegree ≤
      ...
      · unfold Polynomial.Bivariate.degreeX
        ...
        exact Polynomial.natDegree_map_le.trans (by
          have hc := WeightedFactorCaps.coeff_cap_of_dvd Q R 131071 95502114 hQ
            (UniqueFactorizationMonoid.dvd_of_mem_normalizedFactors hRQ) hQweightedX
          by_cases hz : R.coeff j = 0
          · simp [hz]
  ...
  · intro R hRQ hp x
    have hRYZ := YZFactorCap.normalizedFactor_YZ_cap Q R 261561 hQ hRQ hQYZ
    have hcoeffCap : ∀ j, Polynomial.Bivariate.degreeX (R.coeff j) ≤ 261561 := by
      intro j
      unfold Polynomial.Bivariate.degreeX
      apply Finset.sup_le
      intro a ha
    ...
    have hevalCap : ∀ p : Polynomial (Polynomial
        ProximityPrize.Benchmark.IRSProfile.Field),
        Polynomial.Bivariate.degreeX p ≤ 261561 →
        (Polynomial.eval (Polynomial.C x) p).natDegree ≤ 261561 := by
      intro p hpdeg
      ...
      exact Polynomial.natDegree_map_le.trans (by
        rw [show (Polynomial.Bivariate.swap p).natDegree =
          ...
          Polynomial.Bivariate.natDegreeY_swap]
        exact hpdeg)
    apply factorXObstruction_eval_natDegree_le R x R.natDegree 261561 hp (le_refl _)
    · unfold Polynomial.Bivariate.degreeX
      apply Finset.sup_le
      intro j hj
  ...
  · let E := ProximityPrize.Benchmark.IRSProfile.Field
    ...
    by_contra hn
    have hle : ringChar E ≤ 729 := Nat.le_of_not_gt hn
