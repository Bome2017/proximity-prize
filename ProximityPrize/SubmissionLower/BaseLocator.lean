...
/-- MCA cannot occur along a zero direction word.  Hence the fixed direction
polynomial in the affine incidence equation is nonzero. -/
theorem bad_slope_receivedPolynomial_direction_ne_zero
    ...
    receivedPolynomial (U 1) ≠ 0 := by
  classical
  obtain ⟨T, hTcard, hcomb, j, hj⟩ := hgamma
  have haff :
      (fun k => ∑ j, AffineLineGenerator IRSProfile.Field gamma j • U j k) =
        Affine.affineComb U (fun _ : Fin 1 => gamma) := by
    funext k
    simp [AffineLineGenerator, Affine.affineComb, Fin.sum_univ_two]
  rw [haff] at hcomb
  have hsplit :
      LinearCode.projectedWord
          (Affine.affineComb U (fun _ : Fin 1 => gamma)) T =
        LinearCode.projectedWord (U 0) T +
          gamma • LinearCode.projectedWord (U 1) T := by
    ext k
    simp [LinearCode.projectedWord, Affine.affineComb]
  have hU1not : LinearCode.projectedWord (U 1) T ∉
      LinearCode.projectedCodeSubmod IRSProfile.baseCode T := by
    intro hU1
    apply hj
    fin_cases j
    · have hmem := Submodule.sub_mem _ (hsplit ▸ hcomb)
          (Submodule.smul_mem _ gamma hU1)
  ...
  have hUzero : U 1 = 0 := by
    funext k
    have heval := receivedPolynomial_eval (U 1) k
    rw [hzero, Polynomial.eval_zero] at heval
    exact heval.symm
  rw [hUzero]
  change (0 : T → IRSProfile.Field) ∈
    LinearCode.projectedCodeSubmod IRSProfile.baseCode T
...
/-- For a fixed admissible triple `(S, P, H)`, the affine Gao equation
determines at most one slope. -/
theorem affine_gao_witness_determines_slope
    (U : Fin 2 → IRSProfile.Index → IRSProfile.Field)
    (gamma eta : IRSProfile.Field) (S : Finset IRSProfile.Index)
    (P H : Polynomial IRSProfile.Field)
    (hgamma : IsMCA (AffineLineGenerator IRSProfile.Field)
      IRSProfile.baseCode gamma U (cellRadius : ℝ))
    (hgammaEq : receivedPolynomial (U 0) +
        Polynomial.C gamma * receivedPolynomial (U 1) =
      P + extensionLocator S * H)
    (hetaEq : receivedPolynomial (U 0) +
        Polynomial.C eta * receivedPolynomial (U 1) =
      P + extensionLocator S * H) :
    gamma = eta := by
  have hdir := bad_slope_receivedPolynomial_direction_ne_zero U gamma hgamma
  have hmul : Polynomial.C gamma * receivedPolynomial (U 1) =
      Polynomial.C eta * receivedPolynomial (U 1) :=
    add_left_cancel (hgammaEq.trans hetaEq.symm)
  have hC : Polynomial.C gamma = Polynomial.C eta :=
    mul_right_cancel₀ hdir hmul
