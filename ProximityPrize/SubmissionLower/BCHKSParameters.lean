...
noncomputable def bchksRadius : ℝ≥0 := (306883 : ℝ≥0) / 1048576

def bchksErrors : ℕ := 76720
...
theorem two_rpow_six_hundred_ge :
    (1042 : ℝ≥0) / 1000 ≤ (2 : ℝ≥0) ^ ((6 : ℝ) / 100) := by
  have hroot :
    (1042 : ℝ≥0) / 1000 ≤
        ((2 : ℝ≥0) ^ (6 : ℕ)) ^ ((100 : ℝ)⁻¹) := by
...
lemma bchks_all_factor_budget :
    632164 * 497543 * 842 ^ 2 + (bchksErrors + 1) * 842 <
      bchksNumerator := by
...
lemma card_sdiff_large_for_bchks {α : Type*} [DecidableEq α]
  ...
  let M : ℕ := 632164 * 497543 * 842 ^ 2 + (bchksErrors + 1) * 842
...
lemma bchks_top_points_arithmetic (r T : ℕ)
    (hT : 632164 * r + (bchksErrors + 1) < T) :
    (262144 - bchksErrors - 131071) * T >
