...
noncomputable def bchksRadius : ℝ≥0 := (305475 : ℝ≥0) / 1048576

def bchksErrors : ℕ := 76368

def bchksNumerator : ℕ := 258000000000000000

def bchksListBound : ℕ := 640
...
/-- Rational lower approximation to `2^(41/100)`, used by the 63.59 score certificate. -/
theorem two_rpow_forty_one_hundred_ge :
    (3321 : ℝ≥0) / 2500 ≤ (2 : ℝ≥0) ^ ((41 : ℝ) / 100) := by
  have hroot :
      (3321 : ℝ≥0) / 2500 ≤
        ((2 : ℝ≥0) ^ (41 : ℕ)) ^ ((100 : ℝ)⁻¹) := by
  ...
  calc
    (3321 : ℝ≥0) / 2500 ≤
        ((2 : ℝ≥0) ^ (41 : ℕ)) ^ ((100 : ℝ)⁻¹) := hroot
    _ = (2 : ℝ≥0) ^ ((41 : ℝ) / 100) := by
      rw [← NNReal.rpow_natCast_mul]
      norm_num [div_eq_mul_inv]

/-- Standalone score arithmetic for the intended `ProtocolClaim 6359`. -/
lemma bchksRadius_score :
    (1 - bchksRadius) ^ IRSProfile.repetitions ≤
      ProximityPrize.Benchmark.claimedError 6359 := by
  calc
    (1 - bchksRadius) ^ IRSProfile.repetitions ≤
        ((1 : ℝ≥0) / 2 ^ (64 : ℕ)) * (3321 / 2500) := by
      rw [← NNReal.coe_le_coe]
      norm_num [bchksRadius, IRSProfile.repetitions, div_le_iff₀]
    _ ≤ ((1 : ℝ≥0) / 2 ^ (64 : ℕ)) *
          (2 : ℝ≥0) ^ ((41 : ℝ) / 100) := by
      exact mul_le_mul_of_nonneg_left two_rpow_forty_one_hundred_ge (by positivity)
    _ = ProximityPrize.Benchmark.claimedError 6359 := by
      unfold ProximityPrize.Benchmark.claimedError
      rw [show -((((6359 : ℕ) : ℝ) / 100)) =
          -((64 : ℕ) : ℝ) + (41 : ℝ) / 100 by norm_num,
...
lemma bchks_all_factor_budget :
    2 * 40870720 * 32414 * 312 ^ 2 + (bchksErrors + 1) * 312 <
      bchksNumerator := by
...
lemma card_sdiff_large_for_bchks {α : Type*} [DecidableEq α]
  ...
  let M : ℕ := 2 * 40870720 * 32414 * 312 ^ 2 + (bchksErrors + 1) * 312
...
lemma bchks_top_points_arithmetic (r T : ℕ)
    (hT : 2 * 40870720 * r + (bchksErrors + 1) < T) :
