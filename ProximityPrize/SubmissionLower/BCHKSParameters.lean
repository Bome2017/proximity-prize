...
/-- Radius chosen for a 63.95-bit claim after the specialized BCHKS extraction. -/
noncomputable def bchksRadius : ℝ≥0 := (306921 : ℝ≥0) / 1048576

def bchksErrors : ℕ := 76730

def bchksNumerator : ℕ := 274980000000000000

def bchksListBound : ℕ := 1410
...
/-- Rational lower approximation to `2^(5/100)`, used by the score certificate. -/
theorem two_rpow_five_hundred_ge :
    (10351 : ℝ≥0) / 10000 ≤ (2 : ℝ≥0) ^ ((5 : ℝ) / 100) := by
  have hroot :
      (10351 : ℝ≥0) / 10000 ≤
        ((2 : ℝ≥0) ^ (5 : ℕ)) ^ ((100 : ℝ)⁻¹) := by
  ...
  calc
    (10351 : ℝ≥0) / 10000 ≤
        ((2 : ℝ≥0) ^ (5 : ℕ)) ^ ((100 : ℝ)⁻¹) := hroot
    _ = (2 : ℝ≥0) ^ ((5 : ℝ) / 100) := by
      rw [← NNReal.rpow_natCast_mul]
      norm_num [div_eq_mul_inv]

/-- Standalone score arithmetic for the intended `ProtocolClaim 6395`. -/
lemma bchksRadius_score :
    (1 - bchksRadius) ^ IRSProfile.repetitions ≤
      ProximityPrize.Benchmark.claimedError 6395 := by
  calc
    (1 - bchksRadius) ^ IRSProfile.repetitions ≤
        ((1 : ℝ≥0) / 2 ^ (64 : ℕ)) * (10351 / 10000) := by
      rw [← NNReal.coe_le_coe]
      norm_num [bchksRadius, IRSProfile.repetitions, div_le_iff₀]
    _ ≤ ((1 : ℝ≥0) / 2 ^ (64 : ℕ)) *
          (2 : ℝ≥0) ^ ((5 : ℝ) / 100) := by
      exact mul_le_mul_of_nonneg_left two_rpow_five_hundred_ge (by positivity)
    _ = ProximityPrize.Benchmark.claimedError 6395 := by
      unfold ProximityPrize.Benchmark.claimedError
      rw [show -((((6395 : ℕ) : ℝ) / 100)) =
          -((64 : ℕ) : ℝ) + (5 : ℝ) / 100 by norm_num,
