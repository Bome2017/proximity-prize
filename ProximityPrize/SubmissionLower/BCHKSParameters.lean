...
/-- Radius chosen for a 63.79-bit claim after the specialized BCHKS extraction. -/
noncomputable def bchksRadius : ℝ≥0 := (306280 : ℝ≥0) / 1048576

def bchksErrors : ℕ := 76570

def bchksNumerator : ℕ := 258000000000000000

def bchksListBound : ℕ := 1248
...
/-- Rational lower approximation to `2^(21/100)`, used by the score certificate. -/
theorem two_rpow_twenty_one_hundred_ge :
    (37 : ℝ≥0) / 32 ≤ (2 : ℝ≥0) ^ ((21 : ℝ) / 100) := by
  have hroot :
      (37 : ℝ≥0) / 32 ≤
        ((2 : ℝ≥0) ^ (21 : ℕ)) ^ ((100 : ℝ)⁻¹) := by
  ...
  calc
    (37 : ℝ≥0) / 32 ≤
        ((2 : ℝ≥0) ^ (21 : ℕ)) ^ ((100 : ℝ)⁻¹) := hroot
    _ = (2 : ℝ≥0) ^ ((21 : ℝ) / 100) := by
      rw [← NNReal.rpow_natCast_mul]
      norm_num [div_eq_mul_inv]

/-- Standalone score arithmetic for the intended `ProtocolClaim 6379`. -/
lemma bchksRadius_score :
    (1 - bchksRadius) ^ IRSProfile.repetitions ≤
      ProximityPrize.Benchmark.claimedError 6379 := by
  calc
    (1 - bchksRadius) ^ IRSProfile.repetitions ≤
        ((1 : ℝ≥0) / 2 ^ (64 : ℕ)) * (37 / 32) := by
      rw [← NNReal.coe_le_coe]
      norm_num [bchksRadius, IRSProfile.repetitions, div_le_iff₀]
    _ ≤ ((1 : ℝ≥0) / 2 ^ (64 : ℕ)) *
          (2 : ℝ≥0) ^ ((21 : ℝ) / 100) := by
      exact mul_le_mul_of_nonneg_left two_rpow_twenty_one_hundred_ge (by positivity)
    _ = ProximityPrize.Benchmark.claimedError 6379 := by
      unfold ProximityPrize.Benchmark.claimedError
      rw [show -((((6379 : ℕ) : ℝ) / 100)) =
          -((64 : ℕ) : ℝ) + (21 : ℝ) / 100 by norm_num,
...
lemma bchks_all_factor_budget :
    2 * 40826205 * 32414 * 312 ^ 2 + (bchksErrors + 1) * 312 <
      bchksNumerator := by
...
lemma card_sdiff_large_for_bchks {α : Type*} [DecidableEq α]
  ...
  let M : ℕ := 2 * 40826205 * 32414 * 312 ^ 2 + (bchksErrors + 1) * 312
...
lemma bchks_top_points_arithmetic (r T : ℕ)
    (hT : 2 * 40826205 * r + (bchksErrors + 1) < T) :
