...
/-!
The whole gcd receives its YS, R, and total caps from the C45/s13,
C45/s12, and B84/s26 kernels respectively. The A70 and auxiliary A63
...
theorem common_C_flag {u0 u1 : I → K} (S : SelectedPair u0 u1) :
    gcd S.QA S.QB ∈ globalCoefficientBox K 8188785 131071 100000 13 :=
...
theorem common_Thin_flag {u0 u1 : I → K} (S : SelectedPair u0 u1) :
    gcd S.QA S.QB ∈ globalCoefficientBox K 8188785 131071 100000 12 :=
...
theorem common_total_le {u0 u1 : I → K} (S : SelectedPair u0 u1) :
    wt residualTotalWeights (gcd S.QA S.QB) ≤ 1417 :=
...
theorem common_slope_le {u0 u1 : I → K} (S : SelectedPair u0 u1) :
    wt residualSWeights (gcd S.QA S.QB) ≤ 12 :=
  ((mem_flagGlobalCoefficientBox_iff (gcd S.QA S.QB)
    8188785 131071 100000 12 (by decide)).mp S.common_Thin_flag).2.1

theorem common_degreeR_le {u0 u1 : I → K} (S : SelectedPair u0 u1) :
    (gcd S.QA S.QB).degreeOf (2 : Fin 4) ≤ 12 := by
