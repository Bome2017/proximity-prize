...
theorem candidate : ProtocolClaimUpper 11613 122369 where
  ...
  unsafeAbove := by
    ...
    · exact ProximityPrize.SubmissionUpper.MultiAnchor.IRSProfile.winningSetDensity_gt_epsilonStar
        δ hδ.1 hbridge
    · have hlower : ProximityGap.gridPt (ι := IRSProfile.Index)
          ProximityPrize.SubmissionUpper.SubHalfPigeonhole.unsafeIndex ≤ δ := by
        simpa [ProximityPrize.SubmissionUpper.MultiAnchor.bridgeIndex,
          ProximityPrize.SubmissionUpper.SubHalfPigeonhole.unsafeIndex] using
          (le_of_not_gt hbridge)
