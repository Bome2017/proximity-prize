...
-/
import ProximityPrize.SubmissionUpper.FibrePencil
...
-- Exact spot-check comparison at `delta* = 122369/262144`:
-- `2^218787 ≤ 139775^12800` is `2^(-11613/100) ≤ (139775/262144)^128`
...
theorem candidate_score :
  ...
  rw [show (-(((11613 : ℕ) : ℝ))) * (1 / 100) =
      -(((11613 : Nat) : ℝ) / 100) by push_cast; ring] at hmono
  rw [show ((12800 : ℕ) : ℝ) * (1 / 100) = ((128 : ℕ) : ℝ) by
    push_cast; norm_num] at hmono
  rwa [NNReal.rpow_natCast] at hmono

/-- 512-fibre pencil on `[122369/262144, 61321/131072)`, then the promoted
prescribed-top family on the remaining suffix. -/
theorem candidate : ProtocolClaimUpper 11613 122369 where
  ...
  unsafeAbove := by
    ...
    by_cases hmid : δ < (61321 / 131072 : ℝ≥0)
    · exact ProximityPrize.SubmissionUpper.FibrePencil.winningSetDensity_gt_epsilon_window
        δ hband.1 hmid
