...
theorem two_pow_259300_lt_centralBinom_129680 :
    2 ^ 259300 < Nat.choose 259360 129680 := by
  have hcentral := Nat.four_pow_lt_mul_centralBinom 129680 (by norm_num)
  rw [Nat.centralBinom_eq_two_mul_choose] at hcentral
  norm_num only [Nat.reduceMul] at hcentral
  by_contra hnot
  have hle : Nat.choose 259360 129680 ≤ 2 ^ 259300 := Nat.le_of_not_gt hnot
  have hbad : 4 ^ 129680 < 129680 * 2 ^ 259300 :=
    lt_of_lt_of_le hcentral (Nat.mul_le_mul_left 129680 hle)
  have hreverse : 129680 * 2 ^ 259300 < 4 ^ 129680 := by
    calc
      129680 * 2 ^ 259300 < 2 ^ 17 * 2 ^ 259300 := by gcongr <;> norm_num
      _ = 2 ^ (17 + 259300) := (pow_add 2 17 259300).symm
      _ < 2 ^ (2 * 129680) := Nat.pow_lt_pow_right (by norm_num) (by norm_num)
      _ = (2 ^ 2) ^ 129680 := pow_mul 2 2 129680
      _ = 4 ^ 129680 := by norm_num
  exact (Nat.not_lt_of_ge hreverse.le) hbad

theorem two_pow_259300_lt_choose (c : Nat) (hc : c ≤ 1392) :
    2 ^ 259300 < Nat.choose 262144 (131072 + c) := by
  have hc' : c ≤ 131072 := by omega
  rw [← Nat.choose_symm (show 131072 + c ≤ 262144 by omega)]
  have hsub : 262144 - (131072 + c) = 131072 - c := by omega
  rw [hsub]
  have hmono : Nat.centralBinom 129680 ≤ Nat.centralBinom (131072 - c) :=
  ...
  exact two_pow_259300_lt_centralBinom_129680.trans_le
    (hmono.trans (Nat.choose_le_choose (131072 - c) hlargeN))

theorem field_pow_le_1392 (c : Nat) (hc : c ≤ 1392) :
    Fintype.card F ^ c ≤ qBound ^ 1392 := by
...
private theorem qBound_product_lt_two_pow_259300 :
    qBound ^ 1392 * qBound ^ 2 < 2 ^ 259300 := by
  calc
    qBound ^ 1392 * qBound ^ 2 = 2 ^ (186 * 1392 + 186 * 2) := by
      rw [qBound_eq, ← pow_mul, ← pow_mul, ← pow_add]
    _ < 2 ^ 259300 :=
      (Nat.pow_lt_pow_iff_right (by norm_num : 1 < (2 : Nat))).2 (by norm_num)

theorem signature_product_lt_two_pow_259300 (c : Nat) (hc : c ≤ 1392) :
    Fintype.card F ^ c * (Fintype.card F - 1) ^ 2 < 2 ^ 259300 := by
  have hqpos : 0 < qBound := by
    rw [qBound_eq]
    positivity
  exact (Nat.mul_lt_mul_of_le_of_lt (field_pow_le_1392 c hc)
    field_sub_one_sq_lt (Nat.pow_pos hqpos)).trans qBound_product_lt_two_pow_259300

theorem signature_space_mul_collision_lt_choose (c : Nat) (hc : c ≤ 1392) :
    Fintype.card (Fin c → F) * (Fintype.card F - 1) ^ 2 <
      Nat.choose 262144 (131072 + c) := by
  rw [Fintype.card_fun, Fintype.card_fin]
  exact (signature_product_lt_two_pow_259300 c hc).trans
    (two_pow_259300_lt_choose c hc)
...
theorem exists_large_signature_fiber (c : Nat) (hc : c ≤ 1392) :
    ∃ a : Fin c → F,
      (Fintype.card F - 1) ^ 2 <
...
`d + c` columns, for each `1 ≤ c ≤ 1392`. -/
theorem winningSetSoundness_eq_one_cell (c : Nat) (hcpos : 0 < c) (hc : c ≤ 1392)
    (δ : ℝ≥0)
...
theorem winningSetSoundness_eq_one_cell_interval
    (c : Nat) (hcpos : 0 < c) (hc : c ≤ 1392) (δ : ℝ≥0)
    (hδ : δ ∈ Set.Ico
...
index `129680`.  The interpolation sets have `d + 1392` columns, while the
zero-codeword uniqueness argument only needs `d` columns. -/
theorem winningSetSoundness_eq_one_suffix (δ : ℝ≥0)
    (hδ : δ ∈ Set.Ico
      (ProximityGap.gridPt (ι := I) 129680)
      Benchmark.IRSProfile.minRelativeDistance) :
    winningSetDensity Benchmark.IRSProfile.encoder δ = 1 := by
  classical
  obtain ⟨a, ha⟩ := exists_large_signature_fiber 1392 (by norm_num)
  let A : Finset (ColumnSets 1392) :=
    Finset.univ.filter fun T : ColumnSets 1392 => signature 1392 T = a
  have hAcard : (Fintype.card F - 1) ^ 2 < A.card := by
    simpa only [A] using ha
  have hApos : 0 < A.card := lt_of_le_of_lt (Nat.zero_le _) hAcard
  obtain ⟨T₀, hT₀⟩ := Finset.card_pos.mp hApos
  apply winningSetSoundness_eq_one_of_interpolation_family
    ...
    (m := d + 1392) (z := d) Benchmark.IRSProfile.encoder δ
  ...
  · have hlt : (δ : Real) < (131073 : Real) / 262144 := by
    ...
    linarith
  · have hgeGrid :
        ((ProximityGap.gridPt (ι := I) 129680 : ℝ≥0) : Real) ≤ (δ : Real) := by
      exact_mod_cast hδ.1
    have hge : (129680 : Real) / 262144 ≤ (δ : Real) := by
  ...
  · exact hAcard
  · intro T hT
    have hsigT : signature 1392 T = a := (Finset.mem_filter.mp hT).2
    have hsigT₀ : signature 1392 T₀ = a := (Finset.mem_filter.mp hT₀).2
