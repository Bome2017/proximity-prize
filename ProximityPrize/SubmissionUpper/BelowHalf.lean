...
theorem two_pow_3300_lt_centralBinom_2048 :
    2 ^ 3300 < Nat.choose 4096 2048 := by
  have hcentral := Nat.four_pow_lt_mul_centralBinom 2048 (by norm_num)
  rw [Nat.centralBinom_eq_two_mul_choose] at hcentral
  norm_num only [Nat.reduceMul] at hcentral
  by_contra hnot
  have hle : Nat.choose 4096 2048 ≤ 2 ^ 3300 := Nat.le_of_not_gt hnot
  have hbad : 4 ^ 2048 < 2048 * 2 ^ 3300 :=
    lt_of_lt_of_le hcentral (Nat.mul_le_mul_left 2048 hle)
  have hreverse : 2048 * 2 ^ 3300 < 4 ^ 2048 := by
    calc
      2048 * 2 ^ 3300 = 2 ^ 11 * 2 ^ 3300 := by norm_num
      _ = 2 ^ (11 + 3300) := (pow_add 2 11 3300).symm
      _ < 2 ^ (2 * 2048) := Nat.pow_lt_pow_right (by norm_num) (by norm_num)
      _ = (2 ^ 2) ^ 2048 := pow_mul 2 2 2048
      _ = 4 ^ 2048 := by norm_num
  exact (Nat.not_lt_of_ge hreverse.le) hbad

theorem two_pow_3300_lt_choose (c : Nat) (hc : c ≤ 15) :
    2 ^ 3300 < Nat.choose 262144 (131072 + c) := by
  have hc' : c ≤ 131072 := by omega
  rw [← Nat.choose_symm (show 131072 + c ≤ 262144 by omega)]
  have hsub : 262144 - (131072 + c) = 131072 - c := by omega
  rw [hsub]
  have hmono : Nat.centralBinom 2048 ≤ Nat.centralBinom (131072 - c) :=
  ...
  exact two_pow_3300_lt_centralBinom_2048.trans_le
    (hmono.trans (Nat.choose_le_choose (131072 - c) hlargeN))

theorem field_pow_le_fifteen (c : Nat) (hc : c ≤ 15) :
    Fintype.card F ^ c ≤ qBound ^ 15 := by
...
private theorem qBound_product_lt_two_pow_3300 :
    qBound ^ 15 * qBound ^ 2 < 2 ^ 3300 := by
  calc
    qBound ^ 15 * qBound ^ 2 = 2 ^ (186 * 15 + 186 * 2) := by
      rw [qBound_eq, ← pow_mul, ← pow_mul, ← pow_add]
    _ < 2 ^ 3300 :=
      (Nat.pow_lt_pow_iff_right (by norm_num : 1 < (2 : Nat))).2 (by norm_num)

theorem signature_product_lt_two_pow_3300 (c : Nat) (hc : c ≤ 15) :
    Fintype.card F ^ c * (Fintype.card F - 1) ^ 2 < 2 ^ 3300 := by
  have hqpos : 0 < qBound := by
    rw [qBound_eq]
    positivity
  exact (Nat.mul_lt_mul_of_le_of_lt (field_pow_le_fifteen c hc)
    field_sub_one_sq_lt (Nat.pow_pos hqpos)).trans qBound_product_lt_two_pow_3300

theorem signature_space_mul_collision_lt_choose (c : Nat) (hc : c ≤ 15) :
    Fintype.card (Fin c → F) * (Fintype.card F - 1) ^ 2 <
      Nat.choose 262144 (131072 + c) := by
  rw [Fintype.card_fun, Fintype.card_fin]
  exact (signature_product_lt_two_pow_3300 c hc).trans
    (two_pow_3300_lt_choose c hc)
...
theorem winningSetSoundness_eq_one_of_interpolation_family
    ...
    (hzero : ∀ (u : Fin messageLength → K) (S : Finset ι), m ≤ S.card →
      (∀ i ∈ S, enc u i = 0) → u = 0) :
...
theorem exists_large_signature_fiber (c : Nat) (hc : c ≤ 15) :
    ∃ a : Fin c → F,
      (Fintype.card F - 1) ^ 2 <
...
`d + c` columns, for each `1 ≤ c ≤ 15`. -/
theorem winningSetSoundness_eq_one_cell (c : Nat) (hcpos : 0 < c) (hc : c ≤ 15)
    (δ : ℝ≥0)
