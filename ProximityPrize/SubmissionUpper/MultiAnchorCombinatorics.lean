...
theorem zeta_primitive : IsPrimitiveRoot zeta quotientSize := by
  ...
  rw [hlog] at h
  change IsPrimitiveRoot (D.omega ^ 512) 512
  norm_num at h ⊢
  exact h
...
theorem locatorNode_injective : Function.Injective locatorNode := by
  ...
  have hi : i.1 + 1 < quotientSize := by
    have := i.isLt
    norm_num [quotientSize] at this ⊢
    omega
  have hj : j.1 + 1 < quotientSize := by
    have := j.isLt
    norm_num [quotientSize] at this ⊢
    omega
  have heq := zeta_primitive.pow_inj hi hj hij
  omega
...
theorem anchorNode_injective : Function.Injective anchorNode := by
  ...
  apply Fin.ext
  have hlog : D.logN = 18 := rfl
  have hi : i.1 + 1 < 2 ^ D.logN := by
    rw [hlog]
    have hi0 : i.1 < 14 := i.isLt
    change i.1 + 1 < 262144
    omega
  have hj : j.1 + 1 < 2 ^ D.logN := by
    rw [hlog]
    have hj0 : j.1 < 14 := j.isLt
    change j.1 + 1 < 262144
    omega
  have heq := D.primitive.pow_inj hi hj hij
  omega
...
theorem anchorNode_ne_quotient (i : Fin anchorCount) (r : Fin quotientSize) :
  ...
  have hexp : i.1 + 1 = fiberSize * r.1 :=
    D.primitive.pow_inj
      (by
        ...
        omega)
      (by
        change fiberSize * r.1 < 2 ^ 18
        have hr0 : r.1 < 512 := r.isLt
        change 512 * r.1 < 262144
        omega)
      heq
  ...
  · simp [hr] at hexp
  · have hlarge : fiberSize ≤ fiberSize * r.1 := Nat.le_mul_of_pos_right _ hr
    rw [← hexp] at hlarge
    exact (Nat.not_le_of_gt hsmall) hlarge
...
noncomputable def rootPolyK (T : CandidateSets) : Polynomial K :=
  ∏ i ∈ (T.1 : Finset (Fin 511)),
    ((Polynomial.X : Polynomial K) - Polynomial.C (locatorNode i))

theorem rootPolyK_monic (T : CandidateSets) : (rootPolyK T).Monic := by
  simpa only [rootPolyK] using
    Polynomial.monic_prod_X_sub_C (fun i : Fin 511 => locatorNode i)
      (T.1 : Finset (Fin 511))
...
private theorem locator_prod_eq_zeta_sum (T : CandidateSets) :
  ...
  exact Finset.prod_pow_eq_pow_sum (T.1 : Finset (Fin 511))
    (fun i => i.1 + 1) zeta
...
theorem rootPolyK_eval_zero_eq_of_exponentSignature_eq (A B : CandidateSets)
  ...
  rw [show (∏ i ∈ (A.1 : Finset (Fin 511)), -locatorNode i) =
      ...
      simpa [A.prop] using
        (Finset.prod_neg (s := (A.1 : Finset (Fin 511))) locatorNode),
    show (∏ i ∈ (B.1 : Finset (Fin 511)), -locatorNode i) =
      (-1 : K) ^ locatorSize *
        ∏ i ∈ (B.1 : Finset (Fin 511)), locatorNode i by
      simpa [B.prop] using
        (Finset.prod_neg (s := (B.1 : Finset (Fin 511))) locatorNode),
  ...
  have hneB : (rootPoly B).eval (emb (locatorNode i)) ≠ 0 := by
    have hbase : (rootPolyK B).eval (locatorNode i) ≠ 0 := by
      ...
      exact fun heq => hiB (by
        ...
        exact Set.powersetCard.mem_coe_iff.mp (by simpa [hij] using hj))
    intro hzero
...
noncomputable def W0 : Polynomial F :=
  ∏ a ∈ fixedPoints, ((Polynomial.X : Polynomial F) - Polynomial.C a)

theorem W0_monic : W0.Monic := by
  simpa only [W0] using
    Polynomial.monic_prod_X_sub_C (fun a : F => a) fixedPoints
