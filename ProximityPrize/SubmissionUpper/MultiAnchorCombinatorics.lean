...
theorem zeta_primitive : IsPrimitiveRoot zeta quotientSize := by
  have hdiv : fiberSize ∣ 2 ^ D.logN := by
    norm_num [fiberSize, D, Benchmark.IRSProfile.baseNttDomain]
  have h := D.primitive.pow_of_dvd (p := fiberSize)
    (by norm_num [fiberSize]) hdiv
  convert h using 1 <;>
    norm_num [zeta, quotientSize, fiberSize, D, Benchmark.IRSProfile.baseNttDomain]
...
theorem locatorNode_injective : Function.Injective locatorNode := by
  intro i j hij
  apply Fin.ext
  apply Nat.add_right_cancel (b := 1)
  exact zeta_primitive.pow_inj (by omega) (by omega) hij
...
theorem anchorNode_injective : Function.Injective anchorNode := by
  ...
  apply Fin.ext
  apply Nat.add_right_cancel (b := 1)
  exact D.primitive.pow_inj (by norm_num [D, Benchmark.IRSProfile.baseNttDomain]; omega)
    (by norm_num [D, Benchmark.IRSProfile.baseNttDomain]; omega) hij
...
theorem anchorNode_ne_quotient (i : Fin anchorCount) (r : Fin quotientSize) :
  ...
  have hexp : i.1 + 1 = fiberSize * r.1 :=
    D.primitive.pow_inj
      (by norm_num [D, Benchmark.IRSProfile.baseNttDomain, anchorCount] at i; omega)
      (by norm_num [D, Benchmark.IRSProfile.baseNttDomain, fiberSize, quotientSize] at r; omega)
      heq
  have hi : 0 < i.1 + 1 := by omega
  have hsmall : i.1 + 1 < fiberSize := by
    norm_num [anchorCount, fiberSize] at i ⊢
    omega
...
noncomputable def rootPolyK (T : CandidateSets) : Polynomial K :=
  (T.1 : Finset (Fin 511)).prod fun i => X - C (locatorNode i)
...
theorem fixedPoints_card : fixedPoints.card = 15 := by
  ...
  norm_num [anchorCount]

noncomputable def W0 : Polynomial F :=
  fixedPoints.prod fun a => X - C a
