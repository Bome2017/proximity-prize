...
theorem zeta_primitive : IsPrimitiveRoot zeta quotientSize := by
  have hdiv : fiberSize ∣ 2 ^ D.logN := by
    show (512 : ℕ) ∣ 2 ^ 18
    norm_num
  have h := D.primitive.pow_of_dvd (p := fiberSize) (by norm_num [fiberSize]) hdiv
  have hquot : 2 ^ D.logN / fiberSize = quotientSize := by
    show 2 ^ 18 / 512 = 512
    norm_num
  rw [hquot] at h
  exact h
...
theorem locatorNode_injective : Function.Injective locatorNode := by
  intro i j hij
  have hij' : zeta ^ (i.1 + 1) = zeta ^ (j.1 + 1) := hij
  have hi : i.1 + 1 < quotientSize := by
    have h : i.1 < 511 := i.isLt
    show i.1 + 1 < 512; omega
  have hj : j.1 + 1 < quotientSize := by
    have h : j.1 < 511 := j.isLt
    show j.1 + 1 < 512; omega
  have hsucc := zeta_primitive.pow_inj hi hj hij'
...
theorem anchorNode_injective : Function.Injective anchorNode := by
  intro i j hij
  have hij' : omega ^ (i.1 + 1) = omega ^ (j.1 + 1) := hij
  have hi : i.1 + 1 < 2 ^ 18 := by
    have h : i.1 < 14 := i.isLt
    omega
  have hj : j.1 + 1 < 2 ^ 18 := by
    have h : j.1 < 14 := j.isLt
    omega
  have hsucc := D.primitive.pow_inj hi hj hij'
  exact Fin.ext (by omega)
...
theorem anchorNode_ne_quotient (i : Fin anchorCount) (r : Fin quotientSize) :
  ...
  intro h
  have hi14 : i.1 < 14 := i.isLt
  have hr512 : r.1 < 512 := r.isLt
  have heq : omega ^ (i.1 + 1) = omega ^ (fiberSize * r.1) :=
    h.trans (pow_mul omega fiberSize r.1).symm
  have hlt1 : i.1 + 1 < 2 ^ 18 := by omega
  have hlt2 : fiberSize * r.1 < 2 ^ 18 := by
    show 512 * r.1 < 2 ^ 18; omega
  have hexp : i.1 + 1 = fiberSize * r.1 := D.primitive.pow_inj hlt1 hlt2 heq
  have hexp' : i.1 + 1 = 512 * r.1 := hexp
  omega
...
noncomputable def rootPolyK (T : CandidateSets) : Polynomial K :=
  (T.1 : Finset (Fin 511)).prod fun i => Polynomial.X - Polynomial.C (locatorNode i)
...
private theorem locator_prod_eq_zeta_sum (T : CandidateSets) :
  ...
  exact Finset.prod_pow_eq_pow_sum (T.1 : Finset (Fin 511)) (fun i => i.1 + 1) zeta
...
theorem rootPoly_injective : Function.Injective rootPoly := by
  ...
  have hneB : (rootPoly B).eval (emb (locatorNode i)) ≠ 0 := by
    have hiB' : i ∉ (B.1 : Finset (Fin 511)) := hiB
    have hbase : (rootPolyK B).eval (locatorNode i) ≠ 0 := by
      rw [rootPolyK, Polynomial.eval_prod, Finset.prod_ne_zero_iff]
      intro j hj
      simp only [Polynomial.eval_sub, Polynomial.eval_X, Polynomial.eval_C, sub_ne_zero]
      intro heq
      have hij : i = j := locatorNode_injective heq
      rw [← hij] at hj
      exact hiB' hj
    intro hzero
  ...
  calc
    ...
    _ = familySize * (familySize - 1) * 271 := by
      rw [Finset.sum_const, nsmul_eq_mul, Finset.offDiag_card, chosenSets_card,
        Nat.mul_sub_left_distrib, mul_one, Nat.cast_id]
