...
With `r = 8396` this certifies the unsafe suffix from `delta* = 15/32` onward.
-/
...
/-- Agreement-set size: `m + r = 131072 + 8396`. -/
abbrev tt : ℕ := 139468
...
theorem card_keys : Fintype.card (Fin rr → K) = (2 ^ 31 - 2 ^ 24 + 1) ^ 8396 := by
  rw [Fintype.card_fun, card_K, Fintype.card_fin]

theorem card_powerset :
    (Finset.powersetCard tt (Finset.univ : Finset Idx)).card = Nat.choose 262144 139468 := by
...
theorem prod_up : (∏ i ∈ Finset.range 8396, (131072 + 1 + i)) ≤ 139468 ^ 8396 := by
  have hconst : (∏ _i ∈ Finset.range 8396, (139468 : ℕ)) = 139468 ^ 8396 := by
    rw [Finset.prod_const, Finset.card_range]
  rw [← hconst]
  exact Finset.prod_le_prod' (fun i hi => by have := Finset.mem_range.mp hi; omega)

theorem prod_lo : (122677 : ℕ) ^ 8396 ≤ ∏ i ∈ Finset.range 8396, (262144 - 131072 - i) := by
  have hconst : (∏ _i ∈ Finset.range 8396, (122677 : ℕ)) = 122677 ^ 8396 := by
    rw [Finset.prod_const, Finset.card_range]
  rw [← hconst]
  exact Finset.prod_le_prod' (fun i hi => by have := Finset.mem_range.mp hi; omega)
...
theorem numeric_fact :
    131072 * ((2 ^ 31 - 2 ^ 24 + 1) ^ 8396 * (((2 ^ 31 - 2 ^ 24 + 1) ^ 6 - 1) ^ 2))
        * 139468 ^ 8396
      ≤ 4 ^ 131072 * 122677 ^ 8396 := by decide

theorem key_counting :
    (2 ^ 31 - 2 ^ 24 + 1) ^ 8396 * (((2 ^ 31 - 2 ^ 24 + 1) ^ 6 - 1) ^ 2)
      < Nat.choose 262144 139468 := by
  by_contra hcon
  push_neg at hcon
  have hshift : Nat.choose 262144 139468 * (∏ i ∈ Finset.range 8396, (131072 + 1 + i))
      = Nat.choose 262144 131072 * (∏ i ∈ Finset.range 8396, (262144 - 131072 - i)) := by
    have h := choose_shift 262144 131072 8396
    rwa [show (131072 + 8396 : ℕ) = 139468 by norm_num] at h
  have hcb : (4 : ℕ) ^ 131072 < 131072 * Nat.choose 262144 131072 := by
    ...
    exact h
  have hkey : Nat.choose 262144 131072 * 122677 ^ 8396
      ≤ Nat.choose 262144 139468 * 139468 ^ 8396 := by
    calc Nat.choose 262144 131072 * 122677 ^ 8396
        ≤ Nat.choose 262144 131072 * (∏ i ∈ Finset.range 8396, (262144 - 131072 - i)) :=
          Nat.mul_le_mul (le_refl _) prod_lo
      _ = Nat.choose 262144 139468 * (∏ i ∈ Finset.range 8396, (131072 + 1 + i)) := hshift.symm
      _ ≤ Nat.choose 262144 139468 * 139468 ^ 8396 := Nat.mul_le_mul (le_refl _) prod_up
  exact abstract_contradiction (by positivity) hcb hkey hcon numeric_fact
...
theorem coeff_eq_of_key {J J' : Finset Idx} (hkey : key J = key J')
    {d : ℕ} (hd1 : 131072 ≤ d) (hd2 : d ≤ 139467) :
    (g J).coeff d = (g J').coeff d := by
  have hi : 139467 - d < 8396 := by omega
  have h := congrFun hkey ⟨139467 - d, hi⟩
  simp only [key] at h
  rwa [show (139468 : ℕ) - 1 - (139467 - d) = d by omega] at h
...
theorem sub_degree_lt {J J' : Finset Idx}
  ...
  rcases lt_trichotomy d 139468 with hlt | heq | hgt
  · have hle : d ≤ 139467 := by omega
...
theorem word_far (u : Fin kk → FF) (S : Finset Idx) (hcard : tt < S.card)
  ...
  classical
  have hSbig : 139468 < S.card := hcard
  ...
  have heq : gF J0 = q :=
    Polynomial.eq_of_degrees_lt_of_eval_index_eq S IRSProfile.domain.injective.injOn
      hPdegS hqdegS heval
  rw [heq] at hnd
  have hnd' : q.natDegree = 139468 := hnd
  have hq0 : q ≠ 0 := by
    intro h0
    rw [h0] at hnd'
    simp at hnd'
...
theorem winningSetDensity_eq_one (δ : ℝ≥0)
  ...
  have hd_lo : (30669 / 65536 : ℝ) ≤ (δ : ℝ) := by exact_mod_cast hlo
  ...
  have hx_hi : (1 - (δ : ℝ)) * 262144 ≤ 139468 := by nlinarith
  ...
  have ht'_lo : 131072 ≤ Nat.ceil ((1 - (δ : ℝ)) * 262144) := by
    ...
    omega
  have ht'_hi : Nat.ceil ((1 - (δ : ℝ)) * 262144) ≤ 139468 :=
