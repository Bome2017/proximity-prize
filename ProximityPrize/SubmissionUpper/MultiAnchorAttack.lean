...
lemma finset_prod_X_sub_C_dvd_of_eval_eq_zero
  ...
  by_cases hp : p = 0
  · simp [hp]
  rw [show (S.prod fun a => Polynomial.X - Polynomial.C a) =
      (Multiset.map (fun a => Polynomial.X - Polynomial.C a) S.1).prod from rfl,
    Multiset.prod_X_sub_C_dvd_iff_le_roots hp S.1]
...
noncomputable def remainderPoly (T : CandidateSets) : Polynomial F :=
  (quotientPoly T - Polynomial.C (challenge T)) /ₘ (Polynomial.X - Polynomial.C lambda)

theorem X_sub_lambda_mul_remainderPoly (T : CandidateSets) :
    (Polynomial.X - Polynomial.C lambda) * remainderPoly T = quotientPoly T - Polynomial.C (challenge T) := by
  rw [remainderPoly, Polynomial.mul_divByMonic_eq_iff_isRoot]
  simp [Polynomial.IsRoot.def, challenge]

noncomputable def W : Polynomial F := W0 * (Polynomial.X - Polynomial.C lambda)
...
theorem W0_eval_ne_zero_of_mem_quotientNodesF {y : F} (hy : y ∈ quotientNodesF) :
    W0.eval y ≠ 0 := by
  rw [W0, Polynomial.eval_prod, Finset.prod_ne_zero_iff]
  intro a ha
...
def blockIndex (q r : Fin 512) : I :=
  ⟨r.1 + 512 * q.1, by
    have hq := q.isLt; have hr := r.isLt
    show r.1 + 512 * q.1 < 2 ^ 18
    omega⟩

@[simp] theorem blockIndex_val (q r : Fin 512) :
    (blockIndex q r).1 = r.1 + 512 * q.1 := rfl

theorem domain_blockIndex_pow (q r : Fin 512) :
    Benchmark.IRSProfile.domain (blockIndex q r) ^ fiberSize = emb (zeta ^ r.1) := by
  have hnode : Benchmark.IRSProfile.domain (blockIndex q r) = emb (omega ^ (r.1 + 512 * q.1)) := by
    show emb (D.node (blockIndex q r)) = emb (omega ^ (r.1 + 512 * q.1))
    rfl
  rw [hnode, ← map_pow]
  congr 1
  calc
    ...
    _ = omega ^ (512 * r.1) * (omega ^ 262144) ^ q.1 := by
      rw [pow_add, pow_mul omega 262144 q.1]
    _ = omega ^ (512 * r.1) := by
      have hroot : omega ^ 262144 = 1 := D.primitive.pow_eq_one
      rw [hroot, one_pow, mul_one]
    _ = (omega ^ 512) ^ r.1 := by rw [pow_mul omega 512 r.1]

theorem domain_pow_mem_quotientNodesF (j : I) :
    Benchmark.IRSProfile.domain j ^ fiberSize ∈ quotientNodesF := by
  have hj : j = blockIndex ⟨j.1 / 512, by have h : j.1 < 262144 := j.isLt; omega⟩
      ⟨j.1 % 512, by omega⟩ := by
    apply Fin.ext
    show j.1 = j.1 % 512 + 512 * (j.1 / 512)
    exact (Nat.mod_add_div j.1 512).symm
  rw [hj, domain_blockIndex_pow, quotientNodesF, Finset.mem_image]
  exact ⟨⟨j.1 % 512, by omega⟩, Finset.mem_univ _, rfl⟩
...
theorem W_eval_domain_pow_ne_zero (j : I) :
  ...
  have hlam : Benchmark.IRSProfile.domain j ^ fiberSize ≠ lambda := by
    intro h
  ...
  exact ⟨h0, sub_ne_zero.mpr hlam⟩

def padIndex (q : Fin 511) : I :=
  blockIndex ⟨q.1, by omega⟩ 0

theorem padIndex_val (q : Fin 511) : (padIndex q).1 = 512 * q.1 := by
  simp [padIndex, blockIndex_val]

theorem padIndex_injective : Function.Injective padIndex := by
  intro a b h
  have hv := congrArg Fin.val h
  rw [padIndex_val, padIndex_val] at hv
...
theorem rootIndexPair_val (iq : Fin 511 × Fin 512) :
    (rootIndexPair iq).1 = (iq.1.1 + 1) + 512 * iq.2.1 := by
  simp [rootIndexPair, blockIndex_val]

theorem rootIndexPair_injective : Function.Injective rootIndexPair := by
  intro a b h
  have hv := congrArg Fin.val h
  rw [rootIndexPair_val, rootIndexPair_val] at hv
  have ha1 := a.1.isLt; have hb1 := b.1.isLt
  refine Prod.ext (Fin.ext ?_) (Fin.ext ?_) <;> omega

noncomputable def padIndices : Finset I :=
  Finset.univ.image padIndex

noncomputable def rootIndices (T : CandidateSets) : Finset I :=
  ((T.1 : Finset (Fin 511)) ×ˢ (Finset.univ : Finset (Fin 512))).image rootIndexPair
...
theorem padIndices_disjoint_rootIndices (T : CandidateSets) :
  ...
  rw [rootIndexPair_val, padIndex_val] at hv
  have := ir.1.isLt
  omega
...
theorem rootPoly_eval_zero_of_mem_rootIndices (T : CandidateSets) (j : I)
  ...
  rw [show rootIndexPair ir = blockIndex ir.2 ⟨ir.1.1 + 1, by omega⟩ from rfl,
  ...
  have hbase : (rootPolyK T).eval (locatorNode ir.1) = 0 := by
    ...
    · exact hi
    · simp
  show (rootPoly T).eval (emb (locatorNode ir.1)) = 0
  rw [show (rootPoly T).eval (emb (locatorNode ir.1)) =
      emb ((rootPolyK T).eval (locatorNode ir.1)) from by simp [rootPoly], hbase, map_zero]

noncomputable def codePoly (T : CandidateSets) : Polynomial F :=
  padPoly * (remainderPoly T).comp (Polynomial.X ^ fiberSize)

theorem codePoly_natDegree_le (T : CandidateSets) : (codePoly T).natDegree ≤ 131071 := by
  calc
    (codePoly T).natDegree ≤ padPoly.natDegree +
        ((remainderPoly T).comp (Polynomial.X ^ fiberSize)).natDegree := Polynomial.natDegree_mul_le
    _ ≤ 511 + (remainderPoly T).natDegree * (Polynomial.X ^ fiberSize : F[X]).natDegree := by
...
theorem scalar_fold_eq_codePoly_of_mem (T : ChosenSets) (j : I)
  ...
  · have hA := padPoly_eval_zero_of_mem_padIndices j hjpad
    simp [scalarWord0, scalarWord1, codePoly, Polynomial.eval_mul,
      Polynomial.eval_comp, Polynomial.eval_pow, Polynomial.eval_X, hA]
...
noncomputable def denomPoly : Polynomial F := Polynomial.X ^ fiberSize - Polynomial.C lambda
...
theorem attackStack_not_joint (δ : ℝ≥0)
  ...
  have hTlarge : 131583 < T.card := by
    ...
    have hδone : δ ≤ 1 := by
      exact NNReal.coe_le_coe.mp
        (hδR.trans (by norm_num : (123141 : ℝ) / 262144 < 1)).le
  ...
  have hQdeg : Q.natDegree ≤ 131583 := by
    calc
      ...
      _ ≤ 131583 := by
        rw [padPoly_natDegree]
        apply max_le
        · exact (Polynomial.natDegree_mul_le (p := q) (q := denomPoly)).trans (by
  ...
  have hzero : T ⊆ Finset.univ.filter fun j : I => Q.eval (Benchmark.IRSProfile.domain j) = 0 := by
    ...
    show (q * denomPoly + padPoly).eval (Benchmark.IRSProfile.domain j) = 0
    rw [Polynomial.eval_add, Polynomial.eval_mul, hqeval, scalarWord1_eq]
    field_simp [denom_eval_domain_ne_zero]
...
theorem goodChallenges_card : goodChallenges.card = familySize := by
  have hinj : Set.InjOn challenge chosenSets := by
    intro A hA B hB hAB
    exact congrArg Subtype.val
      (@challenge_injective ⟨A, hA⟩ ⟨B, hB⟩ hAB)
...
theorem epsilonStar_lt_family_div_field :
  ...
  have hb : (Fintype.card F : ℝ≥0) ≠ 0 := Nat.cast_ne_zero.mpr Fintype.card_ne_zero
  rw [← ENNReal.coe_natCast familySize, ← ENNReal.coe_natCast (Fintype.card F),
    ← ENNReal.coe_div hb, ENNReal.coe_lt_coe]
  rw [ProximityPrize.Benchmark.Upper.epsilonStar, ProximityGap.prizeThreshold, hcardF]
  push_cast
  norm_num [_root_.KoalaBear.fieldSize, div_lt_div_iff₀]
...
theorem winningSetDensity_gt_epsilonStar (δ : ℝ≥0)
  ...
  have hbridge := epsCa_le_winningSetDensity
