...
theorem domain_pow_mem_quotientNodesF (j : I) :
  ...
  let qr : Fin 512 × Fin 512 := finProdFinEquiv.symm j
...
theorem W_eval_domain_pow_ne_zero (j : I) :
  ...
  have hλ : Benchmark.IRSProfile.domain j ^ fiberSize ≠ lambda := by
    intro h
...
theorem padIndex_injective : Function.Injective padIndex := by
  ...
  change finProdFinEquiv
    ...
    finProdFinEquiv ((⟨b.1, by omega⟩ : Fin 512), (0 : Fin 512)) at h
...
theorem rootIndexPair_injective : Function.Injective rootIndexPair := by
  intro a b h
  change finProdFinEquiv (a.2, (⟨a.1.1 + 1, by omega⟩ : Fin 512)) =
    finProdFinEquiv (b.2, (⟨b.1.1 + 1, by omega⟩ : Fin 512)) at h
  have hp := finProdFinEquiv.injective h
  apply Prod.ext
  · apply Fin.ext
...
noncomputable def codePoly (T : CandidateSets) : Polynomial F :=
  padPoly * (remainderPoly T).comp (X ^ fiberSize)

theorem codePoly_natDegree_le (T : CandidateSets) : (codePoly T).natDegree ≤ 131071 := by
  calc
    (codePoly T).natDegree ≤ padPoly.natDegree +
        ((remainderPoly T).comp (X ^ fiberSize)).natDegree := Polynomial.natDegree_mul_le
    _ ≤ 511 + (remainderPoly T).natDegree * (X ^ fiberSize : F[X]).natDegree := by
...
noncomputable def denomPoly : Polynomial F := X ^ fiberSize - C lambda
...
theorem goodChallenges_card : goodChallenges.card = familySize := by
  have hinj : Set.InjOn challenge chosenSets := by
    intro A hA B hB hAB
    exact congrArg Subtype.val
      (challenge_injective (⟨A, hA⟩ : ChosenSets) (⟨B, hB⟩ : ChosenSets) hAB)
...
theorem epsilonStar_lt_family_div_field :
  ...
  rw [ProximityPrize.Benchmark.Upper.epsilonStar, ProximityGap.prizeThreshold,
    hcardF]
  norm_num [familySize, _root_.KoalaBear.fieldSize, ENNReal.div_eq_inv_mul]
...
theorem winningSetDensity_gt_epsilonStar (δ : ℝ≥0)
  ...
  have hbridge := ProximityGap.epsCa_le_winningSetDensity
