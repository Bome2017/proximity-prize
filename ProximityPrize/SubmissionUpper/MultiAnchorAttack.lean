...
lemma finset_prod_X_sub_C_dvd_of_eval_eq_zero
    ...
    (∏ a ∈ S, ((Polynomial.X : Polynomial F) - Polynomial.C a)) ∣ p := by
  classical
  by_cases hp : p = 0
  · simp [hp]
  change (S.1.map (fun a => (Polynomial.X : Polynomial F) - Polynomial.C a)).prod ∣ p
...
noncomputable def remainderPoly (T : CandidateSets) : Polynomial F :=
  (quotientPoly T - Polynomial.C (challenge T)) /ₘ
    ((Polynomial.X : Polynomial F) - Polynomial.C lambda)

theorem X_sub_lambda_mul_remainderPoly (T : CandidateSets) :
    ((Polynomial.X : Polynomial F) - Polynomial.C lambda) * remainderPoly T =
...
noncomputable def W : Polynomial F :=
  W0 * ((Polynomial.X : Polynomial F) - Polynomial.C lambda)

theorem W_monic : W.Monic := by
  rw [W]
  exact W0_monic.mul (Polynomial.monic_X_sub_C lambda)
...
theorem locator_pencil (T : ChosenSets) :
    rootPoly T.1 = rootPoly baseSet.1 + C (challenge T.1) * W0 +
      W * remainderPoly T.1 := by
  have hD := W0_mul_quotientPoly T
  have hV := X_sub_lambda_mul_remainderPoly T.1
  rw [diffPoly] at hD
  calc
    rootPoly T.1 = rootPoly baseSet.1 + W0 * quotientPoly T.1 := by
      rw [hD]
      ring
    _ = rootPoly baseSet.1 + C (challenge T.1) * W0 +
        W0 * (((Polynomial.X : Polynomial F) - C lambda) * remainderPoly T.1) := by
      rw [hV]
      ring
    _ = rootPoly baseSet.1 + C (challenge T.1) * W0 + W * remainderPoly T.1 := by
      rw [W]
      ring
...
theorem challenge_injective :
  ...
  change challenge A.1 = challenge B.1 at hAB
  calc
    (rootPoly A.1).eval lambda =
        (rootPoly baseSet.1).eval lambda + challenge A.1 * W0.eval lambda := hPA
    _ = (rootPoly baseSet.1).eval lambda + challenge B.1 * W0.eval lambda := by
      rw [hAB]
    _ = (rootPoly B.1).eval lambda := hPB.symm
...
def blockIndex (q r : Fin 512) : I :=
  (finProdFinEquiv : Fin 512 × Fin 512 ≃ I) (q, r)
...
theorem domain_blockIndex_pow (q r : Fin 512) :
  ...
  calc
    (omega ^ (r.1 + 512 * q.1)) ^ 512 =
      ...
      congr 1
      omega
    _ = omega ^ (512 * r.1) * omega ^ (262144 * q.1) := by rw [pow_add]
    _ = (omega ^ 512) ^ r.1 * (omega ^ 262144) ^ q.1 := by
      simp only [pow_mul]
    _ = omega ^ (512 * r.1) := by
      have hroot : omega ^ 262144 = 1 := by
        have hz := zeta_primitive.pow_eq_one
        change (omega ^ 512) ^ 512 = 1 at hz
        rw [← pow_mul] at hz
        norm_num at hz ⊢
        exact hz
      rw [hroot, one_pow, mul_one, pow_mul]
...
theorem domain_pow_mem_quotientNodesF (j : I) :
  ...
  let qr : Fin 512 × Fin 512 :=
    (finProdFinEquiv : Fin 512 × Fin 512 ≃ I).symm j
  have hj : j = blockIndex qr.1 qr.2 := by
    change j = (finProdFinEquiv : Fin 512 × Fin 512 ≃ I)
      ((finProdFinEquiv : Fin 512 × Fin 512 ≃ I).symm j)
    exact ((finProdFinEquiv : Fin 512 × Fin 512 ≃ I).apply_symm_apply j).symm
...
theorem padIndex_injective : Function.Injective padIndex := by
  intro a b h
  simp only [padIndex, blockIndex] at h
  have hp := (finProdFinEquiv : Fin 512 × Fin 512 ≃ I).injective h
...
attribute [irreducible] padNodes

noncomputable def padPoly : Polynomial F :=
  ∏ a ∈ padNodes, ((Polynomial.X : Polynomial F) - Polynomial.C a)
...
theorem rootIndexPair_injective : Function.Injective rootIndexPair := by
  intro a b h
  simp only [rootIndexPair, blockIndex] at h
  have hp := (finProdFinEquiv : Fin 512 × Fin 512 ≃ I).injective h
  apply Prod.ext
  · apply Fin.ext
    have hs := congrArg Prod.snd hp
    have hsval := congrArg Fin.val hs
    change a.1.1 + 1 = b.1.1 + 1 at hsval
    omega
...
theorem padIndices_disjoint_rootIndices (T : CandidateSets) :
  ...
  simp only [rootIndexPair, padIndex, blockIndex] at heq
  have hp := (finProdFinEquiv : Fin 512 × Fin 512 ≃ I).injective heq
  have hs := congrArg Prod.snd hp
  have hsval := congrArg Fin.val hs
  change ir.1.1 + 1 = 0 at hsval
  omega
...
theorem rootPoly_eval_zero_of_mem_rootIndices (T : CandidateSets) (j : I)
  ...
  rw [rootIndexPair, domain_blockIndex_pow]
  change (rootPoly T).eval (emb (locatorNode ir.1)) = 0
  have hbase : (rootPolyK T).eval (locatorNode ir.1) = 0 := by
    ...
    · simp
  calc
    (rootPoly T).eval (emb (locatorNode ir.1)) =
        emb ((rootPolyK T).eval (locatorNode ir.1)) := by
      simp only [rootPoly, Polynomial.eval_map, Polynomial.eval₂_hom]
    _ = 0 := by rw [hbase, map_zero]

noncomputable def fiberMonomial : Polynomial F :=
  (Polynomial.X : Polynomial F) ^ fiberSize

theorem fiberMonomial_natDegree : fiberMonomial.natDegree = fiberSize := by
  rw [fiberMonomial, Polynomial.natDegree_X_pow]

@[simp] theorem fiberMonomial_eval (x : F) : fiberMonomial.eval x = x ^ fiberSize := by
  simp [fiberMonomial]

private structure CodePolyCertificate (T : CandidateSets) where
  poly : Polynomial F
  eq_poly : poly = padPoly * (remainderPoly T).comp fiberMonomial
  natDegree_le : poly.natDegree ≤ 131071

noncomputable def codePolyCertificate (T : CandidateSets) : CodePolyCertificate T :=
  ⟨padPoly * (remainderPoly T).comp fiberMonomial, rfl, by
    calc
      (padPoly * (remainderPoly T).comp fiberMonomial).natDegree ≤ padPoly.natDegree +
          ((remainderPoly T).comp fiberMonomial).natDegree :=
        Polynomial.natDegree_mul_le (p := padPoly)
          (q := (remainderPoly T).comp fiberMonomial)
      _ ≤ 511 + (remainderPoly T).natDegree * fiberMonomial.natDegree := by
        rw [padPoly_natDegree]
        gcongr
        exact Polynomial.natDegree_comp_le
      _ ≤ 511 + 255 * 512 := by
        rw [fiberMonomial_natDegree]
        gcongr
        exact remainderPoly_natDegree_le T
      _ = 131071 := by norm_num⟩

attribute [irreducible] codePolyCertificate

local notation "codePoly" => fun T => CodePolyCertificate.poly (codePolyCertificate T)

theorem codePoly_eq (T : CandidateSets) :
    codePoly T = padPoly * (remainderPoly T).comp fiberMonomial := by
  exact (codePolyCertificate T).eq_poly

theorem codePoly_natDegree_le (T : CandidateSets) : (codePoly T).natDegree ≤ 131071 := by
  exact (codePolyCertificate T).natDegree_le
...
theorem exists_message (T : CandidateSets) : ∃ msg : Fin k → F,
    ∀ (j : I) (row : Fin s), Benchmark.IRSProfile.encoder msg j row =
      if row = 0 then (codePoly T).eval (Benchmark.IRSProfile.domain j) else 0 := by
  have hdeg : (codePoly T).degree < (k / s : Nat) := by
    simpa only [rowDimension_eq] using codePoly_degree_lt T
  simpa only using
    ProximityPrize.SubmissionUpper.PrescribedTop.exists_message (codePoly T) hdeg

noncomputable def message (T : CandidateSets) : Fin k → F :=
  Classical.choose (exists_message T)

theorem encoder_message_apply (T : CandidateSets) (j : I) (row : Fin s) :
    Benchmark.IRSProfile.encoder (message T) j row =
      if row = 0 then (codePoly T).eval (Benchmark.IRSProfile.domain j) else 0 := by
  exact Classical.choose_spec (exists_message T) j row
...
@[simp] theorem attackStack_zero : attackStack 0 = word0 := rfl

@[simp] theorem attackStack_one : attackStack 1 = word1 := rfl
...
theorem scalar_fold_eq_codePoly_of_mem (T : ChosenSets) (j : I)
  ...
  · have hA := padPoly_eval_zero_of_mem_padIndices j hjpad
    simp [scalarWord0, scalarWord1, codePoly_eq, Polynomial.eval_mul,
      fiberMonomial_eval, hA]
  · have hP := rootPoly_eval_zero_of_mem_rootIndices T.1 j hjroot
    ...
    rw [hP] at hpencil
    simp only [scalarWord0, scalarWord1, codePoly_eq, Polynomial.eval_mul,
      Polynomial.eval_comp, fiberMonomial_eval]
    field_simp [hW]
...
attribute [irreducible] denomPoly
...
lemma card_filter_eval_eq_zero_le_natDegree {Q : Polynomial F} (hQ : Q ≠ 0) :
  ...
  change Benchmark.IRSProfile.domain j ∈ Q.roots.toFinset
  rw [Multiset.mem_toFinset, Polynomial.mem_roots hQ]
  have hj' : j ∈ (Finset.univ.filter fun i : I =>
      Q.eval (Benchmark.IRSProfile.domain i) = 0) := hj
  simpa only [Polynomial.IsRoot.def] using (Finset.mem_filter.mp hj').2

lemma agreement_card_gt_of_lt_bridge (δ : ℝ≥0)
    (hδ : δ < ProximityGap.gridPt (ι := I) bridgeIndex) (T : Finset I)
    (hT : (T.card : ℝ≥0) ≥ (1 - δ) * (Fintype.card I : ℝ≥0)) :
    131583 < T.card := by
  ...
  have hpre : (131583 : ℝ) < (1 - (δ : ℝ)) * 262144 := by
    linarith only [hδR]
  have hcardR : (131583 : ℝ) < (T.card : ℝ) := hpre.trans_le hTR
  exact_mod_cast hcardR

lemma natDegree_le_rowBudget {q : Polynomial F} (hqdeg : q.degree < (rowK : Nat)) :
    q.natDegree ≤ 131071 := by
  by_cases hq0 : q = 0
  · simp [hq0]
...
lemma obstructionPoly_ne_zero (q : Polynomial F) (hqnat : q.natDegree ≤ 131071) :
    q * denomPoly + padPoly ≠ 0 := by
  intro hQ
  by_cases hq0 : q = 0
  · simp only [hq0, zero_mul, zero_add] at hQ
    exact padPoly_monic.ne_zero hQ
  · have hEq : padPoly = -(q * denomPoly) := by
      apply eq_neg_iff_add_eq_zero.mpr
      simpa only [add_comm] using hQ
    ...
    change 511 = q.natDegree + 512 at hnat
    omega

lemma obstructionPoly_natDegree_le (q : Polynomial F) (hqnat : q.natDegree ≤ 131071) :
    (q * denomPoly + padPoly).natDegree ≤ 131583 := by
  calc
    (q * denomPoly + padPoly).natDegree ≤
        max (q * denomPoly).natDegree padPoly.natDegree :=
      Polynomial.natDegree_add_le _ _
    _ ≤ 131583 := by
      rw [padPoly_natDegree]
      apply max_le
      · refine (Polynomial.natDegree_mul_le (p := q) (q := denomPoly)).trans ?_
        rw [denomPoly_natDegree]
        have hadd := Nat.add_le_add_right hqnat 512
        norm_num [fiberSize] at hadd ⊢
        exact hadd
      · norm_num
...
theorem attackStack_not_joint (δ : ℝ≥0)
  ...
  have hqnat : q.natDegree ≤ 131071 := natDegree_le_rowBudget hqdeg
  have hTlarge : 131583 < T.card := agreement_card_gt_of_lt_bridge δ hδ T hT
  let Q : Polynomial F := q * denomPoly + padPoly
  have hQne : Q ≠ 0 := by simpa only [Q] using obstructionPoly_ne_zero q hqnat
  have hQdeg : Q.natDegree ≤ 131583 := by
    simpa only [Q] using obstructionPoly_natDegree_le q hqnat
  have hzero : T ⊆ Finset.univ.filter fun j : I => Q.eval (Benchmark.IRSProfile.domain j) = 0 := by
    ...
    have hqeval : q.eval (Benchmark.IRSProfile.domain j) = scalarWord1 j := by
      change q.eval (Benchmark.IRSProfile.domain j) = scalarWord1 j at hrow
      exact hrow
    dsimp only [Q]
    rw [Polynomial.eval_add, Polynomial.eval_mul, hqeval, scalarWord1_eq]
...
attribute [irreducible] challenge attackStack

noncomputable def goodChallenges : Finset F :=
  (Finset.univ : Finset ChosenSets).image (fun T => challenge T.1)

theorem mem_goodChallenges_iff {x : F} :
    x ∈ goodChallenges ↔ ∃ T : ChosenSets, challenge T.1 = x := by
  rw [goodChallenges, Finset.mem_image]
  simp only [Finset.mem_univ, true_and]

theorem goodChallenges_card : goodChallenges.card = familySize := by
  rw [goodChallenges, Finset.card_image_of_injective _ challenge_injective,
    Finset.card_univ, Fintype.card_coe, chosenSets_card]

theorem goodChallenges_subset_close (δ : ℝ≥0)
    (hlower : ProximityGap.gridPt (ι := I) unsafeIndex ≤ δ)
    (hupper : δ < ProximityGap.gridPt (ι := I) bridgeIndex) :
    goodChallenges ⊆ Finset.univ.filter fun γ : F =>
      δᵣ(attackStack 0 + γ • attackStack 1,
        (Benchmark.IRSProfile.code : Set (I → Fin s → F))) ≤ (δ : ℝ≥0) := by
  intro γ hγ
  have hex := mem_goodChallenges_iff.mp hγ
  let T : ChosenSets := Classical.choose hex
  have hTγ : challenge T.1 = γ := Classical.choose_spec hex
  have hclose :
      δᵣ(attackStack 0 + challenge T.1 • attackStack 1,
        (Benchmark.IRSProfile.code : Set (I → Fin s → F))) ≤ (δ : ℝ≥0) := by
    simpa only [attackStack_zero, attackStack_one] using fold_close T δ hlower hupper
  simpa only [Finset.mem_filter, Finset.mem_univ, true_and] using (hTγ ▸ hclose)
...
theorem family_div_field_le_epsCa (δ : ℝ≥0)
  ...
  have hsubset : goodChallenges ⊆ Finset.univ.filter fun γ : F =>
      δᵣ(attackStack 0 + γ • attackStack 1,
        (Benchmark.IRSProfile.code : Set (I → Fin s → F))) ≤ (δ : ℝ≥0) := by
    exact goodChallenges_subset_close δ hlower hupper
...
theorem epsilonStar_lt_family_div_field :
  ...
  have hcoe : ((ProximityGap.prizeThreshold : ℝ≥0) : ENNReal) <
      (((familySize : ℝ≥0) / (Fintype.card F : ℝ≥0) : ℝ≥0) : ENNReal) :=
    ENNReal.coe_lt_coe.mpr hnn
  rw [ENNReal.coe_div (by exact_mod_cast Fintype.card_ne_zero)] at hcoe
  simpa only [ENNReal.coe_natCast] using hcoe
