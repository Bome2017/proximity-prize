...
/-!
# NTT-fibre rational pencil below the prescribed-top grain

The promoted prescribed-top family keys vanishing polynomials of `t = m+r`
subsets in the base field. That grain is exhausted at `r = 8430`
(`t = 139502`, unsafe index `122642`).

This module restores the NTT multiplicative grain that those patches never
used: `x ↦ x^512` partitions the size-`2^18` domain into 512 fibres of 512
points. One low-degree polynomial in `Y = X^512` then purchases 512 agreement
coordinates per combinatorial choice.

Construction (one mechanism):
* 272 full fibres plus a fixed 511-point core → agreement `139775`
* 14 top coefficients of `V_U` plus the fibre-exponent sum mod 512
* rational pencil through `0` and a separating `α`, so `deg Q ≤ 255`
* message `R_C(X) · Q(X^512)` stays strictly below the row dimension `131072`

Handoff is glued to the *existing* prescribed-top theorem at
`61321/131072 = 122642/262144`. The fibre violation lemma is stated on the
whole open interval `δ < 61321/131072` (so `(1-δ)n > 139502`), which covers
the real gap that a `122641` cut would leave. PrescribedTop.lean is not edited.
-/

namespace ProximityPrize.SubmissionUpper.FibrePencil
...
noncomputable def V (U : Finset Small) : Polynomial K :=
  ∏ b ∈ U, ((Polynomial.X : Polynomial K) - Polynomial.C (FibrePencil.y b))
...
set_option maxHeartbeats 1000000 in
set_option maxRecDepth 10000 in
theorem f2_far (δ : ℝ≥0) (hδ : δ < (61321 / 131072 : ℝ≥0)) :
    ¬ RelaxedRelationFor (ℓ := 2) IRSProfile.encoder δ (0 : Fin kk → FF)
      ![0, 0] ![f1, f2] := by
  intro hrel
  rcases hrel with ⟨W, ⟨M, hW, _hc⟩, S, hS, hA⟩
  have hd : (δ : ℝ) < 61321 / 131072 := by exact_mod_cast hδ
  have hScard : 131583 < S.card := by
    have hreal : (139502 : ℝ) < (1 - (δ : ℝ)) * 262144 := by nlinarith
    have hcardR : (139502 : ℝ) < (S.card : ℝ) := by
      calc (139502 : ℝ) < (1 - (δ : ℝ)) * 262144 := hreal
        _ ≤ (S.card : ℝ) := by simpa using hS
    have hnat : 139502 < S.card := by exact_mod_cast hcardR
    omega
...
noncomputable def attack (δ : ℝ≥0) (hhi : δ < (61321 / 131072 : ℝ≥0)) :
...
end ProximityPrize.SubmissionUpper.FibrePencil
