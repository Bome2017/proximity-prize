...
abbrev rowK := 131072
abbrev extra := 8380
abbrev agreement := 139452
abbrev signatureWidth := 8381
abbrev unsafeIndex := 122692
...
theorem base_field_pow_8393_lt_two_pow_260183 :
    (Fintype.card K) ^ 8393 < 2 ^ 260183 := by
  have hpow : ∀ q : Nat, q < 2 ^ 31 → q ^ 8393 < 2 ^ 260183 := by
    intro q hq
    calc
      q ^ 8393 < (2 ^ 31) ^ 8393 :=
        Nat.pow_lt_pow_left hq (by norm_num)
      _ = 2 ^ (31 * 8393) := (pow_mul 2 31 8393).symm
      _ = 2 ^ 260183 := by norm_num
  exact hpow (Fintype.card K) base_field_card_lt_two_pow_31

theorem signature_sieve_lt_two_pow_260183 :
  ...
  have hcombine : ∀ q : Nat,
      q ^ signatureWidth * q ^ 12 = q ^ 8393 := by
    intro q
    change q ^ 8381 * q ^ 12 = q ^ 8393
    rw [← pow_add]
  calc
    Fintype.card (Fin signatureWidth → K) * (Fintype.card F - 1) ^ 2 ≤
    ...
    _ = (Fintype.card K) ^ 8393 := hcombine (Fintype.card K)
    _ < 2 ^ 260183 := base_field_pow_8393_lt_two_pow_260183

theorem two_pow_262125_lt_central_choose :
    2 ^ 262125 < Nat.choose 262144 131072 := by
  have hcentral := Nat.four_pow_lt_mul_centralBinom 131072 (by norm_num)
  rw [Nat.centralBinom_eq_two_mul_choose] at hcentral
  norm_num only [Nat.reduceMul] at hcentral
  by_contra hnot
  have hle : Nat.choose 262144 131072 ≤ 2 ^ 262125 :=
    Nat.le_of_not_gt hnot
  have hbad : 4 ^ 131072 < 131072 * 2 ^ 262125 :=
    lt_of_lt_of_le hcentral (Nat.mul_le_mul_left 131072 hle)
  have hreverse : 131072 * 2 ^ 262125 < 4 ^ 131072 := by
    calc
      131072 * 2 ^ 262125 < 2 ^ 18 * 2 ^ 262125 := by
        gcongr <;> norm_num
      _ = 2 ^ (18 + 262125) := (pow_add 2 18 262125).symm
      _ < 2 ^ (2 * 131072) :=
        Nat.pow_lt_pow_right (by norm_num) (by norm_num)
      _ = (2 ^ 2) ^ 131072 := pow_mul 2 2 131072
      _ = 4 ^ 131072 := by norm_num
  exact (Nat.not_lt_of_ge hreverse.le) hbad

theorem scaled_sequence_step (f : Nat → Nat) (c : Nat) (hc : c < 8380)
    (ih : f 0 * 7 ^ c ≤ f c * 8 ^ c)
    (hrec : f (c + 1) * (131072 + c + 1) = f c * (131072 - c)) :
    f 0 * 7 ^ (c + 1) ≤ f (c + 1) * 8 ^ (c + 1) := by
  apply Nat.le_of_mul_le_mul_right (c := 131072 + c + 1) ?_ (by positivity)
  calc
    (f 0 * 7 ^ (c + 1)) * (131072 + c + 1) =
        (f 0 * 7 ^ c) * (7 * (131072 + c + 1)) := by
          simp only [pow_succ, mul_assoc, mul_left_comm, mul_comm]
    _ ≤ (f c * 8 ^ c) * (8 * (131072 - c)) := by
          exact Nat.mul_le_mul ih (by omega)
    _ = (f (c + 1) * 8 ^ (c + 1)) * (131072 + c + 1) := by
          rw [pow_succ]
          calc
            (f c * 8 ^ c) * (8 * (131072 - c)) =
                (f c * (131072 - c)) * (8 ^ c * 8) := by
                  simp only [mul_assoc, mul_left_comm, mul_comm]
            _ = (f (c + 1) * (131072 + c + 1)) * (8 ^ c * 8) := by rw [hrec]
            _ = (f (c + 1) * (8 ^ c * 8)) * (131072 + c + 1) := by
                  simp only [mul_assoc, mul_left_comm, mul_comm]

theorem scaled_sequence (f : Nat → Nat)
    (hrec : ∀ c < 8380,
      f (c + 1) * (131072 + c + 1) = f c * (131072 - c))
    (c : Nat) (hc : c ≤ 8380) :
    f 0 * 7 ^ c ≤ f c * 8 ^ c := by
  induction c with
  | zero => simp
  | succ c ih =>
      exact scaled_sequence_step f c (by omega) (ih (by omega)) (hrec c (by omega))

theorem choose_sequence_recurrence (c : Nat) (hc : c < 8380) :
    Nat.choose 262144 (131072 + (c + 1)) * (131072 + c + 1) =
      Nat.choose 262144 (131072 + c) * (131072 - c) := by
  have h := Nat.choose_succ_right_eq 262144 (131072 + c)
  have hsub : 262144 - (131072 + c) = 131072 - c := by omega
  rw [hsub] at h
  simpa only [Nat.add_assoc] using h

theorem central_scaled_le_offcenter (c : Nat) (hc : c ≤ 8380) :
    Nat.choose 262144 131072 * 7 ^ c ≤
      Nat.choose 262144 (131072 + c) * 8 ^ c := by
  simpa only [Nat.add_zero] using scaled_sequence
    (fun j => Nat.choose 262144 (131072 + j)) choose_sequence_recurrence c hc

theorem two_pow_23464_lt_seven_pow : 2 ^ 23464 < 7 ^ 8380 := by
  have hsmall : 2 ^ 14 < 7 ^ 5 := by norm_num
  have hpow := Nat.pow_lt_pow_left hsmall (by norm_num : (1676 : Nat) ≠ 0)
  simpa only [← pow_mul] using hpow

theorem lt_of_lt_of_le_of_le {a b c d : Nat}
    (hab : a < b) (hbc : b ≤ c) (hcd : c ≤ d) : a < d :=
  hab.trans_le (hbc.trans hcd)

theorem lt_trans_opaque {a b c : Nat} (hab : a < b) (hbc : b < c) : a < c :=
  hab.trans hbc

theorem lt_of_lt_of_lt_of_lt {a b c d : Nat}
    (hab : a < b) (hbc : b < c) (hcd : c < d) : a < d :=
  hab.trans (hbc.trans hcd)

theorem two_pow_260448_lt_offcenter_choose :
    2 ^ 260448 < Nat.choose 262144 139452 := by
  by_contra hnot
  have hoff : Nat.choose 262144 139452 ≤ 2 ^ 260448 := Nat.le_of_not_gt hnot
  have hscale := central_scaled_le_offcenter 8380 (by norm_num)
  norm_num only [Nat.reduceAdd] at hscale
  have hlower : 2 ^ 285589 < Nat.choose 262144 131072 * 7 ^ 8380 := by
    calc
      2 ^ 285589 = 2 ^ 262125 * 2 ^ 23464 := by rw [← pow_add]
      _ < Nat.choose 262144 131072 * 2 ^ 23464 :=
        Nat.mul_lt_mul_of_pos_right two_pow_262125_lt_central_choose (by positivity)
      _ < Nat.choose 262144 131072 * 7 ^ 8380 :=
        Nat.mul_lt_mul_of_pos_left two_pow_23464_lt_seven_pow
          (Nat.choose_pos (by norm_num))
  have hupper : Nat.choose 262144 139452 * 8 ^ 8380 ≤ 2 ^ 285588 := by
    calc
      Nat.choose 262144 139452 * 8 ^ 8380 ≤ 2 ^ 260448 * 8 ^ 8380 :=
        Nat.mul_le_mul_right _ hoff
      _ = 2 ^ 285588 := by
        rw [show (8 : Nat) = 2 ^ 3 by norm_num, ← pow_mul, ← pow_add]
  have hbad : 2 ^ 285589 < 2 ^ 285588 :=
    lt_of_lt_of_le_of_le
      (a := 2 ^ 285589)
      (b := Nat.choose 262144 131072 * 7 ^ 8380)
      (c := Nat.choose 262144 139452 * 8 ^ 8380)
      (d := 2 ^ 285588) hlower hscale hupper
  exact (Nat.not_lt_of_ge
    (Nat.pow_le_pow_right (by norm_num : (0 : Nat) < 2) (by norm_num))) hbad

theorem two_pow_260448_lt_card_candidateSets :
    2 ^ 260448 < Fintype.card CandidateSets := by
  have hconcrete : 2 ^ 260448 < Nat.choose 262144 139452 :=
    two_pow_260448_lt_offcenter_choose
...
theorem signature_card_mul_field_sq_lt :
  ...
  lt_of_lt_of_lt_of_lt
    (a := Fintype.card (Fin signatureWidth → K) * (Fintype.card F - 1) ^ 2)
    (b := 2 ^ 260183) (c := 2 ^ 260448) (d := Fintype.card CandidateSets)
    signature_sieve_lt_two_pow_260183
    (Nat.pow_lt_pow_right (by norm_num) (by norm_num))
    two_pow_260448_lt_card_candidateSets
